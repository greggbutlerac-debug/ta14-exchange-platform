...
type InstitutionalMetrics = {
  registeredGovernances:number|null;
  publishedShowrooms:number|null;
  countriesObserved:number|null;
  academyVisits:number|null;
  showroomVisits:number|null;
  afaEabaReceipts:number|null;
  afaEabaReports:number|null;
};
type SiteActivityResponse = { counted:boolean; newVisitor?:boolean; visitors?:number; pageViews?:number; updatedAt?:string; institutional?:InstitutionalMetrics };
type SiteActivityState = { visitors:number|null; pageViews:number|null; institutional:InstitutionalMetrics|null; status:"loading"|"ready"|"unavailable" };
...
export function SiteActivityCounter(){
  ...
  const [activity,setActivity]=useState<SiteActivityState>({visitors:null,pageViews:null,institutional:null,status:"loading"});
  ...
  useEffect(()=>{
    ...
    async function recordActivity(){
      try{
        ...
        setActivity({visitors:payload.visitors,pageViews:payload.pageViews,institutional:payload.institutional??null,status:"ready"});
  ...
  const institutional=[
    {label:"Registered governance records",value:activity.institutional?.registeredGovernances??null},
  ...
  return <section aria-label="TA-14 Exchange public activity" className={`${styles.dashboard} ${activity.status==="loading"?styles.loading:""}`}>
    <div className={styles.inner}>
      ...
      <div className={styles.scoreboard}>
        <div className={styles.scoreboardHead}><span>Institutional state</span><strong>LIVE LEDGER COUNTS</strong></div>
        <div className={styles.scoreboardGrid}>{institutional.map(metric=><div className={styles.scoreItem} key={metric.label}><strong>{displayCount(metric.value,activity.status)}</strong><span>{metric.label}</span></div>)}</div>
        <p className={styles.boundary}>Activity and institutional-state counts only. Registration is not certification. Traffic is not execution evidence. Examination counts establish recorded artifacts, not universal efficacy.</p>
      </div>
