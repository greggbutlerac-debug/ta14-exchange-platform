...
export function CommercialTrialConversion(){
  ...
  return <aside aria-label="14-day TA-14 trial" style={{position:'fixed',left:'max(16px,calc((100vw - 1280px)/2))',bottom:18,zIndex:45,width:'min(430px,calc(100vw - 32px))',padding:18,border:'1px solid rgba(107,224,255,.48)',borderRadius:16,background:'linear-gradient(145deg,rgba(3,15,25,.98),rgba(6,31,44,.98))',boxShadow:'0 24px 70px rgba(0,0,0,.48)',color:'#eaf8ff',fontFamily:'Inter,system-ui,sans-serif'}}>
    ...
    <p style={{margin:'0 0 13px',fontSize:11,lineHeight:1.55,color:'#9db5c4'}}>Preserve the work, bring a real AI system, and continue governing evidence and consequence. 14 days free. No credit card. No contract.</p>
    <Link href={href} onClick={()=>signal(pathname,href,'Contextual 14-day trial CTA')} style={{display:'block',padding:'13px 15px',textAlign:'center',textDecoration:'none',background:'#86e9ff',color:'#021018',fontSize:10,fontWeight:1000,letterSpacing:'.08em'}}>START MY 14 DAYS FREE →</Link>
    <small style={{display:'block',marginTop:9,textAlign:'center',color:'#688596',fontSize:9}}>Then choose a paid plan only if you decide to continue.</small>
  </aside>;
}
