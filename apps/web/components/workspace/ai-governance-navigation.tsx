...
const gapIxcHref='/workspace/ai-governance/gap-ixc';
...
const workspaceNavigation:NavigationItem[]=[
 ...
 {href:gapIxcHref,label:'GAP-IXC Assurance',glyph:'GX',matchPrefixes:[gapIxcHref],institutional:true},
...
const mobileLabels=new Set(['Mission Control','AI Governance Home','GAP-IXC Assurance','Adversarial Examination','Consequence Examination','Founding Demonstrations','Interoperability Examinations','Artifact Registry']);
...
function isItemActive(pathname:string,item:NavigationItem){
 if(item.href==='/workspace/routes')return pathname==='/workspace/routes'||(pathname.startsWith('/workspace/routes/')&&!pathname.startsWith('/workspace/routes/new'));
 if(item.href===aiGovernanceHomeHref)return pathname===aiGovernanceHomeHref;
 if(item.href===registryHref)return pathname===registryHref||(pathname.startsWith(`${registryHref}/`)&&!pathname.startsWith(registryInboxHref));
 const prefixes=item.matchPrefixes??[item.href];return prefixes.some(prefix=>pathname===prefix||pathname.startsWith(`${prefix}/`));
}

export function AiGovernanceNavigation(){const pathname=usePathname();return <>
 <aside className="ta14-sidebar" aria-label="TA-14 institutional and AI Governance workspace navigation">
  <section><span className="ta14-nav-label">Institution</span><nav className="ta14-nav-list">{workspaceNavigation.map(item=>{const active=isItemActive(pathname,item);return <Link aria-current={active?'page':undefined} className={`ta14-nav-item${active?' active':''}${item.partner?' partner':''}${item.institutional?' institutional':''}`} href={item.href} key={item.href}><span className="ta14-nav-glyph" aria-hidden="true">{item.glyph}</span><span>{item.label}</span></Link>})}</nav></section>
  <article className="ta14-sidebar-note"><small>GAP-IXC Assurance</small><strong>What has actually been established?</strong><p>Freeze the proposition, admit evidence, separate governance basis, authority, proposition support, implementation, execution, and consequence, then preserve the qualification and independence boundary with every state.</p><Link href={gapIxcHref}>Open GAP-IXC Assurance →</Link></article>
  ...
  <article className="ta14-sidebar-note"><small>TA-14 Partner Review Network</small><strong>Independent architectures. Written boundaries.</strong><p>Explore the current partner-review pathways and learn what each governance architecture contributes.</p><Link href={partnerReviewNetworkHref}>Explore the network →</Link></article>
 </aside>
 <nav className="ta14-mobile-nav" aria-label="Mobile institutional and AI Governance navigation">{mobileNavigation.map(item=>{const active=isItemActive(pathname,item);return <Link aria-current={active?'page':undefined} className={`ta14-mobile-link${active?' active':''}${item.partner?' partner':''}${item.institutional?' institutional':''}`} href={item.href} key={item.href}><b aria-hidden="true">{item.glyph}</b><span>{item.label}</span></Link>})}</nav>
 </>}
