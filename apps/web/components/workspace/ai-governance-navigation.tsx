...
const mobileLabels = new Set([
  'Mission Control', 'AI Governance Home', 'Playground', 'Provenance Reconciliation',
  'Adversarial Examination', 'Consequence Examination', 'EU AI Act · World 05',
  'Founding Demonstrations', 'Interoperability Examinations', 'Reviews & Responses',
  'Artifact Registry', 'Institutional Authority', 'Registry Inbox',
]);
...
export function AiGovernanceNavigation() {
  ...
  return <>
    <aside className="ta14-sidebar" aria-label="TA-14 institutional and AI Governance workspace navigation">
      <section><span className="ta14-nav-label">Institution</span><nav className="ta14-nav-list">
        {workspaceNavigation.map((item) => { const active = isItemActive(pathname, item); return <Link aria-current={active ? 'page' : undefined} className={`ta14-nav-item${active ? ' active' : ''}${item.partner ? ' partner' : ''}${item.institutional ? ' institutional' : ''}`} href={item.href} key={item.href}><span className="ta14-nav-glyph" aria-hidden="true">{item.glyph}</span><span>{item.label}</span></Link>; })}
      </nav></section>

      <article className="ta14-sidebar-note"><small>Provenance Reconciliation</small><strong>Overlap is not a finding.</strong><p>Registered architectures can petition for a bounded provenance record, preserve evidence, identify registered or external parties, and place neutral outreach under TA-14 institutional control.</p><Link href={provenanceHref}>Open Provenance Reconciliation →</Link></article>
    ...
    <nav className="ta14-mobile-nav" aria-label="Mobile institutional and AI Governance navigation">
      {mobileNavigation.map((item) => { const active = isItemActive(pathname, item); return <Link aria-current={active ? 'page' : undefined} className={`ta14-mobile-link${active ? ' active' : ''}${item.partner ? ' partner' : ''}${item.institutional ? ' institutional' : ''}`} href={item.href} key={item.href}><b aria-hidden="true">{item.glyph}</b><span>{item.label}</span></Link>; })}
    </nav>
  </>;
}
