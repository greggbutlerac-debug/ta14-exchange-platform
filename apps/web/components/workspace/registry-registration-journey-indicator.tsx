...
export function RegistryRegistrationJourneyIndicator({
  ...
  const inactive = summary?.stalled ?? 0;
  const incomplete = summary?.needsAttention ?? 0;

  const label = loading
    ? 'Loading registration pipeline'
    : unavailable
      ? 'Registration pipeline unavailable'
      : failed > 0
        ? `${failed} registration ${failed === 1 ? 'journey requires' : 'journeys require'} review`
        : active > 0 || inactive > 0
          ? `${active} recently active · ${inactive} inactive 24h+`
          : 'No incomplete registration journeys';
  ...
  return (
    <div
      className={[
        ...
        .join(' ')}
      aria-label={label}
      title="Administrative registration pipeline. Inactive 24h+ means an incomplete journey has had no newer activity for at least 24 hours; it does not mean the registration failed."
    >
      <div className="ta14-registration-journey-indicator__heading">
        ...
        <div>
          ...
          {!loading && !unavailable && summary ? (
            <span className="ta14-registration-journey-indicator__context">
              {incomplete} incomplete journey{incomplete === 1 ? '' : 's'} · administrative telemetry only
            </span>
          ) : null}
        </div>
      </div>
      ...
      {!loading && !unavailable && summary ? (
        <div className="ta14-registration-journey-indicator__metrics">
          ...
          <span><strong>{summary.draftSaved}</strong>Draft</span>
          <span><strong>{summary.submitted}</strong>Submitted</span>
          <span><strong>{summary.registered}</strong>Registered</span>
          <span><strong>{summary.inProgress}</strong>Recently active</span>
          <span><strong>{summary.stalled}</strong>Inactive 24h+</span>
          <span className={failed > 0 ? 'metric-alert' : ''}>
      ...
      <style jsx>{`
        ...
        .ta14-registration-journey-indicator.is-unavailable { opacity: 0.58; }
        ...
        .has-active .ta14-registration-journey-indicator__signal {
          box-shadow: 0 0 14px currentColor;
          opacity: 0.92;
        }
        .has-failed .ta14-registration-journey-indicator__signal {
          box-shadow: 0 0 14px currentColor;
          opacity: 1;
        }
        ...
        .ta14-registration-journey-indicator__context {
          display: block;
          margin-top: 3px;
          font-size: 8px;
          line-height: 1.35;
        ...
        .ta14-registration-journey-indicator__metrics .metric-alert {
