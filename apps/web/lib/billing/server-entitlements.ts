...
export type EuAiActEntitlement={authenticated:boolean;userId:string|null;email:string|null;tier:EuAiActTier;planKey:string|null;subscriptionId:string|null;status:string;capabilities:EuAiActCapability[];source:'verified_subscription'|'free';};
...
export async function getCurrentEuAiActEntitlement():Promise<EuAiActEntitlement>{
 ...
 if(!active)return{authenticated:true,userId:user.id,email:user.email??null,tier:'free',planKey:null,subscriptionId:null,status:rows[0]?.status??'FREE',capabilities:capabilitiesForTier('free'),source:'free'};
 const planKey=active.plan_key??planKeyFromPayPalPlanId(active.paypal_plan_id),tier=tierFromPlanKey(planKey);
 if(!active.owner_user_id){await admin.from('ta14_billing_subscriptions').update({owner_user_id:user.id,plan_key:planKey,updated_at:new Date().toISOString()}).eq('paypal_subscription_id',active.paypal_subscription_id).is('owner_user_id',null)}
 return{authenticated:true,userId:user.id,email:user.email??null,tier,planKey,subscriptionId:active.paypal_subscription_id,status:active.status,capabilities:capabilitiesForTier(tier),source:'verified_subscription'};
}
