...
export default function RuntimeScenarioRunnerPage() {
  ...
  const [evidenceAttachments, setEvidenceAttachments] =
    useState<RuntimeEvidenceSummary[]>([]);
  ...
  const evidenceBinding = useMemo(
    ...
    [evidenceAttachments, observedGateResults],
  );

  const evidenceCounts = useMemo(
    () => evidenceAttachmentCounts(evidenceAttachments),
    [evidenceAttachments],
  );

  const automaticDetermination = useMemo(
    () =>
      determineFromGateResults(evidenceBinding.gateResults, {
        designatedEscalationPresent:
          evidenceBinding.gateResults.some(
            (result) => result.status === "ESCALATED",
          ),
      }),
    [evidenceBinding.gateResults],
  );
  ...
  const scenarioRun = useMemo<ScenarioRun | null>(() => {
    ...
    return {
      ...
      gateResults: evidenceBinding.gateResults,
  ...
  useEffect(() => {
    const activeSession = loadRuntimeTestSession() ?? null;

    setStoredRuns(
      listStoredScenarioRuns(RUNTIME_EXECUTION_LANE.laneId),
    );
    setTestSession(activeSession);
    setEvidenceAttachments(
      listRuntimeEvidenceAttachments(
        RUNTIME_EXECUTION_LANE.laneId,
        activeSession
          ? {
              routeDraftId: activeSession.routeDraftId,
              testSessionId: activeSession.sessionId,
            }
          : {},
      ),
    );
    ...
    const storedRun = saveStoredScenarioRun(
      createStoredScenarioRun({
        ...
        metadata: {
          ...
          evidenceAttachmentCount: evidenceCounts.total,
          supportingEvidenceCount:
            evidenceCounts.supporting,
          conflictingEvidenceCount:
            evidenceCounts.conflicting,
          gateBoundEvidenceCount:
            evidenceCounts.gateBound,
          unboundEvidenceIds:
            evidenceBinding.unboundEvidenceIds,
        },
      }),
    );
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto w-full max-w-[92rem] px-5 py-6 lg:px-8">
        ...
        <section className="mb-6 rounded-3xl border border-emerald-300/20 bg-emerald-300/[0.04] p-5 md:p-6">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">
                Evidence bound to testing
              </p>
              <h2 className="mt-2 text-xl font-black">
                {evidenceCounts.total} evidence candidate
                {evidenceCounts.total === 1 ? "" : "s"}
              </h2>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                {evidenceCounts.supporting} supporting ·{" "}
                {evidenceCounts.conflicting} conflicting ·{" "}
                {evidenceCounts.gateBound} gate-bound ·{" "}
                {evidenceCounts.routeLevel} route-level
              </p>
            </div>

            <Link
              href="/ai-governance/playground/runtime-execution/evidence"
              className="rounded-xl border border-emerald-300/30 px-5 py-3 text-sm font-bold text-emerald-100 transition hover:border-emerald-300/60 hover:bg-emerald-300/[0.06]"
            >
              Manage Evidence
            </Link>
          </div>

          {evidenceBinding.unboundEvidenceIds.length > 0 ? (
            <p className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-3 text-xs leading-5 text-amber-100/75">
              {evidenceBinding.unboundEvidenceIds.length} evidence candidate
              {evidenceBinding.unboundEvidenceIds.length === 1 ? "" : "s"}{" "}
              remain route-level or otherwise unbound to a tested gate.
            </p>
          ) : null}
        </section>
