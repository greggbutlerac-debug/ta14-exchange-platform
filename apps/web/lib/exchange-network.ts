...
export type ExchangeNetworkRoute = {
  id: string;
  routeId: string;
  title: string;
  domain: string;
...
export type ExchangeNetworkEvent = {
  id: string;
  routeId: string;
  eventType: string;
  eventState: string;
...
export type ExchangeNetworkSnapshot = {
  routes: ExchangeNetworkRoute[];
  events: ExchangeNetworkEvent[];
...
export async function getExchangeNetworkSnapshot(): Promise<ExchangeNetworkSnapshot> {
  ...
  const [routesResult, eventsResult] = await Promise.all([
    supabase
      ...
      .select("id,route_id,route_name,domain,owner,status,version,route_data,updated_at")
      .order("updated_at", { ascending: false })
      .limit(100),
    supabase
      .from("exchange_route_events")
      .select("id,route_id,event_type,event_state,summary,actor_label,event_data,occurred_at")
  ...
  const routes = ((routesResult.data ?? []) as RouteRow[]).map((row) => ({
    id: row.id,
  ...
  return { routes, events, eventCounts };
}
