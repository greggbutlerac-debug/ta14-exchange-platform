'use client';

import Link from 'next/link';
import { createPortal } from 'react-dom';
import { useEffect, useState } from 'react';
import AtlasFrontDoor from './atlas-front-door/page';

export default function HomePage() {
  const [worldGrid, setWorldGrid] = useState<Element | null>(null);

  useEffect(() => {
    const grid = document.querySelector('.worldGrid');
    setWorldGrid(grid);

    const heading = document.querySelector('.worldDeck .sectionHeading.compact small');
    if (heading) heading.textContent = 'ONE INSTITUTION · FIVE GOVERNED WORLDS';
  }, []);

  return (
    <>
      <AtlasFrontDoor />
      {worldGrid
        ? createPortal(
            <Link
              href="/eu-ai-act"
              aria-label="Enter EU AI Act World"
              style={{
                gridColumn: '1 / -1',
                minHeight: 330,
                marginTop: 22,
                padding: '42px clamp(24px,5vw,72px)',
                border: '1px solid rgba(105,158,255,.72)',
                background:
                  'radial-gradient(circle at 50% 0%,rgba(47,93,184,.32),transparent 42%),linear-gradient(135deg,rgba(5,18,42,.98),rgba(8,24,55,.96) 52%,rgba(4,12,29,.99))',
                boxShadow:
                  'inset 0 0 60px rgba(69,126,255,.12),0 0 34px rgba(69,126,255,.16)',
                color: '#eef5ff',
                textDecoration: 'none',
                position: 'relative',
                overflow: 'hidden',
                display: 'grid',
                gridTemplateColumns: 'minmax(120px,220px) 1fr auto',
                gap: 32,
                alignItems: 'center',
              }}
            >
              <div style={{ textAlign: 'center' }}>
                <div
                  style={{
                    width: 118,
                    height: 118,
                    margin: '0 auto 14px',
                    borderRadius: '50%',
                    display: 'grid',
                    placeItems: 'center',
                    border: '1px solid rgba(130,177,255,.8)',
                    background:
                      'radial-gradient(circle at 34% 26%,#fff,#76a8ff 8%,#173c82 42%,#030a18 76%)',
                    boxShadow: '0 0 34px rgba(86,142,255,.55)',
                    fontFamily: 'Georgia,serif',
                    fontSize: 34,
                    fontWeight: 900,
                  }}
                >
                  EU
                </div>
                <small style={{ color: '#8dbbff', letterSpacing: '.16em', fontWeight: 900 }}>
                  GOVERNED WORLD 05
                </small>
              </div>

              <div>
                <small style={{ color: '#8dbbff', letterSpacing: '.18em', fontWeight: 900 }}>
                  LAW · REGULATION · EVIDENCE · READINESS
                </small>
                <h2
                  style={{
                    margin: '10px 0 12px',
                    fontFamily: 'Georgia,serif',
                    fontSize: 'clamp(34px,4vw,58px)',
                    lineHeight: .95,
                    color: '#f4f8ff',
                  }}
                >
                  EU AI ACT WORLD
                </h2>
                <p style={{ margin: 0, maxWidth: 760, color: '#b9c9df', lineHeight: 1.65, fontSize: 13 }}>
                  Enter the governed law world through the EU AI Act. Classify systems, map applicable requirements,
                  build evidence, preserve readiness and obligations, then move into the Law Library, current law,
                  proposed law, Academy pathways, regulatory analysis, and future jurisdictional instruments.
                </p>
                <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap', marginTop: 20 }}>
                  {['EU AI ACT','LAW LIBRARY','CURRENT LAW','PROPOSED LAW','LAW ACADEMY','REGULATORY EVIDENCE'].map(item => (
                    <span
                      key={item}
                      style={{
                        padding: '7px 9px',
                        border: '1px solid rgba(128,170,242,.24)',
                        color: '#a9c8fa',
                        fontSize: 7,
                        letterSpacing: '.1em',
                        fontWeight: 900,
                      }}
                    >
                      {item}
                    </span>
                  ))}
                </div>
              </div>

              <strong
                style={{
                  minWidth: 180,
                  padding: '17px 18px',
                  border: '1px solid rgba(126,172,255,.68)',
                  background: 'linear-gradient(180deg,rgba(62,111,205,.28),rgba(5,15,34,.94))',
                  color: '#cfe0ff',
                  textAlign: 'center',
                  fontSize: 10,
                  letterSpacing: '.12em',
                }}
              >
                ENTER LAW WORLD →
              </strong>
            </Link>,
            worldGrid,
          )
        : null}
    </>
  );
}
