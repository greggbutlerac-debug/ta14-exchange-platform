...
const cards = [
  ...
  },
  {
    ...
    description: "Preserve an approved Runtime governed-record candidate, generate its SHA-256 integrity envelope, and create an immutable preserved record.",
    status: "Ready",
  },
  {
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {cards.map((card) => (
            <Link
              key={card.href}
              href={card.href}
              className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 hover:bg-white/[0.07]"
            >
              <div className="flex items-center justify-between">
                <h2 className="font-semibold">{card.title}</h2>
                <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-1 text-xs">
                  {card.status}
                </span>
              </div>
              <p className="mt-4 text-sm text-white/65">{card.description}</p>
            </Link>
          ))}
        </div>
      </div>
