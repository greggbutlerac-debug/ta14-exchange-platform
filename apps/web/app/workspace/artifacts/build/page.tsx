...
function StudioPage() {
  ...
  function renderAdmissibility() {
    return <section><SectionTitle eyebrow="Admissibility / binding" title="May this material support this consequence?" description="Validity alone is not enough. Test purpose, time, jurisdiction, route, authority, disclosure, conflict, and consequence fit." actions={<button onClick={evaluateAllGates}>Re-evaluate</button>} />
      ...
      <div className="two-col"><article className="panel"><h3>Binding policy basis</h3>{splitLines(snapshot.route.policyBasis).map((item) => <p className="binding-rule" key={item}>{item}</p>)}</article><article className="panel"><h3>Admissibility stop conditions</h3>{REASON_CODES.filter((r) => ["ADMISSIBILITY","EVIDENCE","AUTHORITY","CONTINUITY"].some((x) => r.domain.includes(x))).slice(0, 16).map((r) => <div className="reason" key={r.code}><Badge tone={r.defaultDetermination.toLowerCase()}>{r.defaultDetermination}</Badge><strong>{r.code}</strong><p>{r.description}</p></div>)}</article></div>
    </section>;
  }
  ...
  return (
    <main className="studio-shell" onDragOver={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={onDrop}>
      <header className="topbar"><div><Link href="/artifacts" className="brand">TA-14 <span>Execution Artifacts</span></Link><span className="engine">Engine {ARTIFACT_ENGINE_VERSION}</span></div><div className="top-actions"><input id="studio-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search templates and controls /" /><button onClick={undo} disabled={!history.length}>Undo</button><button onClick={() => importRef.current?.click()}>Import</button><button onClick={exportJson}>Export</button><button className="primary" onClick={() => setShowJson(true)}>Inspect JSON</button></div></header>
