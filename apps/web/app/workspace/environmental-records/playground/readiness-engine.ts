...
function absent(text:string, subject:string){const esc=subject.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');return new RegExp(`(?:no|not|without|missing|unknown|unavailable|not attached|not supplied|not provided|not recorded|not included|not identified|does not identify|does not include|does not state|is not stated|is not attached|was not supplied|were not recorded)[^\\n.]{0,90}${esc}|${esc}[^\\n.]{0,90}(?:missing|unknown|unavailable|not attached|not supplied|not provided|not recorded|not included|not identified|not stated|not established)`,`i`).test(text)}
function affirmative(text:string,positive:RegExp,negative:RegExp){return positive.test(text)&&!negative.test(text)}
...
export function examineEnvironmentalReadiness(input:{text:string;inspectionObject:string;proposition:string;recordClass:string}):ReadinessReport{
 ...
 const sourceIdentity=has(lower,/(record id|air id|device id|device:|sensor id|instrument id|laboratory|technician|facility:|building:|site:|manufacturer|serial)/i);
 add('source_identity','Source identity',sourceIdentity?'ESTABLISHED':'MISSING',sourceIdentity?'The submitted package contains identifiable source, facility, device, or record identity.':'Reliable source or record identity is not established.',/(record id|air id|device id|sensor id|instrument id|facility:|building:|site:|serial)/i);
 const datePattern=/(\b20\d{2}[-\/]\d{1,2}[-\/]\d{1,2}\b|\b(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2},?\s+20\d{2}\b|\b\d{1,2}\s+(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+20\d{2}\b)/i;
 const timeBoundary=datePattern.test(lower)||has(lower,/(\b\d{1,2}:\d{2}\b|monitoring period|record boundary|observation boundary|timestamp|from .* to|through)/i);
 add('time_boundary','Time boundary',timeBoundary?'ESTABLISHED':'MISSING',timeBoundary?'A temporal observation boundary is present.':'A sufficiently clear temporal boundary is not established.',/(20\d{2}|\d{1,2}:\d{2}|monitoring period|record boundary|observation boundary|timestamp)/i);
 ...
 const thresholdPositive=has(lower,/(threshold source:\s*\S|reference standard:\s*\S|declared .*threshold[^\n]*:\s*\d|ashrae\s*\d|epa\s+[^\n]+|who\s+[^\n]+|osha\s+[^\n]+|declared limit:\s*\d|guideline:\s*\d)/i);
 ...
 const objectText=input.inspectionObject.trim(), objectLower=objectText.toLowerCase(), objectIdentity=objectText.length>=20;
 ...
 const objectTemporal=datePattern.test(objectLower)||has(objectLower,/(\b\d{1,2}:\d{2}\b|from .* to|through|during|period|interval|at time|between)/i);
 ...
 const objectCondition=has(objectLower,/(condition|concentration|level|reading|relationship|differential|state|exposure|event|excursion|performance|operation|quality|contamination|temperature|humidity|pressure|pm2\.?5|particulate|co2|voc|radon|moisture|response|intervention|outcome)/i);
 ...
 const conflictNegative=has(lower,/(conflicting evidence\s*:\s*(none|none identified|no|not identified)|material contradictory evidence\s*:\s*(none|none identified|no|not identified)|no (material )?(conflicting|contradictory) evidence|conflicting (records|readings|evidence)[^\n]{0,50}(none|not identified|absent))/i);
 const conflictPositive=has(lower,/(conflicting (records|readings|evidence)\s*:\s*(present|yes|identified)|contradictory (records|readings|evidence)\s*:\s*(present|yes|identified)|sensor disagreement\s*:\s*(present|yes|identified)|measurements disagree)/i);
 const conflict=conflictPositive&&!conflictNegative;
 add('conflict','Conflicting evidence',conflict?'CONFLICT':'NOT_APPLICABLE',conflict?'The package affirmatively declares conflicting evidence requiring resolution or explicit bounding.':'No affirmative evidentiary conflict is established.',/(conflicting|contradictory|sensor disagreement|measurements disagree)/i);
 ...
 const authorityPositive=has(lower,/(authority record id:\s*\S|authorized by:\s*\S|approved by:\s*\S|responsible party:\s*\S|owner authorization:\s*\S|engineer of record:\s*\S|permit:\s*\S)/i);
 add('authority','Authority / reliance boundary',authorityAbsent?'MISSING':authorityPositive?'ESTABLISHED':'MISSING',authorityAbsent?'Authority for consequential intervention is explicitly absent or not established.':authorityPositive?'An affirmative authority or responsibility boundary is present.':'Authority for consequential reliance is not established.',/(authority record id|authorized by|authority status|approved by|responsible party|owner authorization|engineer of record|permit)/i);
 const changed=affirmative(lower,/(changed condition id:\s*\S[\s\S]{0,700}(observed condition|changed-condition state|changed condition:\s*(present|established)))/i,/(no governed changed-condition evaluation|changed condition.{0,60}(not established|missing|absent))/i);
 const validation=affirmative(lower,/(validation id:\s*\S[\s\S]{0,900}(pre-execution evidence state|validation time))/i,/(pre-execution validation.{0,60}(not performed|missing|absent))/i);
 const binding=affirmative(lower,/(binding record id:\s*\S[\s\S]{0,700}bound (evidence|changed condition|authority|permitted consequence))/i,/(binding.{0,60}(not performed|missing|absent)|no .*binding)/i);
 const revalidation=affirmative(lower,/(pre-commit revalidation[\s\S]{0,900}(revalidation determination|authority status)[\s\S]{0,120}(allow|hold|deny|escalate))/i,/(revalidation.{0,60}(not performed|missing|absent))/i);
 const commit=affirmative(lower,/(commit record id:\s*\S[\s\S]{0,500}(authorized execution|execution boundary))/i,/(commit.{0,60}(not issued|missing|absent))/i);
 const execution=affirmative(lower,/(execution event id:\s*\S[\s\S]{0,700}(execution result|command after intervention|completed within bound authority))/i,/(execution.{0,60}(not performed|not initiated|missing|absent))/i);
 const outcome=affirmative(lower,/(outcome record id:\s*\S[\s\S]{0,900}(observed post-intervention condition|post-intervention observations|observed state))/i,/(post-intervention outcome record.{0,60}(not|missing|absent)|no post-intervention outcome)/i);
 ...
 const weights:Record<string,number>={source_identity:1,time_boundary:1,location:1,calibration:1.5,continuity:1.5,provenance:1.5,threshold_source:1,object_localization:1.5,proposition:1,authority:1.5,intervention_chain:2.5,conflict:1.5};
 ...
 if(!text||text.length<40)state='INSUFFICIENT_OR_INCONCLUSIVE'; else if(objectStatus!=='ESTABLISHED')state='NOT_YET_GOVERNED'; else if(conflicts>0||criticalWeak>=4||missing>=6)state='NOT_YET_GOVERNED'; else if(criticalWeak>=2||missing>=3||partial>=2)state='PARTIALLY_GOVERNABLE'; else if(missing>=1||partial>=1)state='READY_FOR_ADMISSIBILITY_REVIEW'; else state='ADMISSIBLE_FOR_DECLARED_INTERPRETATION';
 ...
 if(interventionStatus==='ESTABLISHED')supportedNow.push('The package preserves a bounded governed chronology from changed condition through validation, authority, binding, revalidation, commit, execution, outcome, and closure.');
 const prohibitedInferences=['Do not convert environmental observations into a medical diagnosis or health outcome.','Do not convert a reported classification or threshold comparison into causal attribution without separate evidence.','Do not treat missing, declared-only, or unparsed evidence as if it were inspected.','Do not treat interpretation readiness as authority to execute a consequential intervention unless the submitted package independently establishes the applicable authority and governed execution chain.'];
 if(objectStatus!=='ESTABLISHED')prohibitedInferences.push('Do not attach the submitted evidence to a broader, different, or insufficiently defined environmental object. A valid evidence chain cannot cure an incorrect object boundary.'); if(gap||rawAbsent)prohibitedInferences.push('Do not make an unrestricted whole-period claim when native sampling continuity or aggregation behavior is not established.'); if(thresholdAbsent)prohibitedInferences.push('Do not treat the record’s ACCEPTABLE classification as independently supported when its threshold source is undisclosed.'); if(conflict)prohibitedInferences.push('Do not select one conflicting source as authoritative without a declared resolution rule or bounded rationale.'); if(interventionStatus!=='ESTABLISHED'&&chainCount>0)prohibitedInferences.push('Do not infer a complete governed intervention chain from isolated or incomplete governance events.');
