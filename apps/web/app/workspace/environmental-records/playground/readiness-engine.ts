...
export function examineEnvironmentalReadiness(input:{text:string;inspectionObject:string;proposition:string;recordClass:string}):ReadinessReport{
 ...
 const weights:Record<string,number>={source_identity:1,time_boundary:1,location:1,calibration:1.5,continuity:1.5,provenance:1.5,threshold_source:1,object_localization:1,proposition:1,authority:1.5,conflict:1.5};
 ...
 const criticalWeak=['calibration','continuity','provenance','threshold_source','authority'].filter(id=>{const s=findings.find(f=>f.id===id)?.status;return s==='MISSING'||s==='PARTIAL'}).length;
 ...
 return {engine:'TA14-EGRI',engineVersion:'1.1.0',state,score,inspectionObject:input.inspectionObject.trim(),proposition:input.proposition.trim(),supportedNow,prohibitedInferences,missingBeforeStrongerReliance,nextAdmissibleSteps,findings,eriEligible,generatedAt:new Date().toISOString()};
}
