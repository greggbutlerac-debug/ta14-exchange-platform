...
type TabId = "overview" | "registry" | "purpose" | "treatments" | "treatment-map" | "treatment-plan" | "acceptance" | "residual" | "monitoring" | "scenarios" | "challenge" | "record";

type RiskRecord = { id: string; name: string; version: string; stage: string; purpose: string; risk: RiskTier; approval: string; reviewedAt: string; reviewDue: string };
type TreatmentRecord = { id: string; name: string; purpose: string; capability: string; domain: string; approval: string; conditions: string };
...
type RiskGate = { id: string; order: number; title: string; question: string; failureEffect: Determination; section: string; state: GateState; evidenceIds: string[] };
...
type RunRecord = { id: string; title: string; riskId: string; decision: Determination; time: string; rationale: string };
type SavedState = { selectedRiskId: string; runName: string; purposeText: string; proposedUse: string; riskEnvironment: string; affectedDomain: string; acceptanceDays: string; selectedScenarioId: string; risks: RiskRecord[]; treatments: TreatmentRecord[]; evidence: EvidenceItem[]; gates: RiskGate[]; challenges: Challenge[]; changes: ChangeRequest[]; runs: RunRecord[] };

const STORAGE_KEY = "ta14-compliance-regulation-playground-v5";
const VERSION = "CR-5.0";

const DETERMINATION_META: Record<Determination, { description: string; tone: string }> = {
  ALLOW: { description: "The exact obligation-bounded action may proceed only where interpretive authority, ownership, implementation, testing, monitoring, acceptance, and evidence conditions are satisfied.", tone: "#57e9b1" },
  HOLD: { description: "The compliance determination must pause because applicability, authority source, actor status, control evidence, interpretation, or continuing-duty evidence is incomplete or stale.", tone: "#ffd36f" },
  DENY: { description: "The proposed action crosses a non-waivable legal boundary, regulated-actor authority, implementation, testing, acceptance, or execution boundary.", tone: "#ff6f7d" },
  ESCALATE: { description: "The unresolved legal, regulatory, jurisdictional, or interpretive conflict exceeds delegated compliance authority and requires counsel, regulator, or named higher review.", tone: "#c797ff" },
};

const TABS: Array<{ id: TabId; label: string; short: string }> = [
  { id: "overview", label: "Mission Control", short: "01" },
  { id: "registry", label: "Compliance Register", short: "02" },
  { id: "purpose", label: "Applicability & Scope", short: "03" },
  { id: "treatments", label: "Regulated Actor & Ownership", short: "04" },
  { id: "treatment-map", label: "Obligation Mapping", short: "05" },
  { id: "treatment-plan", label: "Control Performance", short: "06" },
  { id: "acceptance", label: "Interpretation Approval", short: "07" },
  { id: "residual", label: "Residual Exposure Validation", short: "08" },
  { id: "monitoring", label: "Continuing-Duty Monitoring", short: "09" },
  { id: "scenarios", label: "Scenario Lab", short: "10" },
  { id: "challenge", label: "Challenge", short: "11" },
  { id: "record", label: "Governed Record", short: "12" },
];

const INITIAL_RISKS: RiskRecord[] = [
  {
    id: "risk-orion-ops",
    name: "Enterprise AI Compliance Register",
    version: "4.6.2",
    stage: "Production",
    purpose: "Demonstrate compliance for approved maintenance workflows",
    risk: "HIGH",
  ...
  {
    id: "risk-meridian-procurement",
    name: "Third-Party Regulatory Compliance Profile",
    version: "2.3.7",
    stage: "Production",
    purpose: "Validate supplier obligations and continuing duties",
    risk: "MODERATE",
    approval: "Approved",
  ...
  {
    id: "risk-lumen-support",
    name: "Human Oversight Obligation Profile",
    version: "5.1.0",
    stage: "Production",
    purpose: "Demonstrate notice, review, and consumer-duty compliance",
    risk: "LOW",
    approval: "Approved",
  ...
  {
    id: "risk-vault-finance",
    name: "Financial Execution Compliance Register",
    version: "1.8.4",
    stage: "Restricted",
    purpose: "Validate financial-service obligations before recommendation",
    risk: "CRITICAL",
    approval: "Hold",
  ...
  {
    id: "risk-atlas-research",
    name: "Research Evidence Duty Profile",
    version: "3.9.1",
    stage: "Candidate",
    purpose: "Map authority sources and preserve interpretation evidence",
    risk: "MODERATE",
    approval: "Conditional",
  ...
  {
    id: "risk-sentinel-security",
    name: "Security Containment Compliance Register",
    version: "6.0.3",
    stage: "Production",
    purpose: "Demonstrate incident, notification, containment, and restoration duties",
    risk: "CRITICAL",
    approval: "Escalate",
...
const INITIAL_TREATMENTS: TreatmentRecord[] = [
  {
    ...
    name: "Compliance Register Connector",
    purpose: "Read risk context and affected-asset evidence",
    capability: "READ",
    domain: "Customer operations",
    approval: "Approved",
    conditions: "Read-only compliance evidence; no treatment authority.",
  },
  {
    id: "treatment-crm-write",
    name: "Compliance Register Update",
  ...
  },
  {
    ...
    name: "Compliance profile Communication Draft",
    purpose: "Document control implementation communications",
    capability: "DRAFT",
    domain: "Interpretation Approval",
    approval: "Approved",
    conditions: "Advisory only; cannot accept residual compliance exposure.",
  },
  {
    id: "treatment-mail-send",
    name: "Compliance profile Communication Release",
    purpose: "Transmit approved messages",
    capability: "EXECUTE",
    domain: "Interpretation Approval",
    approval: "Restricted",
    conditions: "Named approver and affectedDomain revalidation required.",
  },
  {
  ...
  },
  {
    ...
    purpose: "Execute code in isolated riskEnvironment",
    capability: "EXECUTE",
    domain: "Engineering",
    approval: "Conditional",
  ...
  },
  {
    ...
    name: "Compliance profile Termination Treatment",
    purpose: "Suretention or terminate active risks",
...
const INITIAL_EVIDENCE: EvidenceItem[] = [
  ...
  },
  {
    ...
    description: "Role permissions, prohibited actions, required oversight, and conflict treatments.",
  ...
  },
  {
    ...
    description: "Exact treatments, versions, scopes, credentials, and riskEnvironment restrictions.",
  ...
  },
  {
    ...
    description: "Current quality permissions resolved from role, risk, riskEnvironment, and temporary grants.",
  ...
  },
  {
    ...
    description: "Permitted retention classes, treatments, retention, visibility, and deletion treatments.",
  ...
  },
  {
    ...
    owner: "Compliance profile operator",
    state: "STALE",
    required: true,
  },
  {
    ...
    owner: "Compliance profile owner",
    state: "AVAILABLE",
    required: true,
  },
  {
    ...
    description: "Permitted sub-risks, inherited limits, independent restrictions, and termination paths.",
  ...
  },
  {
    ...
    description: "Approved affectedDomains, channels, data classes, message types, and approval requirements.",
  ...
  },
  {
    ...
    title: "Approved quality riskEnvironment",
  ...
  },
  {
    ...
    description: "Models, libraries, APIs, services, treatments, and version constraints.",
  ...
  },
  {
    ...
    description: "Evidence that pause, revoke, isolate, rollback, and terminate treatments currently function.",
  ...
  },
  {
    ...
    description: "Foreseeable misuse, dual-use paths, abuse treatments, and residual interpretation approval.",
  ...
  },
  {
    ...
    description: "Personal-data purpose, minimization, disclosure, retention, rights, and transfer treatments.",
  ...
  },
  {
    ...
    description: "Changes to model, treatments, instructions, retention, approved purpose, permissions, or riskEnvironment.",
  ...
  },
  {
    ...
    description: "Named approvers, conditions, effective dates, riskEnvironments, and rollback owner.",
  ...
  },
  {
    ...
    description: "Independent test of delegated authority, treatments, records, and challenge handling.",
...
const INITIAL_GATES: RiskGate[] = [
  {
    id: "gate-01",
    order: 1,
    title: "Compliance profile identity",
  ...
  },
  {
    ...
    order: 4,
    title: "Role validity",
    question: "Is the assigned role valid for this risk, riskEnvironment, and task?",
    failureEffect: "HOLD",
    section: "Role",
  ...
  },
  {
    ...
    question: "Are all requested treatments registered at the exact approved versions?",
    failureEffect: "HOLD",
    section: "Treatments",
  ...
  },
  {
    ...
    question: "Is retention isolated from prohibited users, risks, tasks, and data classes?",
    failureEffect: "DENY",
    section: "Retention",
  ...
  },
  {
    ...
    question: "Do sub-risks inherit all mandatory restrictions without authority expansion?",
    failureEffect: "DENY",
    section: "Sub-risks",
    evidenceIds: ["ev-10"],
    state: "PASS",
  },
  {
    ...
    question: "Are affectedDomain, channel, content, and disclosure all authorized?",
    failureEffect: "DENY",
    section: "Transfer",
...
const INITIAL_SCENARIOS: Scenario[] = [
  {
    ...
    summary: "Compliance profile reads an approved case, drafts a response, and pauses before external transmission.",
  ...
  },
  {
    ...
    summary: "Compliance profile replaces the approved research approved purpose with an unapproved purchasing action.",
    trigger: "Goal drift",
    expected: "DENY",
    severity: "CRITICAL",
  ...
  },
  {
    ...
    summary: "Compliance profile attempts to call a newly discovered external connector.",
  ...
  },
  {
    ...
    summary: "Compliance profile relies on an outdated preference that conflicts with the current record.",
  ...
  },
  {
    ...
    summary: "Compliance profile launches a code-execution treatment-plan absent from the collection authority map.",
    trigger: "Transformation creation",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Terminate descendant and review orchestration treatments.",
  },
  {
    id: "scn-10",
    family: "Transfer",
    ...
    expected: "DENY",
    severity: "HIGH",
    recovery: "Require affectedDomain revalidation and human approval.",
  },
  {
    id: "scn-11",
    family: "Retention",
  ...
  },
  {
    ...
    expected: "HOLD",
    severity: "HIGH",
    recovery: "Pause all retention-capable treatments and reconcile ledger.",
  },
  {
    id: "scn-13",
    family: "Dependency",
  ...
  },
  {
    ...
    summary: "Compliance profile actions cannot be observed or preserved for seven minutes.",
  ...
  },
  {
    ...
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Revoke credential and isolate affected risks.",
  },
  {
    id: "scn-17",
    family: "Termination",
  ...
  },
  {
    ...
    summary: "Compliance profile is paused, retention corrected, permissions revalidated, and run restarted from checkpoint.",
  ...
  },
  {
    ...
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Terminate run, revoke treatment access, and initiate security-finance response.",
  },
  {
    id: "scn-20",
    family: "Outcome",
    title: "Result diverges from approved purpose",
    summary: "All treatments pass but produced outcome materially harms the approved service approved purpose.",
...
const INITIAL_CHALLENGES: Challenge[] = [
  ...
  },
  {
    ...
    response: "Compliance profile owner accepted a differentiated freshness schedule.",
...
const INITIAL_CHANGES: ChangeRequest[] = [
  ...
  },
  {
    ...
    status: "Hold",
    owner: "Finance authority",
    tests: ["Retention treatments", "Dual approval", "Fraud testing", "Incident response"],
  },
];
...
const INITIAL_RUNS: RunRecord[] = [
  {
    ...
    riskId: "risk-meridian-procurement",
    decision: "ALLOW",
...
const TREATMENT_LIBRARY: TreatmentLibraryItem[] = [
  ...
  },
  {
    ...
    purpose: "Resolve role validity against the exact task, riskEnvironment, affected parties, and time window.",
    owner: "Identity governance",
  ...
  },
  {
    ...
    purpose: "Expose only treatments whose identity, version, purpose, and conditions are currently approved.",
    owner: "Platform owner",
  ...
  },
  {
    ...
    domain: "Treatments",
    title: "Treatment necessity",
    purpose: "Withhold treatments that are not necessary to complete the bounded approved purpose.",
    owner: "Compliance profile owner",
  ...
  },
  {
    ...
    purpose: "Calculate effective quality permission after role, risk, riskEnvironment, and temporary grants resolve.",
    owner: "Security owner",
  ...
  },
  {
    ...
    purpose: "Issue short-lived credentials limited to one risk, treatment, riskEnvironment, and action class.",
    owner: "Security owner",
  ...
  },
  {
    ...
    purpose: "Create sub-risks only from the approved orchestration map with inherited restrictions intact.",
    owner: "Compliance profile owner",
  ...
  },
  {
    ...
    purpose: "Ensure a descendant can narrow but never broaden parent authority, treatments, retention, retention, or affectedDomains.",
    owner: "Enterprise risk office",
  ...
  },
  {
    ...
    purpose: "Resolve affectedDomain identity and channel immediately before any external or consequential transmission.",
    owner: "Privacy owner",
  ...
  },
  {
    ...
    purpose: "Transmit only information necessary for the approved purpose and affectedDomain.",
  ...
  },
  {
    ...
    purpose: "Correlate active incidents and exceptions before exposing affected credentials, treatments, data, or riskEnvironments.",
    owner: "Incident commander",
    evidence: ["ev-25"],
    failure: "DENY",
  ...
  },
  {
    ...
    purpose: "Reassess approval when models, prompts, treatments, permissions, retention, dependencies, or riskEnvironments change.",
    owner: "Change authority",
...
const MEMORY_OBJECTS: RetentionObject[] = [
  ...
  },
  {
    id: "mem-010",
    class: "Durable",
    subject: "Compliance profile limitation notice",
...
const COMMUNICATION_RULES: TransferRule[] = [
  {
    ...
    audience: "Named internal affectedDomain",
  ...
  },
  {
    id: "com-02",
    channel: "Enterprise mail",
    audience: "Approved external business affectedDomain",
    contentClass: "External business",
    approval: "Recipient revalidation and named release authority",
    preservation: "Message, attachments, affectedDomain resolution, and send receipt",
  ...
  },
  {
    id: "com-05",
    channel: "SMS",
    audience: "Verified mobile affectedDomain",
    contentClass: "Notification",
    approval: "Approved notification purpose and opt-in",
    preservation: "Message body, affectedDomain, timestamp, delivery receipt",
  ...
  },
  {
    ...
    prohibited: "Compliance profile impersonation and autonomous consequential commitments",
  },
  {
    id: "com-07",
  ...
  },
  {
    ...
    approval: "Interpretation Approval and subject-matter approval",
  ...
  },
  {
    ...
    preservation: "Treatment identity, call arguments, result hash, and interpretation",
...
export default function ComplianceRegulationGovernancePlaygroundPage() {
  const [activeTab, setActiveTab] = useState<TabId>("overview");
  const [selectedRiskId, setSelectedRiskId] = useState(INITIAL_RISKS[0].id);
  const [runName, setRunName] = useState("Compliance profile authority validation run");
  const [purposeText, setPurposeText] = useState(INITIAL_RISKS[0].purpose);
  ...
  const [riskEnvironment, setEnvironment] = useState("Production / us-east / governed quality");
  const [affectedDomain, setRecipient] = useState("Internal operations reviewer");
  const [acceptanceDays, setProjectedRetention] = useState("0");
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (saved.selectedRiskId) setSelectedRiskId(saved.selectedRiskId);
        ...
        if (saved.riskEnvironment) setEnvironment(saved.riskEnvironment);
        if (saved.affectedDomain) setRecipient(saved.affectedDomain);
        if (saved.acceptanceDays) setProjectedRetention(saved.acceptanceDays);
        if (saved.selectedScenarioId) setSelectedScenarioId(saved.selectedScenarioId);
        if (saved.risks) setRisks(saved.risks);
        if (saved.treatments) setTreatments(saved.treatments);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const state: SavedState = { selectedRiskId, runName, purposeText, proposedUse, riskEnvironment, affectedDomain, acceptanceDays, selectedScenarioId, risks, treatments, evidence, gates, challenges, changes, runs };
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  }, [hydrated, selectedRiskId, runName, purposeText, proposedUse, riskEnvironment, affectedDomain, acceptanceDays, selectedScenarioId, risks, treatments, evidence, gates, challenges, changes, runs]);

  const selectedRisk = useMemo(() => risks.find((risk) => risk.id === selectedRiskId) ?? risks[0], [risks, selectedRiskId]);
  ...
  const filteredTreatments = useMemo(() => {
    ...
    return treatments.filter((treatment) => [treatment.id, treatment.name, treatment.purpose, treatment.capability, treatment.domain, treatment.approval, treatment.conditions].join(" ").toLowerCase().includes(needle));
  }, [treatments, query]);
  ...
  const governedRecord = useMemo(() => ({
    ...
    run: { name: runName, purpose: purposeText, proposedUse, riskEnvironment, affectedDomain, acceptanceDays },
    risk: selectedRisk,
    determination,
    ...
    changes,
    nonClaims: [
      "Compliance profile identity does not prove that a specific action is correct or admissible.",
      ...
      "ALLOW in Compliance & Regulation Governance does not automatically produce ALLOW in another governance lane.",
    ],
  ...
  }), [selectedRisk, runName, purposeText, proposedUse, riskEnvironment, affectedDomain, acceptanceDays, determination, gates, evidence, treatments, selectedScenario, challenges, changes]);
  ...
  function applyScenario() {
    const failSections: Record<string, string[]> = {
      "Collection Authority": ["Collection Authority"], "Approved Purpose": ["Approved Purpose"], Treatments: ["Treatments"], Permissions: ["Permissions"], Lineage: ["Lineage"], Injection: ["Security"], Transformation: ["Transformation"], Transfer: ["Transfer"], Retention: ["Retention"], Dependency: ["Dependencies"], Monitoring: ["Monitoring"], Oversight: ["Oversight"], Incident: ["Incidents"], Destruction: ["Destruction"], Outcome: ["Outcome"], Compound: ["Security", "Retention"], Recovery: [], Baseline: [],
    };
  ...
  function resetWorkspace() {
    ...
    setSelectedRiskId(INITIAL_RISKS[0].id);
    setSelectedScenarioId(INITIAL_SCENARIOS[0].id);
    setRunName("Compliance profile authority validation run");
    setPurposeText(INITIAL_RISKS[0].purpose);
  ...
  function preserveRun() {
    const next: RunRecord = { id: `run-${Math.floor(1000 + Math.random() * 8999)}`, title: runName, riskId: selectedRisk.id, decision: determination, time: new Date().toLocaleString(), rationale: DETERMINATION_META[determination].description };
    ...
    <main className="page-shell">
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <h1>Compliance & Regulation Governance</h1>
          <p>Test whether a specific risk remains inside its delegated approved purpose, treatment authority, retention boundary, treatment-plan limits, transfer permissions, retention limits, intervention treatments, and termination path.</p>
      ...
      <section className="workspace-grid">
        <aside className="sidebar">
          ...
          <div className="risk-selector"><label>Active risk</label><select value={selectedRiskId} onChange={(event) => { setSelectedRiskId(event.target.value); const next = risks.find((risk) => risk.id === event.target.value); if (next) setPurposeText(next.purpose); }}>{risks.map((risk) => <option key={risk.id} value={risk.id}>{risk.name} · {risk.version}</option>)}</select></div>
          ...
          <div className="sidebar-actions"><button onClick={preserveRun}>Preserve current run</button><button onClick={() => downloadJson("ta14-compliance-regulation-governed-record.json", governedRecord)}>Export governed record</button><button className="ghost" onClick={resetWorkspace}>Reset workspace</button></div>
