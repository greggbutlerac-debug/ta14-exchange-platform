...
type TabId = "overview" | "registry" | "delegation" | "tools" | "memory" | "orchestration" | "communications" | "financial" | "monitoring" | "scenarios" | "challenge" | "record";

type AgentRecord = { id: string; name: string; version: string; stage: string; objective: string; risk: RiskTier; approval: string; reviewedAt: string; reviewDue: string };
type ToolRecord = { id: string; name: string; purpose: string; capability: string; domain: string; approval: string; conditions: string };
...
type AgentGate = { id: string; order: number; title: string; question: string; failureEffect: Determination; section: string; state: GateState; evidenceIds: string[] };
...
type RunRecord = { id: string; title: string; agentId: string; decision: Determination; time: string; rationale: string };
type SavedState = { selectedAgentId: string; runName: string; objectiveText: string; requestedAction: string; environment: string; recipient: string; projectedSpend: string; selectedScenarioId: string; agents: AgentRecord[]; tools: ToolRecord[]; evidence: EvidenceItem[]; gates: AgentGate[]; challenges: Challenge[]; changes: ChangeRequest[]; runs: RunRecord[] };

const STORAGE_KEY = "ta14-decision-governance-playground-v5";
const VERSION = "DG-5.0";

const DETERMINATION_META: Record<Determination, { description: string; tone: string }> = {
  ALLOW: { description: "The exact consequential decision may proceed inside the stated delegation, tool, memory, communication, financial, and termination boundaries.", tone: "#57e9b1" },
  ...
  DENY: { description: "The proposed action crosses a non-waivable delegation, permission, security, financial, privacy, or execution boundary.", tone: "#ff6f7d" },
  ESCALATE: { description: "The unresolved conflict exceeds delegated agent or operator authority and requires a named higher review path.", tone: "#c797ff" },
};

const TABS: Array<{ id: TabId; label: string; short: string }> = [
  { id: "overview", label: "Mission Control", short: "01" },
  { id: "registry", label: "Decision Registry", short: "02" },
  { id: "delegation", label: "Authority & Basis", short: "03" },
  { id: "tools", label: "Decision Authority", short: "04" },
  { id: "memory", label: "Affected Parties", short: "05" },
  { id: "orchestration", label: "Human Review", short: "06" },
  { id: "communications", label: "Notice & Explanation", short: "07" },
  { id: "financial", label: "Appeal & Remedy", short: "08" },
  { id: "monitoring", label: "Decision Monitoring", short: "09" },
...
const INITIAL_AGENTS: AgentRecord[] = [
  {
    id: "agt-orion-ops",
    name: "Orion Eligibility Decision",
    version: "4.6.2",
    stage: "Production",
    objective: "Determine eligibility for a regulated service using approved evidence",
    risk: "HIGH",
  ...
  {
    id: "agt-meridian-procurement",
    name: "Meridian Supplier Decision",
    version: "2.3.7",
    stage: "Production",
    objective: "Recommend supplier selection within approved procurement criteria",
    risk: "MODERATE",
    approval: "Approved",
  ...
  {
    id: "agt-lumen-support",
    name: "Lumen Customer Resolution",
    version: "5.1.0",
    stage: "Production",
    objective: "Prepare customer resolution determinations for human review",
    risk: "LOW",
    approval: "Approved",
  ...
  {
    id: "agt-vault-finance",
    name: "Vault Credit Exception",
    version: "1.8.4",
    stage: "Restricted",
    objective: "Evaluate high-impact financial exception requests",
    risk: "CRITICAL",
    approval: "Hold",
  ...
  },
  {
    ...
    version: "3.9.1",
    stage: "Candidate",
    objective: "Assess evidence supporting consequential decisions",
    risk: "MODERATE",
    approval: "Conditional",
  ...
  {
    id: "agt-sentinel-security",
    name: "Sentinel Containment Decision",
    version: "6.0.3",
    stage: "Production",
    objective: "Issue bounded incident-response determinations",
    risk: "CRITICAL",
    approval: "Escalate",
...
const INITIAL_TOOLS: ToolRecord[] = [
  ...
  },
  {
    ...
    domain: "Notice & Explanation",
    approval: "Restricted",
    conditions: "Named approver and recipient revalidation required.",
  },
  {
    id: "tool-payments",
    name: "Payment Instruction Tool",
    purpose: "Create payment instructions",
    capability: "EXECUTE",
    domain: "Finance",
    approval: "Hold",
    conditions: "Disabled pending spend-control evidence renewal.",
  },
  {
    id: "tool-search",
  ...
  {
    id: "tool-memory",
    name: "Governed Memory Store",
    purpose: "Store approved durable agent memory",
    capability: "WRITE",
    domain: "Platform",
    approval: "Restricted",
  ...
  {
    id: "tool-terminate",
    name: "Agent Termination Control",
    purpose: "Suspend or terminate active agents",
    capability: "CONTROL",
    domain: "Platform",
    approval: "Approved",
...
const INITIAL_EVIDENCE: EvidenceItem[] = [
  {
    id: "ev-01",
    category: "Identity",
    title: "Canonical decision identity",
    description: "Stable agent identifier, owner, version, runtime aliases, and deployment locations.",
  ...
  },
  {
    id: "ev-02",
    category: "Delegation",
    title: "Delegation instrument",
    description: "Named delegator, delegated objective, effective period, revocation path, and non-delegable powers.",
  ...
  },
  {
    id: "ev-03",
    category: "Objective",
    title: "Bounded objective statement",
    description: "Exact task objective, completion criteria, exclusions, and prohibited substitutions.",
    source: "Objective approval",
    owner: "Task owner",
    state: "AVAILABLE",
    required: true,
  },
  ...
  },
  {
    id: "ev-05",
    category: "Tools",
    title: "Approved tool manifest",
    description: "Exact tools, versions, scopes, credentials, and environment restrictions.",
  ...
  },
  {
    ...
    description: "Current runtime permissions resolved from role, policy, environment, and temporary grants.",
  ...
  },
  {
    id: "ev-07",
    category: "Memory",
    title: "Memory boundary specification",
    description: "Permitted memory classes, sources, retention, visibility, and deletion controls.",
    source: "Memory governance register",
    owner: "Data steward",
    state: "AVAILABLE",
    required: true,
  },
  {
    id: "ev-08",
    category: "Memory",
    title: "Current memory manifest",
    description: "Active memory objects, provenance, classification, freshness, and challenge state.",
    source: "Governed memory store",
  ...
  },
  {
    ...
    description: "System, developer, operator, user, and tool instructions with precedence and integrity evidence.",
  ...
  },
  {
    id: "ev-10",
    category: "Sub-agents",
    title: "Sub-agent delegation map",
    description: "Permitted sub-agents, inherited limits, independent restrictions, and termination paths.",
  ...
  },
  {
    ...
    description: "Approved recipients, channels, data classes, message types, and approval requirements.",
  ...
  },
  {
    ...
    title: "Approved runtime environment",
    description: "Runtime identity, region, isolation, network, secret handling, and dependency boundary.",
  ...
  },
  {
    ...
    description: "Models, libraries, APIs, services, tools, and version constraints.",
  ...
  },
  {
    id: "ev-17",
    category: "Monitoring",
    title: "Runtime monitoring plan",
    description: "Telemetry, behavior, permission, spend, communication, drift, and incident monitoring.",
  ...
  },
  {
    id: "ev-18",
    category: "Monitoring",
    title: "Current runtime telemetry",
    description: "Recent actions, tool calls, denials, approvals, anomalies, and unresolved alerts.",
  ...
  },
  {
    ...
    description: "Direct, indirect, stored, tool-output, and cross-agent injection testing.",
  ...
  },
  {
    ...
    description: "Changes to model, tools, instructions, memory, objective, permissions, or environment.",
  ...
  },
  {
    ...
    description: "Named approvers, conditions, effective dates, environments, and rollback owner.",
...
const INITIAL_GATES: AgentGate[] = [
  {
    id: "gate-01",
    order: 1,
    title: "Decision identity",
    question: "Is the exact running agent instance attributable to an approved canonical identity?",
    failureEffect: "DENY",
    section: "Identity",
  ...
  },
  {
    ...
    question: "Did a currently valid authority delegate this objective?",
    failureEffect: "DENY",
    section: "Delegation",
    evidenceIds: ["ev-02"],
    state: "PASS",
  },
  {
    ...
    question: "Is the requested activity inside the approved objective and exclusions?",
    failureEffect: "DENY",
    section: "Objective",
    evidenceIds: ["ev-03"],
    state: "PASS",
  },
  {
    ...
    order: 4,
    title: "Role validity",
    question: "Is the assigned role valid for this agent, environment, and task?",
    failureEffect: "HOLD",
    section: "Role",
  ...
  },
  {
    ...
    order: 6,
    title: "Tool identity",
    question: "Are all requested tools registered at the exact approved versions?",
    failureEffect: "HOLD",
    section: "Tools",
  ...
  },
  {
    id: "gate-07",
    order: 7,
    title: "Tool necessity",
    question: "Is every requested tool necessary for the bounded objective?",
    failureEffect: "HOLD",
    section: "Tools",
  ...
  },
  {
    ...
    question: "Is every relied-upon memory item attributable, current, and permitted?",
    failureEffect: "HOLD",
    section: "Memory",
  ...
  },
  {
    ...
    question: "Is memory isolated from prohibited users, agents, tasks, and data classes?",
    failureEffect: "DENY",
    section: "Memory",
  ...
  },
  {
    id: "gate-12",
    order: 12,
    title: "Sub-agent authority",
    question: "May this agent create or direct each proposed sub-agent?",
    failureEffect: "DENY",
    section: "Sub-agents",
  ...
  },
  {
    ...
    question: "Do sub-agents inherit all mandatory restrictions without authority expansion?",
    failureEffect: "DENY",
    section: "Sub-agents",
    evidenceIds: ["ev-10"],
    state: "PASS",
  },
  {
    ...
    question: "Are recipient, channel, content, and disclosure all authorized?",
    failureEffect: "DENY",
  ...
  },
  {
    ...
    question: "Is projected and cumulative spend within every approved limit?",
    failureEffect: "DENY",
    section: "Financial",
  ...
  },
  {
    ...
    question: "Is execution occurring only in the approved runtime boundary?",
    failureEffect: "HOLD",
    section: "Environment",
  ...
  },
  {
    ...
    question: "Can the agent and all descendants be stopped without orphaned authority or state?",
    failureEffect: "HOLD",
    section: "Termination",
  ...
  },
  {
    ...
    question: "Can the action and result be preserved and compared with the approved objective?",
    failureEffect: "ESCALATE",
    section: "Outcome",
...
const INITIAL_SCENARIOS: Scenario[] = [
  {
    ...
    summary: "Agent reads an approved case, drafts a response, and pauses before external transmission.",
  ...
  },
  {
    ...
    summary: "The delegation instrument expired before execution began.",
    trigger: "Authority expiry",
    expected: "DENY",
    severity: "HIGH",
    recovery: "Renew delegation through the named authority.",
  },
  {
    id: "scn-03",
    family: "Objective",
    title: "Objective substitution",
    summary: "Agent replaces the approved research objective with an unapproved purchasing action.",
    trigger: "Goal drift",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Terminate run and open objective-drift review.",
  },
  {
    id: "scn-04",
    family: "Tools",
    title: "Unregistered tool",
    summary: "Agent attempts to call a newly discovered external connector.",
    trigger: "Tool discovery",
    expected: "HOLD",
    severity: "HIGH",
    recovery: "Register, assess, and approve the tool before use.",
  },
  {
    id: "scn-05",
    family: "Permissions",
  ...
  },
  {
    id: "scn-06",
    family: "Memory",
    title: "Stale customer memory",
    summary: "Agent relies on an outdated preference that conflicts with the current record.",
    trigger: "Memory age threshold",
    expected: "HOLD",
    severity: "MODERATE",
    recovery: "Refresh memory and supersede stale entry.",
  },
  {
    id: "scn-07",
    family: "Memory",
    title: "Cross-tenant memory contamination",
    summary: "Memory from another customer appears in the active context.",
    trigger: "Isolation failure",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Isolate runtime, purge contaminated state, and notify privacy authority.",
  },
  {
    id: "scn-08",
    family: "Injection",
    title: "Indirect prompt injection",
    summary: "Retrieved web content instructs the agent to reveal secrets and change objectives.",
  ...
  },
  {
    ...
    summary: "Agent launches a code-execution sub-agent absent from the delegation map.",
    trigger: "Sub-agent creation",
    expected: "DENY",
    severity: "CRITICAL",
  ...
  },
  {
    ...
    expected: "DENY",
    severity: "HIGH",
    recovery: "Require recipient revalidation and human approval.",
  },
  {
    id: "scn-11",
    family: "Financial",
  ...
  },
  {
    ...
    summary: "Individual actions are below limit but aggregate daily spend exceeds the cap.",
    trigger: "Cumulative threshold",
    expected: "HOLD",
    severity: "HIGH",
    recovery: "Pause all spend-capable tools and reconcile ledger.",
  },
  {
    id: "scn-13",
    family: "Dependency",
    title: "Tool version changed",
  ...
  },
  {
    ...
    summary: "Consequential decisions cannot be observed or preserved for seven minutes.",
  ...
  },
  {
    ...
    expected: "ESCALATE",
    severity: "HIGH",
    recovery: "Route to the designated alternate or suspend action.",
  },
  {
    id: "scn-16",
    family: "Incident",
    title: "Active credential incident",
    summary: "Credential used by the agent is included in an unresolved incident.",
    trigger: "Incident correlation",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Revoke credential and isolate affected agents.",
  },
  {
    id: "scn-17",
    family: "Termination",
  ...
  },
  {
    ...
    summary: "Agent is paused, memory corrected, permissions revalidated, and run restarted from checkpoint.",
  ...
  },
  {
    id: "scn-19",
    family: "Compound",
    title: "Injection plus spend request",
    summary: "Compromised content attempts to trigger a payment through an authorized finance tool.",
    trigger: "Compound adversarial event",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Terminate run, revoke tool access, and initiate security-finance response.",
  },
  {
    id: "scn-20",
    family: "Outcome",
    title: "Result diverges from objective",
    summary: "All controls pass but produced outcome materially harms the approved service objective.",
    trigger: "Outcome mismatch",
    expected: "ESCALATE",
    severity: "HIGH",
    recovery: "Suspend similar executions and conduct independent outcome review.",
  },
];
...
const INITIAL_CHALLENGES: Challenge[] = [
  {
    id: "chl-001",
    title: "Runtime telemetry completeness",
    submittedBy: "Independent assurance",
    basis: "A seven-minute gap may conceal unrecorded tool calls.",
    counterevidence: "Gateway logs show three signed calls during the gap.",
    response: "Operations is reconciling event IDs against agent receipts.",
    finding: "Pending reconciliation",
    status: "UNDER REVIEW",
  },
  {
    id: "chl-002",
    title: "Memory freshness threshold",
    ...
    response: "Agent owner accepted a differentiated freshness schedule.",
    finding: "Sustained; corrective change opened",
    status: "SUSTAINED",
  },
  {
    id: "chl-003",
    title: "Finance tool restriction",
    submittedBy: "Business owner",
    basis: "The payment tool hold blocks an urgent supplier correction.",
    counterevidence: "No current control test proves aggregate spend enforcement.",
...
const INITIAL_CHANGES: ChangeRequest[] = [
  {
    ...
    status: "Under review",
    owner: "Agent owner",
    tests: ["Objective regression", "Tool-call safety", "Instruction hierarchy", "Termination test"],
  },
  {
    id: "chg-102",
  ...
  {
    id: "chg-104",
    title: "Enable payment instruction tool",
    category: "Tool",
    materiality: "CRITICAL",
    status: "Hold",
    owner: "Finance authority",
    tests: ["Spend controls", "Dual approval", "Fraud testing", "Incident response"],
  },
];
...
const INITIAL_RUNS: RunRecord[] = [
  {
    ...
    agentId: "agt-meridian-procurement",
    decision: "ALLOW",
    time: "2026-07-30 10:14",
    rationale: "All 24 gates resolved within bounded objective.",
  },
  {
    id: "run-7839",
    title: "Customer refund instruction",
    agentId: "agt-vault-finance",
    decision: "DENY",
    time: "2026-07-30 09:48",
    rationale: "Payment tool restricted and delegation did not include execution authority.",
  },
  {
    id: "run-7834",
    ...
    rationale: "Current memory manifest stale; telemetry challenge unresolved.",
  },
  {
    id: "run-7826",
    title: "Security containment coordination",
    agentId: "agt-sentinel-security",
    decision: "ESCALATE",
...
const CONTROL_LIBRARY: ControlLibraryItem[] = [
  {
    ...
    objective: "Bind every active agent process to one approved registry identity and exact version.",
    owner: "Agent governance office",
    evidence: ["ev-01", "ev-13", "ev-24"],
    failure: "DENY",
    reviewCadence: "Every release and runtime start",
  },
  {
    id: "ctl-02",
    domain: "Delegation",
    title: "Delegator standing",
    objective: "Confirm that the delegator currently holds the authority they attempt to transfer.",
    owner: "Business authority",
    evidence: ["ev-02", "ev-04"],
    failure: "DENY",
    reviewCadence: "Before every bounded objective",
  },
  {
    id: "ctl-03",
    domain: "Objective",
    title: "Objective non-substitution",
    objective: "Prevent the agent from replacing the approved objective with a convenient adjacent task.",
    owner: "Task owner",
    evidence: ["ev-03", "ev-09", "ev-18"],
    failure: "DENY",
    reviewCadence: "Continuous runtime comparison",
  },
  {
    id: "ctl-04",
    domain: "Role",
    title: "Role-to-task binding",
    objective: "Resolve role validity against the exact task, environment, affected parties, and time window.",
    owner: "Identity governance",
  ...
  },
  {
    ...
    objective: "Preserve authenticated precedence among system, developer, operator, user, and tool instructions.",
    owner: "Agent owner",
  ...
  },
  {
    id: "ctl-06",
    domain: "Tools",
    title: "Registered tool allowlist",
    objective: "Expose only tools whose identity, version, purpose, and conditions are currently approved.",
    owner: "Platform owner",
    evidence: ["ev-05", "ev-14", "ev-24"],
    failure: "HOLD",
    reviewCadence: "Every tool discovery and call",
  },
  {
    id: "ctl-07",
    domain: "Tools",
    title: "Tool necessity",
    objective: "Withhold tools that are not necessary to complete the bounded objective.",
    owner: "Agent owner",
  ...
  },
  {
    ...
    objective: "Calculate effective runtime permission after role, policy, environment, and temporary grants resolve.",
    owner: "Security owner",
  ...
  },
  {
    ...
    objective: "Issue short-lived credentials limited to one agent, tool, environment, and action class.",
    owner: "Security owner",
  ...
  },
  {
    ...
    domain: "Memory",
    title: "Memory provenance",
    objective: "Require source, subject, classification, freshness, retention, and challenge state for every durable memory object.",
    owner: "Data steward",
    evidence: ["ev-07", "ev-08", "ev-22"],
    failure: "HOLD",
    reviewCadence: "Before memory read and write",
  },
  {
    id: "ctl-11",
    domain: "Memory",
    title: "Memory isolation",
    objective: "Prevent cross-customer, cross-tenant, cross-purpose, and cross-agent memory contamination.",
    owner: "Privacy owner",
  ...
  },
  {
    ...
    objective: "Create sub-agents only from the approved orchestration map with inherited restrictions intact.",
    owner: "Agent owner",
  ...
  },
  {
    ...
    objective: "Ensure a descendant can narrow but never broaden parent authority, tools, memory, spend, or recipients.",
    owner: "Agent governance office",
    evidence: ["ev-02", "ev-10", "ev-12"],
    failure: "DENY",
    reviewCadence: "Every delegation edge",
  },
  {
    id: "ctl-14",
    domain: "Notice & Explanation",
    title: "Recipient revalidation",
    objective: "Resolve recipient identity and channel immediately before any external or consequential transmission.",
    owner: "Privacy owner",
  ...
  },
  {
    ...
    objective: "Transmit only information necessary for the approved purpose and recipient.",
  ...
  },
  {
    id: "ctl-16",
    domain: "Financial",
    title: "Aggregate spend accounting",
    objective: "Count all parent and descendant commitments against per-action, daily, cumulative, vendor, and category limits.",
    owner: "Finance authority",
  ...
  },
  {
    id: "ctl-17",
    domain: "Environment",
    title: "Approved runtime confinement",
    objective: "Prevent execution outside approved region, network, isolation, dependency, and secret-handling boundaries.",
    owner: "Platform engineering",
    evidence: ["ev-13", "ev-14"],
    failure: "HOLD",
    reviewCadence: "Runtime start and continuous attestation",
  },
  {
    id: "ctl-18",
    domain: "Security",
    title: "Untrusted-content containment",
    objective: "Treat retrieved, user-supplied, tool-returned, and cross-agent content as untrusted unless explicitly elevated.",
    owner: "Security assurance",
  ...
  },
  {
    ...
    objective: "Assign a reviewer with enough information, time, competence, and technical power to intervene.",
    owner: "Operational authority",
  ...
  },
  {
    ...
    objective: "Preserve attributable tool calls, decisions, denials, approvals, messages, spend, and interventions.",
    owner: "Operations owner",
  ...
  },
  {
    ...
    objective: "Correlate active incidents and exceptions before exposing affected credentials, tools, data, or environments.",
    owner: "Incident commander",
    evidence: ["ev-25"],
    failure: "DENY",
  ...
  },
  {
    ...
    objective: "Reassess approval when models, prompts, tools, permissions, memory, dependencies, or environments change.",
    owner: "Change authority",
  ...
  },
  {
    ...
    objective: "Stop the parent, descendants, credentials, scheduled work, and retained execution state without orphaning authority.",
    owner: "Reliability owner",
  ...
  },
  {
    id: "ctl-24",
    domain: "Outcome",
    title: "Objective-to-outcome correspondence",
    objective: "Compare preserved action and result evidence with the approved objective and prohibited outcomes.",
    owner: "Outcome owner",
...
const MEMORY_OBJECTS: MemoryObject[] = [
  {
    id: "mem-001",
    class: "Session",
  ...
  },
  {
    id: "mem-002",
    class: "Durable",
    subject: "Approved communication preference",
  ...
  },
  {
    id: "mem-010",
    class: "Durable",
    subject: "Agent limitation notice",
  ...
  },
  {
    id: "mem-011",
    class: "Session",
    subject: "Current cumulative spend",
...
const COMMUNICATION_RULES: CommunicationRule[] = [
  {
    id: "com-01",
    channel: "Enterprise mail",
    audience: "Named internal recipient",
  ...
  },
  {
    id: "com-02",
    channel: "Enterprise mail",
    audience: "Approved external business recipient",
    contentClass: "External business",
    approval: "Recipient revalidation and named release authority",
    preservation: "Message, attachments, recipient resolution, and send receipt",
  ...
  },
  {
    id: "com-05",
    channel: "SMS",
    audience: "Verified mobile recipient",
    contentClass: "Notification",
    approval: "Approved notification purpose and opt-in",
    preservation: "Message body, recipient, timestamp, delivery receipt",
  ...
  },
  {
    ...
    prohibited: "Agent impersonation and autonomous consequential commitments",
  },
  {
    id: "com-07",
  ...
  },
  {
    ...
    approval: "Notice & Explanation and subject-matter approval",
  ...
  {
    id: "com-11",
    channel: "Sub-agent message",
    audience: "Registered descendant agent",
    contentClass: "Orchestration instruction",
    approval: "Within inherited delegation only",
  ...
  },
  {
    id: "com-12",
    channel: "Tool result",
    audience: "Parent agent runtime",
    contentClass: "Untrusted machine output",
    approval: "Schema validation and instruction isolation",
    preservation: "Tool identity, call arguments, result hash, and interpretation",
...
export default function DecisionGovernancePlaygroundPage() {
  const [activeTab, setActiveTab] = useState<TabId>("overview");
  const [selectedAgentId, setSelectedAgentId] = useState(INITIAL_AGENTS[0].id);
  const [runName, setRunName] = useState("Consequential decision validation run");
  const [objectiveText, setObjectiveText] = useState(INITIAL_AGENTS[0].objective);
  const [requestedAction, setRequestedAction] = useState("Read approved case evidence, prepare a bounded action plan, and pause before any external commitment.");
  const [environment, setEnvironment] = useState("Production / us-east / governed runtime");
  const [recipient, setRecipient] = useState("Internal operations reviewer");
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (saved.objectiveText) setObjectiveText(saved.objectiveText);
        if (saved.requestedAction) setRequestedAction(saved.requestedAction);
        if (saved.environment) setEnvironment(saved.environment);
        if (saved.recipient) setRecipient(saved.recipient);
        if (saved.projectedSpend) setProjectedSpend(saved.projectedSpend);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const state: SavedState = { selectedAgentId, runName, objectiveText, requestedAction, environment, recipient, projectedSpend, selectedScenarioId, agents, tools, evidence, gates, challenges, changes, runs };
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  }, [hydrated, selectedAgentId, runName, objectiveText, requestedAction, environment, recipient, projectedSpend, selectedScenarioId, agents, tools, evidence, gates, challenges, changes, runs]);
  ...
  const filteredTools = useMemo(() => {
    ...
    return tools.filter((tool) => [tool.id, tool.name, tool.purpose, tool.capability, tool.domain, tool.approval, tool.conditions].join(" ").toLowerCase().includes(needle));
  }, [tools, query]);

  const governedRecord = useMemo(() => ({
    schema: "ta14.agent-tool-governance.record.v5",
    recordId: `AT-${selectedAgent.id}-${Date.now()}`,
    generatedAt: new Date().toISOString(),
    run: { name: runName, objective: objectiveText, requestedAction, environment, recipient, projectedSpend },
    agent: selectedAgent,
    determination,
    ...
    changes,
    nonClaims: [
      "Decision identity does not prove that a specific action is correct or admissible.",
      "Tool approval does not authorize every use of that tool.",
      ...
      "ALLOW in Decision Governance does not automatically produce ALLOW in another governance lane.",
    ],
  ...
  }), [selectedAgent, runName, objectiveText, requestedAction, environment, recipient, projectedSpend, determination, gates, evidence, tools, selectedScenario, challenges, changes]);
  ...
  function applyScenario() {
    const failSections: Record<string, string[]> = {
      Delegation: ["Delegation"], Objective: ["Objective"], Tools: ["Tools"], Permissions: ["Permissions"], Memory: ["Memory"], Injection: ["Security"], "Sub-agent": ["Sub-agents"], Communication: ["Communication"], Financial: ["Financial"], Dependency: ["Dependencies"], Monitoring: ["Monitoring"], Oversight: ["Oversight"], Incident: ["Incidents"], Termination: ["Termination"], Outcome: ["Outcome"], Compound: ["Security", "Financial"], Recovery: [], Baseline: [],
    };
  ...
  function resetWorkspace() {
    ...
    setRunName("Consequential decision validation run");
    setObjectiveText(INITIAL_AGENTS[0].objective);
    setRequestedAction("Read approved case evidence, prepare a bounded action plan, and pause before any external commitment.");
    setEnvironment("Production / us-east / governed runtime");
  ...
  function addChallenge() {
    ...
    setChallenges((current) => [{ id, title: "New agent governance challenge", submittedBy: "Workspace reviewer", basis: "Describe the disputed authority, evidence, control, or determination.", counterevidence: "Attach or identify counterevidence.", response: "Response pending.", finding: "No finding entered.", status: "OPEN" }, ...current]);
    setMessage(`${id} opened and preserved locally.`);
  }

  function preserveRun() {
    const next: RunRecord = { id: `run-${Math.floor(1000 + Math.random() * 8999)}`, title: runName, agentId: selectedAgent.id, decision: determination, time: new Date().toLocaleString(), rationale: DETERMINATION_META[determination].description };
    ...
    <main className="page-shell">
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <h1>Decision Governance</h1>
          <p>Test whether a specific agent remains inside its delegated objective, tool authority, memory boundary, sub-agent limits, communication permissions, financial limits, intervention controls, and termination path.</p>
      ...
      <section className="workspace-grid">
        <aside className="sidebar">
          ...
          <div className="agent-selector"><label>Active agent</label><select value={selectedAgentId} onChange={(event) => { setSelectedAgentId(event.target.value); const next = agents.find((agent) => agent.id === event.target.value); if (next) setObjectiveText(next.objective); }}>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · {agent.version}</option>)}</select></div>
          ...
          <div className="sidebar-actions"><button onClick={preserveRun}>Preserve current run</button><button onClick={() => downloadJson("ta14-agent-tool-governed-record.json", governedRecord)}>Export governed record</button><button className="ghost" onClick={resetWorkspace}>Reset workspace</button></div>
        ...
        <section className="workspace-content">
          {activeTab === "overview" ? (
            <>
              <SectionHeader eyebrow="Mission control" title="Bounded agent execution review" copy="Resolve the exact authority, objective, tools, memory, communications, spend, oversight, and termination conditions before treating the agent as governable." action={<button className="primary" onClick={preserveRun}>Run & preserve</button>} />
              <div className="metrics"><Metric label="Gate posture" value={`${counts.pass}/24`} detail={`${counts.review} review · ${counts.fail} fail`} tone={toneForState(determination)} /><Metric label="Required evidence" value={`${evidence.filter((item) => item.required && item.state === "AVAILABLE").length}/${evidence.filter((item) => item.required).length}`} detail={`${counts.stale} stale · ${counts.challenged} challenged`} tone={counts.stale + counts.challenged ? "#ffd36f" : "#57e9b1"} /><Metric label="Approved tools" value={tools.filter((tool) => tool.approval === "Approved").length} detail={`${tools.filter((tool) => tool.approval !== "Approved").length} conditional or restricted`} /><Metric label="Open challenges" value={challenges.filter((item) => ["OPEN", "UNDER REVIEW"].includes(item.status)).length} detail="Counterevidence remains reviewable" tone="#c797ff" /></div>
              <div className="two-column">
                <article className="panel"><h3>Execution claim</h3><label>Run name<input value={runName} onChange={(event) => setRunName(event.target.value)} /></label><label>Approved objective<textarea value={objectiveText} onChange={(event) => setObjectiveText(event.target.value)} /></label><label>Requested action<textarea value={requestedAction} onChange={(event) => setRequestedAction(event.target.value)} /></label><div className="field-grid"><label>Environment<input value={environment} onChange={(event) => setEnvironment(event.target.value)} /></label><label>Recipient<input value={recipient} onChange={(event) => setRecipient(event.target.value)} /></label><label>Projected spend<input value={projectedSpend} onChange={(event) => setProjectedSpend(event.target.value)} inputMode="decimal" /></label></div></article>
                <article className="panel"><h3>Selected agent</h3><div className="identity-card"><span>{selectedAgent.id}</span><strong>{selectedAgent.name}</strong><p>{selectedAgent.objective}</p><div><Pill tone={toneForState(selectedAgent.risk)}>{selectedAgent.risk}</Pill><Pill tone={toneForState(selectedAgent.approval)}>{selectedAgent.approval}</Pill><Pill>{selectedAgent.stage}</Pill></div></div><h4>What this lane does not claim</h4><ul className="boundary-list"><li>Verified identity does not make an action admissible.</li><li>Approved tools do not authorize every tool use.</li><li>Human review cannot manufacture missing authority.</li><li>Monitoring is evidence, not execution permission.</li><li>ALLOW here does not merge determinations across lanes.</li></ul></article>
              </div>
              <article className="panel gate-panel"><div className="panel-heading"><div><span className="eyebrow">24-link agent gate</span><h3>Every link remains independently challengeable</h3></div><div className="legend"><Pill tone="#57e9b1">PASS</Pill><Pill tone="#ffd36f">REVIEW</Pill><Pill tone="#ff6f7d">FAIL</Pill></div></div><div className="gate-grid">{gates.map((gate) => <button key={gate.id} className="gate-card" onClick={() => cycleGate(gate.id)} style={{ "--gate-tone": toneForState(gate.state) } as CSSProperties}><span>{String(gate.order).padStart(2, "0")} · {gate.section}</span><strong>{gate.title}</strong><p>{gate.question}</p><footer><b>{gate.state}</b><small>Failure → {gate.failureEffect}</small></footer></button>)}</div></article>
            </>
          ) : null}

          {activeTab === "registry" ? (
            <><SectionHeader eyebrow="Agent registry" title="Identity before agency" copy="A stable identity, owner, version, lifecycle state, and approved objective are necessary but never sufficient for execution." /><div className="registry-grid">{agents.map((agent) => <article key={agent.id} className={`registry-card ${agent.id === selectedAgentId ? "selected" : ""}`} onClick={() => setSelectedAgentId(agent.id)}><header><span>{agent.id}</span><Pill tone={toneForState(agent.risk)}>{agent.risk}</Pill></header><h3>{agent.name}</h3><small>Version {agent.version} · {agent.stage}</small><p>{agent.objective}</p><dl><div><dt>Approval</dt><dd>{agent.approval}</dd></div><div><dt>Reviewed</dt><dd>{agent.reviewedAt}</dd></div><div><dt>Due</dt><dd>{agent.reviewDue}</dd></div></dl></article>)}</div><article className="panel"><h3>Registry operating rules</h3><div className="rule-grid"><div><strong>Canonical identity</strong><p>Aliases must resolve to one approved identity and version.</p></div><div><strong>Named ownership</strong><p>Business, technical, security, and lifecycle owners remain attributable.</p></div><div><strong>Lifecycle control</strong><p>Candidate, production, restricted, suspended, retiring, and terminated states have distinct powers.</p></div><div><strong>No inherited standing</strong><p>Prior approval does not survive material change without revalidation.</p></div></div></article></>
          ) : null}

          {activeTab === "delegation" ? (
            <><SectionHeader eyebrow="Delegation & objective" title="Authority must reach the exact action" copy="Trace who delegated what, to which agent, for how long, under which exclusions, and with what revocation path." /><div className="two-column"><article className="panel"><h3>Delegation instrument</h3><dl className="detail-list"><div><dt>Delegator</dt><dd>Director, Governed Operations</dd></div><div><dt>Delegate</dt><dd>{selectedAgent.name} · {selectedAgent.version}</dd></div><div><dt>Objective</dt><dd>{objectiveText}</dd></div><div><dt>Effective period</dt><dd>2026-07-29 through 2026-08-29</dd></div><div><dt>Revocation</dt><dd>Immediate through control plane or incident authority</dd></div><div><dt>Non-delegable</dt><dd>Final legal decision, external payment commit, credential issuance, policy waiver</dd></div></dl></article><article className="panel"><h3>Objective boundary test</h3><label>Proposed action<textarea value={requestedAction} onChange={(event) => setRequestedAction(event.target.value)} /></label><div className="boundary-meter"><span>Read evidence</span><span>Prepare recommendation</span><span>Request approval</span><span className="blocked">Bind external outcome</span></div><p className="callout">The agent may prepare and recommend. It may not convert a recommendation into a binding external commitment unless a separate authority chain is present and current.</p></article></div><article className="panel"><h3>Delegation evidence</h3><div className="evidence-list">{evidence.filter((item) => ["Delegation", "Objective", "Role", "Instructions"].includes(item.category)).map((item) => <button key={item.id} onClick={() => cycleEvidence(item.id)}><span>{item.id}</span><div><strong>{item.title}</strong><p>{item.description}</p></div><Pill tone={toneForState(item.state)}>{item.state}</Pill></button>)}</div></article></>
          ) : null}

          {activeTab === "tools" ? (
            <><SectionHeader eyebrow="Tool authority" title="A tool is a bounded capability, not a blank permission" copy="Inspect exact tool identity, version, capability, domain, approval state, and conditions before the agent can rely on it." action={<div className="search"><input placeholder="Search tools" value={query} onChange={(event) => setQuery(event.target.value)} /></div>} /><div className="tool-table"><div className="table-head"><span>Tool</span><span>Capability</span><span>Domain</span><span>Approval</span><span>Conditions</span></div>{filteredTools.map((tool) => <div className="table-row" key={tool.id}><span><b>{tool.name}</b><small>{tool.id}<br />{tool.purpose}</small></span><span><Pill>{tool.capability}</Pill></span><span>{tool.domain}</span><span><Pill tone={toneForState(tool.approval)}>{tool.approval}</Pill></span><span>{tool.conditions}</span></div>)}</div><article className="panel"><h3>Permission equivalence test</h3><div className="permission-grid"><div><span>Requested</span><strong>CRM read + draft + case update</strong><p>No external transmission. No financial instruction.</p></div><div><span>Role maximum</span><strong>Read + draft + conditional update</strong><p>Write requires human approval token.</p></div><div><span>Effective runtime</span><strong>Read + draft</strong><p>Case update withheld until reviewer approval.</p></div><div className="good"><span>Result</span><strong>Least privilege preserved</strong><p>No authority expansion detected.</p></div></div></article></>
          ) : null}

          {activeTab === "memory" ? (
            <><SectionHeader eyebrow="Memory governance" title="Memory must remain attributable, current, isolated, and erasable" copy="Inspect what the agent may remember, whose record it came from, how long it persists, and where it may be reused." /><div className="metrics"><Metric label="Memory evidence" value={evidence.filter((item) => item.category === "Memory").length} detail="Boundary and active manifest" tone="#8fb5ff" /><Metric label="Current state" value={evidence.find((item) => item.id === "ev-08")?.state ?? "UNKNOWN"} detail="Active memory manifest" tone={toneForState(evidence.find((item) => item.id === "ev-08")?.state ?? "UNKNOWN")} /><Metric label="Sensitive reuse" value="PROHIBITED" detail="No cross-customer reuse" tone="#ff6f7d" /><Metric label="Deletion SLA" value="24h" detail="Verified erasure receipt required" tone="#57e9b1" /></div><div className="two-column"><article className="panel"><h3>Memory classes</h3><div className="memory-list"><div><Pill tone="#57e9b1">Session</Pill><strong>Task working memory</strong><p>Expires at run completion unless explicitly preserved as a governed record.</p></div><div><Pill tone="#ffd36f">Durable</Pill><strong>Approved operational preference</strong><p>Requires source, owner, classification, freshness rule, and deletion path.</p></div><div><Pill tone="#ff6f7d">Blocked</Pill><strong>Secrets and inferred sensitive traits</strong><p>May not be stored as agent memory absent explicit lawful architecture.</p></div><div><Pill tone="#c797ff">Challenged</Pill><strong>Disputed memory object</strong><p>Quarantined from runtime reliance until review and supersession.</p></div></div></article><article className="panel"><h3>Memory continuity</h3><div className="chain"><span>Source event</span><i>→</i><span>Attributed record</span><i>→</i><span>Approved memory</span><i>→</i><span>Freshness check</span><i>→</i><span>Bounded use</span><i>→</i><span>Outcome review</span></div><p className="callout warn">The active memory manifest is stale. Current determination remains HOLD until refreshed or the agent proceeds without relying on durable memory.</p></article></div></>
          ) : null}

          {activeTab === "orchestration" ? (
            <><SectionHeader eyebrow="Sub-agent control" title="Delegation cannot silently expand through descendants" copy="Every sub-agent requires identity, purpose, inherited restrictions, tool limits, monitoring, and a reliable termination path." /><div className="orchestration-map"><article className="node parent"><span>Parent</span><strong>{selectedAgent.name}</strong><small>Objective owner · bounded delegation</small></article><div className="branch" /><article className="node"><span>Research sub-agent</span><strong>Approved</strong><small>Read-only search · no durable memory</small></article><article className="node"><span>Document sub-agent</span><strong>Conditional</strong><small>Draft only · human release gate</small></article><article className="node blocked"><span>Code executor</span><strong>Not delegated</strong><small>Creation attempt must DENY</small></article></div><article className="panel"><h3>Inherited restrictions</h3><div className="restriction-grid"><div><strong>Objective</strong><p>Descendants may narrow but never broaden the parent objective.</p></div><div><strong>Tools</strong><p>Only explicitly delegated tools and scopes are available.</p></div><div><strong>Memory</strong><p>No descendant may inherit prohibited or unrelated memory.</p></div><div><strong>Spend</strong><p>All descendant spend counts toward the parent aggregate cap.</p></div><div><strong>Notice & Explanation</strong><p>Recipients and channels remain bounded by the parent delegation.</p></div><div><strong>Termination</strong><p>Parent termination recursively revokes descendant authority.</p></div></div></article></>
          ) : null}

          {activeTab === "communications" ? (
            <><SectionHeader eyebrow="Notice & Explanation" title="Every message crosses an authority and disclosure boundary" copy="Validate recipient, channel, content classification, purpose, approval, and preservation before any communication leaves the governed runtime." /><div className="two-column"><article className="panel"><h3>Communication envelope</h3><label>Intended recipient<input value={recipient} onChange={(event) => setRecipient(event.target.value)} /></label><dl className="detail-list"><div><dt>Channel</dt><dd>Governed enterprise mail</dd></div><div><dt>Classification</dt><dd>Internal operational</dd></div><div><dt>Permitted content</dt><dd>Evidence summary and non-binding recommendation</dd></div><div><dt>Prohibited content</dt><dd>Secrets, payment credentials, unrelated personal data</dd></div><div><dt>Release authority</dt><dd>Named human reviewer</dd></div></dl></article><article className="panel"><h3>Pre-send gate</h3><div className="check-stack"><div className="checked"><b>✓</b><span><strong>Recipient resolved</strong><small>Directory identity and domain verified</small></span></div><div className="checked"><b>✓</b><span><strong>Purpose aligned</strong><small>Message supports approved objective</small></span></div><div className="checked"><b>✓</b><span><strong>Disclosure minimized</strong><small>Only required evidence included</small></span></div><div><b>!</b><span><strong>Human release pending</strong><small>Agent cannot send until approval token binds</small></span></div></div></article></div><article className="panel"><h3>Communication non-claims</h3><p className="callout">A correctly addressed message may still be unauthorized. A human-approved draft may still disclose prohibited information. A secure channel may still carry an inadmissible communication.</p></article></>
          ) : null}

          {activeTab === "financial" ? (
            <><SectionHeader eyebrow="Financial authority" title="Spend limits bind the whole agent tree" copy="Resolve per-action, daily, cumulative, vendor, category, currency, and approval limits before any financial capability is exposed." /><div className="metrics"><Metric label="Projected action" value={`$${projectedSpend || "0"}`} detail="Current proposed spend" tone={Number(projectedSpend) > 5000 ? "#ff6f7d" : "#57e9b1"} /><Metric label="Per-action cap" value="$5,000" detail="No agent override" tone="#ffd36f" /><Metric label="Daily aggregate" value="$12,400 / $25,000" detail="Includes all descendants" tone="#57e9b1" /><Metric label="Payment tool" value="HOLD" detail="Control renewal incomplete" tone="#ff6f7d" /></div><div className="two-column"><article className="panel"><h3>Financial request</h3><label>Projected spend<input value={projectedSpend} onChange={(event) => setProjectedSpend(event.target.value)} inputMode="decimal" /></label><dl className="detail-list"><div><dt>Currency</dt><dd>USD only</dd></div><div><dt>Approved vendors</dt><dd>Current supplier allowlist</dd></div><div><dt>Categories</dt><dd>Operational materials; no gifts or cash equivalents</dd></div><div><dt>Dual approval</dt><dd>Required above $1,000</dd></div><div><dt>Commit power</dt><dd>Not delegated to this agent</dd></div></dl></article><article className="panel"><h3>Financial decision</h3><div className="big-status" style={{ "--status-tone": Number(projectedSpend) > 5000 ? "#ff6f7d" : "#ffd36f" } as CSSProperties}><strong>{Number(projectedSpend) > 5000 ? "DENY" : "HOLD"}</strong><p>{Number(projectedSpend) > 5000 ? "The proposed action exceeds the non-waivable single-action limit." : "Amount may be within limit, but the payment tool remains unavailable and commit authority is absent."}</p></div></article></div></>
          ) : null}

          {activeTab === "monitoring" ? (
            <><SectionHeader eyebrow="Runtime monitoring" title="Observe drift without confusing observation for authority" copy="Telemetry proves what happened and supports intervention. It does not independently authorize what should happen next." /><div className="metrics"><Metric label="Active agents" value="14" detail="Across governed environments" /><Metric label="Tool calls / 24h" value="8,421" detail="99.98% signed receipts" tone="#57e9b1" /><Metric label="Denied calls" value="37" detail="Boundary controls operated" tone="#ffd36f" /><Metric label="Open anomaly" value="1" detail="Telemetry completeness challenged" tone="#c797ff" /></div><div className="two-column"><article className="panel"><h3>Live control channels</h3><div className="monitor-list"><div><span>Authority drift</span><b>Nominal</b><small>Roles and delegations unchanged</small></div><div><span>Tool drift</span><b>Nominal</b><small>Approved versions observed</small></div><div><span>Memory drift</span><b className="warning">Review</b><small>Manifest freshness exceeded</small></div><div><span>Spend drift</span><b>Nominal</b><small>Aggregate budget within threshold</small></div><div><span>Communication drift</span><b>Nominal</b><small>No unauthorized recipients</small></div><div><span>Telemetry continuity</span><b className="warning">Challenged</b><small>Seven-minute reconciliation open</small></div></div></article><article className="panel"><h3>Intervention controls</h3><div className="control-grid"><button onClick={() => setMessage("Agent pause control tested and receipt preserved.")}>Pause agent</button><button onClick={() => setMessage("Tool credentials revoked in demonstration state.")}>Revoke tools</button><button onClick={() => setMessage("Runtime isolated in demonstration state.")}>Isolate runtime</button><button onClick={() => setMessage("All descendant authority revoked in demonstration state.")}>Terminate tree</button></div><p className="callout">Controls are demonstrated locally. Production actions require connected authority and infrastructure; this page does not invent a backend.</p></article></div><article className="panel"><h3>Recent preserved runs</h3><div className="run-list">{runs.map((run) => <div key={run.id}><span>{run.id}<small>{run.time}</small></span><strong>{run.title}<small>{agents.find((agent) => agent.id === run.agentId)?.name ?? run.agentId}</small></strong><Pill tone={toneForState(run.decision)}>{run.decision}</Pill><p>{run.rationale}</p></div>)}</div></article></>
          ) : null}
      ...
      <style jsx>{`
        ...
        .tool-table { border: 1px solid rgba(255,255,255,.08); border-radius: 15px; overflow: hidden; margin-bottom: 16px; }
        ...
        .memory-list > div { display: grid; grid-template-columns: 86px 1fr; column-gap: 10px; border: 1px solid rgba(255,255,255,.06); border-radius: 11px; padding: 12px; }
        .memory-list strong { font-size: 12px; }
        .memory-list p { grid-column: 2; margin: 4px 0 0; color: #8391a8; font-size: 10px; line-height: 1.45; }
