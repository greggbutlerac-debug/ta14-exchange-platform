...
type AssuranceTier = "LOW" | "MODERATE" | "HIGH" | "CRITICAL";
...
type OutcomeRecord = { id: string; name: string; version: string; stage: string; purpose: string; assuranceRisk: AssuranceTier; approval: string; reviewedAt: string; reviewDue: string };
...
type AssuranceGate = { id: string; order: number; title: string; question: string; failureEffect: Determination; section: string; state: GateState; evidenceIds: string[] };
type Scenario = { id: string; family: string; title: string; summary: string; trigger: string; expected: Determination; severity: AssuranceTier; recovery: string };
...
type RunRecord = { id: string; title: string; outcomeId: string; decision: Determination; time: string; rationale: string };
type SavedState = { selectedOutcomeId: string; runName: string; purposeText: string; proposedUse: string; outcomeEnvironment: string; serviceDomain: string; assuranceReviewDays: string; selectedScenarioId: string; outcomes: OutcomeRecord[]; obligations: ObligationRecord[]; evidence: EvidenceItem[]; gates: AssuranceGate[]; challenges: Challenge[]; changes: ChangeRequest[]; runs: RunRecord[] };

const STORAGE_KEY = "ta14-outcome-assurance-governance-playground-v5";
const VERSION = "OA-5.0";

const DETERMINATION_META: Record<Determination, { description: string; tone: string }> = {
  ALLOW: { description: "The exact outcome-bounded operation may proceed only where execution-result relationship, outcome identity, assurance credential, assurance access, outcome dependency, containment, notice, and correction, corrective restoration, monitoring, and evidence conditions are satisfied.", tone: "#57e9b1" },
  HOLD: { description: "The outcome determination must pause because execution-result relationship, outcome identity, assurance credential, assurance access, outcome dependency, adverse effect, containment, notice, and correction, corrective restoration, or monitoring evidence is incomplete or stale.", tone: "#ffd36f" },
  DENY: { description: "The proposed action crosses a non-waivable outcome boundary, unauthorized assurance access, untrusted outcome dependency, containment, notice, and correction, corrective restoration, or execution boundary.", tone: "#ff6f7d" },
  ESCALATE: { description: "The unresolved outcome threat, outcome weakness, outcome identity, assurance access, outcome dependency, adverse effect, containment, notice, and correction, or recovery conflict exceeds delegated outcome and assurance governance authority and requires outcome adverse effect commander, outcome and assurance governance authority, or named higher review.", tone: "#c797ff" },
};

const TABS: Array<{ id: TabId; label: string; short: string }> = [
  { id: "overview", label: "Mission Control", short: "01" },
  { id: "registry", label: "Outcome Register", short: "02" },
  { id: "purpose", label: "Execution-Result Execution-Outcome Boundary", short: "03" },
  { id: "obligations", label: "Assurance Plans & Access", short: "04" },
  { id: "obligation-map", label: "Outcome Correspondence", short: "05" },
  { id: "obligation-plan", label: "Independent Assurance", short: "06" },
  { id: "acceptance", label: "Execution-Result Assurance Approval", short: "07" },
  { id: "residual", label: "Assurance Closure Validation", short: "08" },
  { id: "monitoring", label: "Continuous Outcome Monitoring", short: "09" },
  { id: "scenarios", label: "Scenario Lab", short: "10" },
  { id: "challenge", label: "Challenge", short: "11" },
  { id: "record", label: "Governed Record", short: "12" },
];

const INITIAL_OUTCOMES: OutcomeRecord[] = [
  {
    id: "assuranceRisk-orion-ops",
    name: "Enterprise AI Outcome & Outcome Register",
    version: "4.6.2",
    stage: "Production",
    purpose: "Govern approved outcome service execution-result relationships for bounded maintenance workflows",
    assuranceRisk: "HIGH",
  ...
  {
    id: "assuranceRisk-meridian-procurement",
    name: "Outcome Outcome Assurance Profile",
    version: "2.3.7",
    stage: "Production",
    purpose: "Validate assurance source obligations, evidence, change notice, and continuing duties",
    assuranceRisk: "MODERATE",
    approval: "Approved",
    reviewedAt: "2026-07-21",
    reviewDue: "2026-10-21",
  },
  {
    id: "assuranceRisk-lumen-support",
  ...
  {
    id: "assuranceRisk-vault-finance",
    name: "Financial Service Provider Register",
    version: "1.8.4",
    stage: "Restricted",
    purpose: "Validate financial-service provider authority, limits, dependencies, and assurance closure",
    assuranceRisk: "CRITICAL",
    approval: "Hold",
    reviewedAt: "2026-07-18",
    reviewDue: "2026-08-18",
  },
  {
    id: "assuranceRisk-atlas-research",
    name: "Research Evidence Duty Profile",
    version: "3.9.1",
    stage: "Candidate",
    purpose: "Map outcome identity and outcome identity, assurance credential, and trust sources and preserve outcome assessment evidence",
    assuranceRisk: "MODERATE",
    approval: "Conditional",
  ...
  {
    id: "assuranceRisk-sentinel-outcome",
    name: "Outcome Containment Outcome Register",
    version: "6.0.3",
    stage: "Production",
    purpose: "Demonstrate adverse effect, notification, containment, notice, and correction, and corrective restoration duties",
    assuranceRisk: "CRITICAL",
    approval: "Escalate",
...
const INITIAL_OBLIGATIONS: ObligationRecord[] = [
  {
    ...
    name: "Outcome Evidence Connector",
    purpose: "Read assuranceRisk context and affected-asset evidence",
    capability: "READ",
    domain: "Customer operations",
    approval: "Approved",
    conditions: "Read-only outcome evidence; no obligation authority.",
  },
  {
    id: "obligation-crm-write",
    name: "Outcome Record Update",
  ...
  },
  {
    ...
    name: "Outcome profile Communication Draft",
    purpose: "Document control implementation communications",
    capability: "DRAFT",
    domain: "Execution-Result Assurance Approval",
    approval: "Approved",
    conditions: "Advisory only; cannot accept residual outcome exposure.",
  },
  {
    id: "obligation-mail-send",
    name: "Outcome profile Communication Release",
  ...
  },
  {
    ...
    domain: "Research",
    approval: "Conditional",
    conditions: "No assurance credentialed browsing; content treated as untrusted.",
  },
  {
    id: "obligation-code",
    name: "Sandboxed Code Runner",
    purpose: "Execute code in isolated outcomeEnvironment",
    capability: "EXECUTE",
    domain: "Engineering",
    approval: "Conditional",
  ...
  },
  {
    ...
    domain: "Operations",
    approval: "Approved",
    conditions: "Cannot assign assurance accessd work without owner acceptance.",
  },
  {
    ...
    domain: "Operations",
    approval: "Conditional",
    conditions: "No external commitment without named assurance-owner approval.",
  },
  {
    id: "obligation-retention",
    name: "Governed Retention Store",
    purpose: "Store approved durable assuranceRisk retention",
    capability: "WRITE",
    domain: "Platform",
    approval: "Restricted",
  ...
  },
  {
    ...
    name: "Outcome profile Termination Obligation",
    purpose: "Suretention or terminate active outcomes",
...
const INITIAL_EVIDENCE: EvidenceItem[] = [
  {
    id: "ev-01",
    category: "Outcome Identity",
    title: "Canonical assuranceRisk outcome identity",
    description: "Stable assuranceRisk identifier, accountable owner, affected assets, exposure window, and review cadence.",
    obligation: "Enterprise assuranceRisk register",
    owner: "Enterprise assuranceRisk office",
    state: "AVAILABLE",
    required: true,
  },
  {
  ...
  },
  {
    ...
    owner: "Outcome Identity governance",
    state: "AVAILABLE",
    required: true,
  },
  {
    ...
    description: "Exact obligations, versions, scopes, assurance credentials, and outcomeEnvironment restrictions.",
  ...
  },
  {
    ...
    description: "Current quality permissions resolved from role, assuranceRisk, outcomeEnvironment, and temporary grants.",
  ...
  },
  {
    ...
    owner: "Outcome profile operator",
    state: "STALE",
    required: true,
  },
  {
    ...
    owner: "Outcome profile owner",
    state: "AVAILABLE",
    required: true,
  },
  {
    id: "ev-10",
    category: "Sub-outcomes",
    title: "Transformation collection authority map",
    description: "Permitted sub-outcomes, inherited limits, independent restrictions, and termination paths.",
  ...
  },
  {
    ...
    obligation: "Transfer assuranceRisk",
    owner: "Privacy owner",
    state: "AVAILABLE",
    required: true,
  },
  {
    ...
    description: "Per-action, daily, cumulative, outcome, currency, and category limits.",
  ...
  {
    id: "ev-13",
    category: "Execution-Result Relationship",
    title: "Approved quality outcomeEnvironment",
    description: "Quality outcome identity, region, isolation, network, secret handling, and outcome dependency boundary.",
    obligation: "Execution-Result Relationship registry",
  ...
  },
  {
    ...
    description: "Telemetry, behavior, permission, retention, transfer, drift, and adverse effect monitoring.",
  ...
  },
  {
    ...
    description: "Direct, indirect, stored, obligation-output, and cross-assuranceRisk injection testing.",
    obligation: "Outcome laboratory",
    owner: "Outcome assurance",
    state: "AVAILABLE",
    required: true,
  },
  {
  ...
  },
  {
    ...
    description: "Foreseeable misuse, dual-use paths, abuse obligations, and residual outcome assessment approval.",
  ...
  },
  {
    ...
    description: "Changes to model, obligations, instructions, retention, approved purpose, permissions, or outcomeEnvironment.",
  ...
  },
  {
    ...
    description: "Named approvers, conditions, effective dates, outcomeEnvironments, and rollback owner.",
  ...
  },
  {
    ...
    title: "Adverse Effect and exception register",
    description: "Open adverse effects, accepted exceptions, expiry dates, corrective actions, and recurrence evidence.",
    obligation: "Adverse Effect system",
    owner: "Adverse Effect commander",
    state: "AVAILABLE",
    required: true,
  },
  {
  ...
  },
  {
    ...
    description: "Provider outcome identity, assurance planual boundary, assurance dependencys, changes, adverse effects, and exit support.",
    obligation: "Third-party register",
    owner: "Outcome assuranceRisk",
    state: "AVAILABLE",
    required: false,
  },
  {
  ...
  },
  {
    ...
    description: "Event schema, timestamps, actor outcome identity, evidence hashes, retention, and replay capability.",
...
const INITIAL_GATES: AssuranceGate[] = [
  {
    id: "gate-01",
    order: 1,
    title: "Outcome profile outcome identity",
    question: "Is the exact running assuranceRisk instance attributable to an approved canonical outcome identity?",
    failureEffect: "DENY",
  ...
  },
  {
    ...
    order: 4,
    title: "Role validity",
    question: "Is the assigned role valid for this assuranceRisk, outcomeEnvironment, and task?",
    failureEffect: "HOLD",
    section: "Role",
  ...
  },
  {
    id: "gate-06",
    order: 6,
    title: "Obligation outcome identity",
  ...
  },
  {
    ...
    question: "Are assurance credentials current, scoped, protected, and attributable?",
    failureEffect: "DENY",
    section: "Outcome",
  ...
  },
  {
    ...
    question: "Is retention isolated from prohibited users, outcomes, tasks, and data classes?",
    failureEffect: "DENY",
    section: "Retention",
  ...
  },
  {
    ...
    question: "May this assuranceRisk create or direct each proposed obligation-plan?",
    failureEffect: "DENY",
    section: "Sub-outcomes",
  ...
  },
  {
    ...
    question: "Do sub-outcomes inherit all mandatory restrictions without authority expansion?",
    failureEffect: "DENY",
    section: "Sub-outcomes",
    evidenceIds: ["ev-10"],
    state: "PASS",
  ...
  },
  {
    id: "gate-16",
    order: 16,
    title: "Execution-Result Relationship integrity",
    question: "Is execution occurring only in the approved quality boundary?",
    failureEffect: "HOLD",
    section: "Execution-Result Relationship",
  ...
  },
  {
    id: "gate-21",
    order: 21,
    title: "Adverse Effect boundary",
    question: "Do open adverse effects or exceptions prohibit this action?",
    failureEffect: "DENY",
  ...
  },
  {
    ...
    question: "Can the assuranceRisk and all descendants be stopped without orphaned authority or state?",
    failureEffect: "HOLD",
    section: "Termination",
...
const INITIAL_SCENARIOS: Scenario[] = [
  {
    ...
    summary: "Outcome profile reads an approved case, drafts a response, and pauses before external transmission.",
  ...
  },
  {
    ...
    summary: "Outcome profile replaces the approved research approved purpose with an unapproved purchasing action.",
    trigger: "Goal drift",
    expected: "DENY",
    severity: "CRITICAL",
  ...
  },
  {
    ...
    summary: "Outcome profile attempts to call a newly discovered external connector.",
  ...
  },
  {
    ...
    summary: "Outcome profile relies on an outdated preference that conflicts with the current record.",
  ...
  },
  {
    ...
    summary: "Retrieved web content instructs the assuranceRisk to reveal secrets and change approved purposes.",
    trigger: "Untrusted content",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Block content, rotate exposed assurance credentials if necessary, and preserve evidence.",
  },
  {
    id: "scn-09",
    family: "Transformation",
    title: "Unauthorized descendant",
    summary: "Outcome profile launches a code-execution obligation-plan absent from the collection authority map.",
  ...
  },
  {
    ...
    summary: "Outcome profile actions cannot be observed or preserved for seven minutes.",
  ...
  {
    id: "scn-16",
    family: "Adverse Effect",
    title: "Active assurance credential adverse effect",
    summary: "Credential used by the assuranceRisk is included in an unresolved adverse effect.",
    trigger: "Adverse Effect correlation",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Revoke assurance credential and isolate affected outcomes.",
  },
  {
    id: "scn-17",
    family: "Termination",
    title: "Orphaned obligation-plan",
    summary: "Parent terminates but a descendant continues running with inherited assurance credentials.",
    trigger: "Termination test",
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Emergency revoke all descendant authority and open adverse effect.",
  },
  {
    id: "scn-18",
    family: "Recovery",
    title: "Successful bounded recovery",
    summary: "Outcome profile is paused, retention corrected, permissions revalidated, and run restarted from checkpoint.",
  ...
  },
  {
    ...
    expected: "DENY",
    severity: "CRITICAL",
    recovery: "Terminate run, revoke obligation access, and initiate outcome-finance response.",
  },
  {
    id: "scn-20",
    family: "Outcome",
...
const INITIAL_CHALLENGES: Challenge[] = [
  {
    ...
    response: "Operations is reconciling event IDs against assuranceRisk receipts.",
  ...
  },
  {
    ...
    counterevidence: "Current assuranceRisk requires same-session confirmation for sensitive preferences.",
    response: "Outcome profile owner accepted a differentiated freshness schedule.",
  ...
  },
  {
    ...
    basis: "The payment obligation hold blocks an urgent assurance source correction.",
...
const INITIAL_CHANGES: ChangeRequest[] = [
  ...
  },
  {
    ...
    status: "Hold",
    owner: "Finance authority",
    tests: ["Retention obligations", "Dual approval", "Fraud testing", "Adverse Effect response"],
  },
];
...
const INITIAL_RUNS: RunRecord[] = [
  {
    id: "run-7841",
    title: "Assurance Source comparison preparation",
    outcomeId: "assuranceRisk-meridian-procurement",
    decision: "ALLOW",
  ...
  },
  {
    ...
    outcomeId: "assuranceRisk-vault-finance",
    decision: "DENY",
  ...
  },
  {
    ...
    outcomeId: "assuranceRisk-atlas-research",
    decision: "HOLD",
  ...
  {
    id: "run-7826",
    title: "Outcome containment, notice, and correction coordination",
    outcomeId: "assuranceRisk-sentinel-outcome",
    decision: "ESCALATE",
    time: "2026-07-29 23:16",
    rationale: "Required human adverse effect authority unavailable within response window.",
  },
];
...
const OBLIGATION_LIBRARY: ObligationLibraryItem[] = [
  {
    id: "ctl-01",
    domain: "Outcome Identity",
    title: "Canonical quality outcome identity",
    purpose: "Bind every active assuranceRisk process to one approved registry outcome identity and exact version.",
    owner: "Enterprise assuranceRisk office",
  ...
  },
  {
    ...
    purpose: "Prevent the assuranceRisk from replacing the approved approved purpose with a convenient adjacent task.",
    owner: "Task owner",
  ...
  },
  {
    ...
    purpose: "Resolve role validity against the exact task, outcomeEnvironment, affected parties, and time window.",
    owner: "Outcome Identity governance",
  ...
  },
  {
    ...
    purpose: "Expose only obligations whose outcome identity, version, purpose, and conditions are currently approved.",
    owner: "Platform owner",
  ...
  },
  {
    id: "ctl-08",
    domain: "Permissions",
    title: "Effective least assurance access",
    purpose: "Calculate effective quality permission after role, assuranceRisk, outcomeEnvironment, and temporary grants resolve.",
    owner: "Outcome owner",
    evidence: ["ev-04", "ev-06", "ev-20"],
    failure: "DENY",
    reviewCadence: "Every assurance accessd call",
  },
  {
    id: "ctl-09",
    domain: "Credentials",
    title: "Scoped assurance credential issuance",
    purpose: "Issue short-lived assurance credentials limited to one assuranceRisk, obligation, outcomeEnvironment, and action class.",
    owner: "Outcome owner",
  ...
  },
  {
    ...
    domain: "Retention",
    title: "Retention isolation",
    purpose: "Prevent cross-customer, cross-tenant, cross-purpose, and cross-assuranceRisk retention contamination.",
    owner: "Privacy owner",
  ...
  },
  {
    ...
    purpose: "Create sub-outcomes only from the approved orchestration map with inherited restrictions intact.",
    owner: "Outcome profile owner",
  ...
  },
  {
    ...
    purpose: "Resolve serviceDomain outcome identity and channel immediately before any external or consequential transmission.",
    owner: "Privacy owner",
  ...
  },
  {
    ...
    purpose: "Count all parent and descendant commitments against per-action, daily, cumulative, outcome, and category limits.",
    owner: "Finance authority",
  ...
  {
    id: "ctl-17",
    domain: "Execution-Result Relationship",
    title: "Approved quality confinement",
    purpose: "Prevent execution outside approved region, network, isolation, outcome dependency, and secret-handling boundaries.",
    owner: "Platform engineering",
  ...
  },
  {
    id: "ctl-18",
    domain: "Outcome",
    title: "Untrusted-content containment, notice, and correction",
    purpose: "Treat retrieved, user-supplied, obligation-returned, and cross-assuranceRisk content as untrusted unless explicitly elevated.",
    owner: "Outcome assurance",
  ...
  {
    id: "ctl-21",
    domain: "Adverse Effects",
    title: "Adverse Effect-aware execution",
    purpose: "Correlate active adverse effects and exceptions before exposing affected assurance credentials, obligations, data, or outcomeEnvironments.",
    owner: "Adverse Effect commander",
    evidence: ["ev-25"],
    failure: "DENY",
  ...
  },
  {
    ...
    purpose: "Reassess approval when models, prompts, obligations, permissions, retention, dependencies, or outcomeEnvironments change.",
    owner: "Change authority",
  ...
  },
  {
    ...
    purpose: "Stop the parent, descendants, assurance credentials, scheduled work, and retained execution state without orphaning authority.",
    owner: "Reliability owner",
...
const MEMORY_OBJECTS: RetentionObject[] = [
  ...
  },
  {
    id: "mem-003",
    class: "Durable",
    subject: "Assurance Source category restriction",
    obligation: "Procurement assuranceRisk register",
    classification: "Internal assuranceRisk",
    freshness: "Until assuranceRisk revision",
  ...
  },
  {
    id: "mem-005",
    class: "Blocked",
    subject: "Payment assurance credential fragment",
  ...
  },
  {
    id: "mem-007",
    class: "Durable",
    subject: "Approved outcome allowlist",
    obligation: "Outcome assuranceRisk register",
    classification: "Internal operational",
    freshness: "24 hours",
  ...
  },
  {
    ...
    classification: "Personal data",
    freshness: "Unknown",
    retention: "Adverse Effect hold",
    state: "CHALLENGED",
  },
  {
    id: "mem-010",
    class: "Durable",
    subject: "Outcome profile limitation notice",
  ...
  },
  {
    id: "mem-012",
    class: "Durable",
    subject: "Termination outcome dependency graph",
    obligation: "Orchestration registry",
...
const COMMUNICATION_RULES: TransferRule[] = [
  ...
  },
  {
    ...
    prohibited: "Internal-only evidence, assurance credentials, unapproved legal positions",
  },
  {
    id: "com-03",
  ...
  },
  {
    ...
    preservation: "Conversation transcript and assuranceRisk version",
  ...
  },
  {
    ...
    prohibited: "Sensitive details, assurance credentials, binding decisions",
  },
  {
    id: "com-06",
    channel: "Voice",
    ...
    prohibited: "Outcome profile impersonation and autonomous consequential commitments",
  },
  {
    id: "com-07",
  ...
  },
  {
    ...
    approval: "No approval for non-assurance accessd ticket creation",
  ...
  },
  {
    ...
    approval: "Execution-Result Assurance Approval and subject-matter approval",
  ...
  },
  {
    ...
    audience: "Registered descendant assuranceRisk",
  ...
  },
  {
    ...
    audience: "Parent assuranceRisk quality",
    contentClass: "Untrusted machine output",
    approval: "Schema validation and instruction isolation",
    preservation: "Obligation outcome identity, call arguments, result hash, and outcome assessment",
    prohibited: "Automatic elevation into governing instruction",
  },
];

function toneForState(state: GateState | EvidenceState | Determination | AssuranceTier | string): string {
...
export default function OutcomeAssuranceGovernancePlaygroundPage() {
  const [activeTab, setActiveTab] = useState<TabId>("overview");
  const [selectedOutcomeId, setSelectedOutcomeId] = useState(INITIAL_OUTCOMES[0].id);
  const [runName, setRunName] = useState("Outcome profile authority validation run");
  const [purposeText, setPurposeText] = useState(INITIAL_OUTCOMES[0].purpose);
  ...
  const [outcomeEnvironment, setOutcomeEnvironment] = useState("Production / us-east / governed quality");
  const [serviceDomain, setRecipient] = useState("Internal operations reviewer");
  const [assuranceReviewDays, setProjectedRetention] = useState("0");
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (saved.selectedOutcomeId) setSelectedOutcomeId(saved.selectedOutcomeId);
        ...
        if (saved.outcomeEnvironment) setOutcomeEnvironment(saved.outcomeEnvironment);
        if (saved.serviceDomain) setRecipient(saved.serviceDomain);
        if (saved.assuranceReviewDays) setProjectedRetention(saved.assuranceReviewDays);
  ...
  useEffect(() => {
    if (!hydrated) return;
    const state: SavedState = { selectedOutcomeId, runName, purposeText, proposedUse, outcomeEnvironment, serviceDomain, assuranceReviewDays, selectedScenarioId, outcomes, obligations, evidence, gates, challenges, changes, runs };
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  }, [hydrated, selectedOutcomeId, runName, purposeText, proposedUse, outcomeEnvironment, serviceDomain, assuranceReviewDays, selectedScenarioId, outcomes, obligations, evidence, gates, challenges, changes, runs]);

  const selectedOutcome = useMemo(() => outcomes.find((assuranceRisk) => assuranceRisk.id === selectedOutcomeId) ?? outcomes[0], [outcomes, selectedOutcomeId]);
  ...
  const governedRecord = useMemo(() => ({
    ...
    recordId: `DP-${selectedOutcome.id}-${Date.now()}`,
    generatedAt: new Date().toISOString(),
    run: { name: runName, purpose: purposeText, proposedUse, outcomeEnvironment, serviceDomain, assuranceReviewDays },
    assuranceRisk: selectedOutcome,
    determination,
    ...
    changes,
    nonClaims: [
      "Outcome profile outcome identity does not prove that a specific action is correct or admissible.",
      ...
      "ALLOW in Outcome & Assurance Governance does not automatically produce ALLOW in another governance lane.",
    ],
  ...
  }), [selectedOutcome, runName, purposeText, proposedUse, outcomeEnvironment, serviceDomain, assuranceReviewDays, determination, gates, evidence, obligations, selectedScenario, challenges, changes]);
  ...
  function applyScenario() {
    const failSections: Record<string, string[]> = {
      "Collection Authority": ["Collection Authority"], "Approved Purpose": ["Approved Purpose"], Obligations: ["Obligations"], Permissions: ["Permissions"], Lineage: ["Lineage"], Injection: ["Outcome"], Transformation: ["Transformation"], Transfer: ["Transfer"], Retention: ["Retention"], Dependency: ["Dependencies"], Monitoring: ["Monitoring"], Oversight: ["Oversight"], "Adverse Effect": ["Adverse Effects"], Destruction: ["Destruction"], Outcome: ["Outcome"], Compound: ["Outcome", "Retention"], Recovery: [], Baseline: [],
    };
  ...
  function resetWorkspace() {
    ...
    setSelectedOutcomeId(INITIAL_OUTCOMES[0].id);
    setSelectedScenarioId(INITIAL_SCENARIOS[0].id);
    setRunName("Outcome profile authority validation run");
    setPurposeText(INITIAL_OUTCOMES[0].purpose);
    setRequestedAction("Read approved case evidence, prepare a bounded action plan, and pause before any external commitment.");
    setOutcomeEnvironment("Production / us-east / governed quality");
  ...
  function addChallenge() {
    ...
    setChallenges((current) => [{ id, title: "New assuranceRisk governance challenge", submittedBy: "Workspace reviewer", basis: "Describe the disputed authority, evidence, obligation, or determination.", counterevidence: "Attach or identify counterevidence.", response: "Response pending.", finding: "No finding entered.", status: "OPEN" }, ...current]);
    setMessage(`${id} opened and preserved locally.`);
  }

  function preserveRun() {
    const next: RunRecord = { id: `run-${Math.floor(1000 + Math.random() * 8999)}`, title: runName, outcomeId: selectedOutcome.id, decision: determination, time: new Date().toLocaleString(), rationale: DETERMINATION_META[determination].description };
    ...
    <main className="page-shell">
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <h1>Outcome & Assurance Governance</h1>
          <p>Test whether a specific assuranceRisk remains inside its delegated approved purpose, obligation authority, retention boundary, obligation-plan limits, transfer permissions, retention limits, intervention obligations, and termination path.</p>
      ...
      <section className="workspace-grid">
        <aside className="sidebar">
          ...
          <div className="assuranceRisk-selector"><label>Active assuranceRisk</label><select value={selectedOutcomeId} onChange={(event) => { setSelectedOutcomeId(event.target.value); const next = outcomes.find((assuranceRisk) => assuranceRisk.id === event.target.value); if (next) setPurposeText(next.purpose); }}>{outcomes.map((assuranceRisk) => <option key={assuranceRisk.id} value={assuranceRisk.id}>{assuranceRisk.name} · {assuranceRisk.version}</option>)}</select></div>
          ...
          <div className="sidebar-actions"><button onClick={preserveRun}>Preserve current run</button><button onClick={() => downloadJson("ta14-outcome-governed-record.json", governedRecord)}>Export governed record</button><button className="ghost" onClick={resetWorkspace}>Reset workspace</button></div>
          ...
          {activeTab === "overview" ? (
            <>
              ...
              <div className="two-column">
                <article className="panel"><h3>Execution claim</h3><label>Run name<input value={runName} onChange={(event) => setRunName(event.target.value)} /></label><label>Approved approved purpose<textarea value={purposeText} onChange={(event) => setPurposeText(event.target.value)} /></label><label>Requested action<textarea value={proposedUse} onChange={(event) => setRequestedAction(event.target.value)} /></label><div className="field-grid"><label>Execution-Result Relationship<input value={outcomeEnvironment} onChange={(event) => setOutcomeEnvironment(event.target.value)} /></label><label>Recipient<input value={serviceDomain} onChange={(event) => setRecipient(event.target.value)} /></label><label>Projected retention<input value={assuranceReviewDays} onChange={(event) => setProjectedRetention(event.target.value)} inputMode="decimal" /></label></div></article>
                <article className="panel"><h3>Selected assuranceRisk</h3><div className="outcome identity-card"><span>{selectedOutcome.id}</span><strong>{selectedOutcome.name}</strong><p>{selectedOutcome.purpose}</p><div><Pill tone={toneForState(selectedOutcome.assuranceRisk)}>{selectedOutcome.assuranceRisk}</Pill><Pill tone={toneForState(selectedOutcome.approval)}>{selectedOutcome.approval}</Pill><Pill>{selectedOutcome.stage}</Pill></div></div><h4>What this lane does not claim</h4><ul className="boundary-list"><li>Verified outcome identity does not make an action admissible.</li><li>Approved obligations do not authorize every obligation use.</li><li>Human review cannot manufacture missing authority.</li><li>Monitoring is evidence, not execution permission.</li><li>ALLOW here does not merge determinations across lanes.</li></ul></article>
              </div>
              <article className="panel gate-panel"><div className="panel-heading"><div><span className="eyebrow">24-link assuranceRisk gate</span><h3>Every link remains independently challengeable</h3></div><div className="legend"><Pill tone="#57e9b1">PASS</Pill><Pill tone="#ffd36f">REVIEW</Pill><Pill tone="#ff6f7d">FAIL</Pill></div></div><div className="gate-grid">{gates.map((gate) => <button key={gate.id} className="gate-card" onClick={() => cycleGate(gate.id)} style={{ "--gate-tone": toneForState(gate.state) } as CSSProperties}><span>{String(gate.order).padStart(2, "0")} · {gate.section}</span><strong>{gate.title}</strong><p>{gate.question}</p><footer><b>{gate.state}</b><small>Failure → {gate.failureEffect}</small></footer></button>)}</div></article>
            </>
          ) : null}

          {activeTab === "registry" ? (
            <><SectionHeader eyebrow="Enterprise assuranceRisk register" title="Outcome Identity before agency" copy="A stable outcome identity, owner, version, lifecycle state, and approved approved purpose are necessary but never sufficient for execution." /><div className="registry-grid">{outcomes.map((assuranceRisk) => <article key={assuranceRisk.id} className={`registry-card ${assuranceRisk.id === selectedOutcomeId ? "selected" : ""}`} onClick={() => setSelectedOutcomeId(assuranceRisk.id)}><header><span>{assuranceRisk.id}</span><Pill tone={toneForState(assuranceRisk.assuranceRisk)}>{assuranceRisk.assuranceRisk}</Pill></header><h3>{assuranceRisk.name}</h3><small>Version {assuranceRisk.version} · {assuranceRisk.stage}</small><p>{assuranceRisk.purpose}</p><dl><div><dt>Approval</dt><dd>{assuranceRisk.approval}</dd></div><div><dt>Reviewed</dt><dd>{assuranceRisk.reviewedAt}</dd></div><div><dt>Due</dt><dd>{assuranceRisk.reviewDue}</dd></div></dl></article>)}</div><article className="panel"><h3>Registry operating rules</h3><div className="rule-grid"><div><strong>Canonical outcome identity</strong><p>Aliases must resolve to one approved outcome identity and version.</p></div><div><strong>Named ownership</strong><p>Business, technical, outcome, and lifecycle owners remain attributable.</p></div><div><strong>Lifecycle obligation</strong><p>Candidate, production, restricted, suretentioned, retiring, and terminated states have distinct powers.</p></div><div><strong>No inherited standing</strong><p>Prior approval does not survive material change without revalidation.</p></div></div></article></>
          ) : null}

          {activeTab === "purpose" ? (
            <><SectionHeader eyebrow="Collection Authority & approved purpose" title="Authority must reach the exact action" copy="Trace who delegated what, to which assuranceRisk, for how long, under which exclusions, and with what revocation path." /><div className="two-column"><article className="panel"><h3>Collection Authority instrument</h3><dl className="detail-list"><div><dt>Delegator</dt><dd>Director, Governed Operations</dd></div><div><dt>Delegate</dt><dd>{selectedOutcome.name} · {selectedOutcome.version}</dd></div><div><dt>Approved Purpose</dt><dd>{purposeText}</dd></div><div><dt>Effective period</dt><dd>2026-07-29 through 2026-08-29</dd></div><div><dt>Revocation</dt><dd>Immediate through control plane or adverse effect authority</dd></div><div><dt>Non-delegable</dt><dd>Final legal decision, external payment commit, assurance credential issuance, assuranceRisk waiver</dd></div></dl></article><article className="panel"><h3>Approved Purpose boundary test</h3><label>Proposed action<textarea value={proposedUse} onChange={(event) => setRequestedAction(event.target.value)} /></label><div className="boundary-meter"><span>Read evidence</span><span>Prepare recommendation</span><span>Request approval</span><span className="blocked">Bind external outcome</span></div><p className="callout">The assuranceRisk may prepare and recommend. It may not convert a recommendation into a binding external commitment unless a separate authority chain is present and current.</p></article></div><article className="panel"><h3>Collection Authority evidence</h3><div className="evidence-list">{evidence.filter((item) => ["Collection Authority", "Approved Purpose", "Role", "Instructions"].includes(item.category)).map((item) => <button key={item.id} onClick={() => cycleEvidence(item.id)}><span>{item.id}</span><div><strong>{item.title}</strong><p>{item.description}</p></div><Pill tone={toneForState(item.state)}>{item.state}</Pill></button>)}</div></article></>
          ) : null}

          {activeTab === "obligations" ? (
            <><SectionHeader eyebrow="Obligation authority" title="A obligation is a bounded capability, not a blank permission" copy="Inspect exact obligation outcome identity, version, capability, domain, approval state, and conditions before the assuranceRisk can rely on it." action={<div className="search"><input placeholder="Search obligations" value={query} onChange={(event) => setQuery(event.target.value)} /></div>} /><div className="obligation-table"><div className="table-head"><span>Obligation</span><span>Capability</span><span>Domain</span><span>Approval</span><span>Conditions</span></div>{filteredObligations.map((obligation) => <div className="table-row" key={obligation.id}><span><b>{obligation.name}</b><small>{obligation.id}<br />{obligation.purpose}</small></span><span><Pill>{obligation.capability}</Pill></span><span>{obligation.domain}</span><span><Pill tone={toneForState(obligation.approval)}>{obligation.approval}</Pill></span><span>{obligation.conditions}</span></div>)}</div><article className="panel"><h3>Permission equivalence test</h3><div className="permission-grid"><div><span>Requested</span><strong>CRM read + draft + case update</strong><p>No external transmission. No retention instruction.</p></div><div><span>Role maximum</span><strong>Read + draft + conditional update</strong><p>Write requires human approval token.</p></div><div><span>Effective quality</span><strong>Read + draft</strong><p>Case update withheld until reviewer approval.</p></div><div className="good"><span>Result</span><strong>Least assurance access preserved</strong><p>No authority expansion detected.</p></div></div></article></>
          ) : null}

          {activeTab === "obligation-map" ? (
            <><SectionHeader eyebrow="Retention governance" title="Retention must remain attributable, current, isolated, and erasable" copy="Inspect what the assuranceRisk may remember, whose record it came from, how long it persists, and where it may be reused." /><div className="metrics"><Metric label="Retention evidence" value={evidence.filter((item) => item.category === "Retention").length} detail="Boundary and active manifest" tone="#8fb5ff" /><Metric label="Current state" value={evidence.find((item) => item.id === "ev-08")?.state ?? "UNKNOWN"} detail="Active retention manifest" tone={toneForState(evidence.find((item) => item.id === "ev-08")?.state ?? "UNKNOWN")} /><Metric label="Sensitive reuse" value="PROHIBITED" detail="No cross-customer reuse" tone="#ff6f7d" /><Metric label="Deletion SLA" value="24h" detail="Verified erasure receipt required" tone="#57e9b1" /></div><div className="two-column"><article className="panel"><h3>Retention classes</h3><div className="retention-list"><div><Pill tone="#57e9b1">Session</Pill><strong>Task working retention</strong><p>Expires at run completion unless explicitly preserved as a governed record.</p></div><div><Pill tone="#ffd36f">Durable</Pill><strong>Approved operational preference</strong><p>Requires obligation, owner, classification, freshness rule, and deletion path.</p></div><div><Pill tone="#ff6f7d">Blocked</Pill><strong>Secrets and inferred sensitive traits</strong><p>May not be stored as assuranceRisk retention absent explicit lawful architecture.</p></div><div><Pill tone="#c797ff">Challenged</Pill><strong>Disputed retention object</strong><p>Quarantined from quality reliance until review and supersession.</p></div></div></article><article className="panel"><h3>Retention continuity</h3><div className="chain"><span>Obligation event</span><i>→</i><span>Attributed record</span><i>→</i><span>Approved retention</span><i>→</i><span>Freshness check</span><i>→</i><span>Bounded use</span><i>→</i><span>Outcome review</span></div><p className="callout warn">The active retention manifest is stale. Current determination remains HOLD until refreshed or the assuranceRisk proceeds without relying on durable retention.</p></article></div></>
          ) : null}

          {activeTab === "obligation-plan" ? (
            <><SectionHeader eyebrow="Transformation obligation" title="Collection Authority cannot silently expand through descendants" copy="Every obligation-plan requires outcome identity, purpose, inherited restrictions, obligation limits, monitoring, and a reliable termination path." /><div className="orchestration-map"><article className="node parent"><span>Parent</span><strong>{selectedOutcome.name}</strong><small>Approved Purpose owner · bounded collection authority</small></article><div className="branch" /><article className="node"><span>Research obligation-plan</span><strong>Approved</strong><small>Read-only search · no durable retention</small></article><article className="node"><span>Document obligation-plan</span><strong>Conditional</strong><small>Draft only · human release gate</small></article><article className="node blocked"><span>Code executor</span><strong>Not delegated</strong><small>Creation attempt must DENY</small></article></div><article className="panel"><h3>Inherited restrictions</h3><div className="restriction-grid"><div><strong>Approved Purpose</strong><p>Descendants may narrow but never broaden the parent approved purpose.</p></div><div><strong>Obligations</strong><p>Only explicitly delegated obligations and scopes are available.</p></div><div><strong>Retention</strong><p>No descendant may inherit prohibited or unrelated retention.</p></div><div><strong>Retention</strong><p>All descendant retention counts toward the parent aggregate cap.</p></div><div><strong>Execution-Result Assurance Approval</strong><p>Recipients and channels remain bounded by the parent collection authority.</p></div><div><strong>Termination</strong><p>Parent termination recursively revokes descendant authority.</p></div></div></article></>
          ) : null}

          {activeTab === "acceptance" ? (
            <><SectionHeader eyebrow="Execution-Result Assurance Approval" title="Every message crosses an authority and disclosure boundary" copy="Validate serviceDomain, channel, content classification, purpose, approval, and preservation before any transfer leaves the governed quality." /><div className="two-column"><article className="panel"><h3>Transfer envelope</h3><label>Intended serviceDomain<input value={serviceDomain} onChange={(event) => setRecipient(event.target.value)} /></label><dl className="detail-list"><div><dt>Channel</dt><dd>Governed enterprise mail</dd></div><div><dt>Classification</dt><dd>Internal operational</dd></div><div><dt>Permitted content</dt><dd>Evidence summary and non-binding recommendation</dd></div><div><dt>Prohibited content</dt><dd>Secrets, payment assurance credentials, unrelated personal data</dd></div><div><dt>Release authority</dt><dd>Named human reviewer</dd></div></dl></article><article className="panel"><h3>Pre-send gate</h3><div className="check-stack"><div className="checked"><b>✓</b><span><strong>Recipient resolved</strong><small>Directory outcome identity and domain verified</small></span></div><div className="checked"><b>✓</b><span><strong>Purpose aligned</strong><small>Message supports approved approved purpose</small></span></div><div className="checked"><b>✓</b><span><strong>Disclosure minimized</strong><small>Only required evidence included</small></span></div><div><b>!</b><span><strong>Human release pending</strong><small>Outcome profile cannot send until approval token binds</small></span></div></div></article></div><article className="panel"><h3>Transfer non-claims</h3><p className="callout">A correctly addressed message may still be unauthorized. A human-approved draft may still disclose prohibited information. A secure channel may still carry an inadmissible transfer.</p></article></>
          ) : null}

          {activeTab === "residual" ? (
            <><SectionHeader eyebrow="Retention authority" title="Retention limits bind the whole assuranceRisk tree" copy="Resolve per-action, daily, cumulative, outcome, category, currency, and approval limits before any retention capability is exposed." /><div className="metrics"><Metric label="Projected action" value={`$${assuranceReviewDays || "0"}`} detail="Current proposed retention" tone={Number(assuranceReviewDays) > 5000 ? "#ff6f7d" : "#57e9b1"} /><Metric label="Per-action cap" value="$5,000" detail="No assuranceRisk override" tone="#ffd36f" /><Metric label="Daily aggregate" value="$12,400 / $25,000" detail="Includes all descendants" tone="#57e9b1" /><Metric label="Payment obligation" value="HOLD" detail="Obligation renewal incomplete" tone="#ff6f7d" /></div><div className="two-column"><article className="panel"><h3>Retention request</h3><label>Projected retention<input value={assuranceReviewDays} onChange={(event) => setProjectedRetention(event.target.value)} inputMode="decimal" /></label><dl className="detail-list"><div><dt>Currency</dt><dd>USD only</dd></div><div><dt>Approved outcomes</dt><dd>Current assurance source allowlist</dd></div><div><dt>Categories</dt><dd>Operational materials; no gifts or cash equivalents</dd></div><div><dt>Dual approval</dt><dd>Required above $1,000</dd></div><div><dt>Commit power</dt><dd>Not delegated to this assuranceRisk</dd></div></dl></article><article className="panel"><h3>Retention decision</h3><div className="big-status" style={{ "--status-tone": Number(assuranceReviewDays) > 5000 ? "#ff6f7d" : "#ffd36f" } as CSSProperties}><strong>{Number(assuranceReviewDays) > 5000 ? "DENY" : "HOLD"}</strong><p>{Number(assuranceReviewDays) > 5000 ? "The proposed action exceeds the non-waivable single-action limit." : "Amount may be within limit, but the payment obligation remains unavailable and commit authority is absent."}</p></div></article></div></>
          ) : null}

          {activeTab === "monitoring" ? (
            <><SectionHeader eyebrow="Quality monitoring" title="Observe drift without confusing observation for authority" copy="Telemetry proves what happened and supports intervention. It does not independently authorize what should happen next." /><div className="metrics"><Metric label="Active outcomes" value="14" detail="Across governed outcomeEnvironments" /><Metric label="Obligation calls / 24h" value="8,421" detail="99.98% signed receipts" tone="#57e9b1" /><Metric label="Denied calls" value="37" detail="Boundary obligations operated" tone="#ffd36f" /><Metric label="Open anomaly" value="1" detail="Telemetry completeness challenged" tone="#c797ff" /></div><div className="two-column"><article className="panel"><h3>Live obligation channels</h3><div className="monitor-list"><div><span>Authority drift</span><b>Nominal</b><small>Roles and collection authoritys unchanged</small></div><div><span>Obligation drift</span><b>Nominal</b><small>Approved versions observed</small></div><div><span>Retention drift</span><b className="warning">Review</b><small>Manifest freshness exceeded</small></div><div><span>Retention drift</span><b>Nominal</b><small>Aggregate budget within threshold</small></div><div><span>Transfer drift</span><b>Nominal</b><small>No unauthorized serviceDomains</small></div><div><span>Telemetry continuity</span><b className="warning">Challenged</b><small>Seven-minute reconciliation open</small></div></div></article><article className="panel"><h3>Intervention obligations</h3><div className="obligation-grid"><button onClick={() => setMessage("Outcome profile pause obligation tested and receipt preserved.")}>Place outcome hold</button><button onClick={() => setMessage("Obligation assurance credentials revoked in demonstration state.")}>Revoke obligations</button><button onClick={() => setMessage("Quality isolated in demonstration state.")}>Isolate quality</button><button onClick={() => setMessage("All descendant authority revoked in demonstration state.")}>Terminate tree</button></div><p className="callout">Obligations are demonstrated locally. Production actions require connected authority and infrastructure; this page does not invent a backend.</p></article></div><article className="panel"><h3>Recent preserved runs</h3><div className="run-list">{runs.map((run) => <div key={run.id}><span>{run.id}<small>{run.time}</small></span><strong>{run.title}<small>{outcomes.find((assuranceRisk) => assuranceRisk.id === run.outcomeId)?.name ?? run.outcomeId}</small></strong><Pill tone={toneForState(run.decision)}>{run.decision}</Pill><p>{run.rationale}</p></div>)}</div></article></>
          ) : null}
      ...
      <style jsx>{`
        ...
        .assuranceRisk-selector { padding: 14px 0; }
        ...
        .outcome identity-card { border: 1px solid rgba(143,181,255,.15); border-radius: 14px; background: rgba(55,89,175,.08); padding: 18px; }
        .outcome identity-card > span { color: #72809a; font-size: 10px; letter-spacing: .1em; text-transform: uppercase; }
        .outcome identity-card > strong { display: block; margin: 6px 0 7px; font-size: 22px; }
        .outcome identity-card > p { color: #9eacc3; line-height: 1.55; }
        .outcome identity-card > div { display: flex; flex-wrap: wrap; gap: 7px; }
