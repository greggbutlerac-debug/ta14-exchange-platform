...
export const metadata={title:'TA-14 AEA Connected Provenance Series',description:'Four connected provenance artifacts preserving separate bilateral boundaries and an eventual combined chronology.'};
...
const cards=[
 ...
 {n:'03',title:'Gary Williams / Elias',subtitle:'Individual Provenance Reconciliation',status:'NEXT',href:null,note:'Separate bilateral record will preserve its own evidence and findings.'},
 {n:'04',title:'Combined Multi-Party Record',subtitle:'Cross-Participant Provenance Chronology',status:'FINAL SERIES RECORD',href:null,note:'Will reconcile the connected routes without collapsing one participant into another.'},
];
...
export default function Page(){return <main style={{minHeight:'100vh',color:'#f5f8fc',background:'radial-gradient(circle at 10% 5%,rgba(34,96,153,.26),transparent 27%),radial-gradient(circle at 88% 9%,rgba(225,177,76,.16),transparent 26%),linear-gradient(180deg,#01050b,#04111f 45%,#02070d)'}}><div style={{width:'min(1180px,calc(100% - 40px))',margin:'0 auto',padding:'44px 0 90px'}}>
 ...
 <p style={{maxWidth:920,color:'#a8bfd3',fontSize:20,lineHeight:1.7}}>Four artifacts. Separate evidentiary boundaries. One connected historical problem. This series lets readers move between each bilateral record and the eventual combined chronology without treating exposure, association, or similarity as causation.</p>
 <div style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:16,marginTop:34}}>{cards.map(c=><article key={c.n} style={{border:'1px solid rgba(148,163,184,.22)',borderRadius:22,padding:24,background:'rgba(6,20,34,.82)'}}>
   <div style={{display:'flex',justifyContent:'space-between',gap:14,alignItems:'start'}}><span style={{width:45,height:45,borderRadius:99,display:'grid',placeItems:'center',border:'1px solid rgba(239,198,110,.36)',color:'#efc66e',fontWeight:950}}>{c.n}</span><span style={{color:c.status==='PUBLISHED'?'#efc66e':'#718aa0',fontSize:10,fontWeight:900,letterSpacing:'.12em'}}>{c.status}</span></div>
   ...
   {c.href?<Link href={c.href} style={{display:'inline-flex',marginTop:8,padding:'12px 16px',borderRadius:10,background:'linear-gradient(135deg,#f1cd7d,#d3a347)',color:'#07111b',fontWeight:950,textDecoration:'none'}}>Open artifact →</Link>:<span style={{display:'inline-flex',marginTop:8,padding:'12px 16px',borderRadius:10,border:'1px solid rgba(148,163,184,.22)',color:'#7890a4'}}>Publication pending</span>}
 </article>)}</div>
 <section style={{marginTop:26,border:'1px solid rgba(239,198,110,.30)',borderRadius:22,padding:25,background:'rgba(12,22,30,.78)'}}><h2 style={{marginTop:0}}>Series rule</h2><p style={{color:'#a8bfd3',lineHeight:1.75,fontSize:17}}>A connected chronology does not create a connected attribution. Each bilateral artifact must stand on its own affirmative evidence. The combined record may map transmission pathways and exposure opportunities, but it cannot manufacture proposition-level causation that the individual records do not establish.</p></section>
