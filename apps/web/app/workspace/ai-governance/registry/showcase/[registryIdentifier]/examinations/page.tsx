...
export default async function ExaminationChronologyPage({params}:{params:Promise<{registryIdentifier:string}>}){
 ...
 const {data,error}=await supabase.from('consequence_examination_runs').select('run_id,freeze_record_id,freeze_sha256,operator_name,final_determination,run_sha256,started_at,sealed_at,outcome_record').eq('governance_registry_identifier',registryIdentifier).eq('status','SEALED').order('sealed_at',{ascending:false});
 ...
 return <main style={{maxWidth:1100,margin:'0 auto',padding:'48px 24px 90px',color:'#e5e7eb'}}>
  ...
  <div style={{display:'grid',gap:14,marginTop:30}}>{runs.length===0?<section style={panel}><h2>No sealed examinations linked yet.</h2><p>The architecture may be registered without implying examination, validation, certification, or universal effectiveness.</p></section>:runs.map((run,i)=><article key={run.run_id} style={panel}>
