...
export default function MachineAdversarialRunner(){
 ...
 const findings=useMemo(()=>machineTests.map(([id])=>{
   ...
   const artifactRefs=(refs[id]??'').split('\n').map(v=>v.trim()).filter(Boolean);
   const computed=evaluateTA14AdversarialEvidence({testId:id,observed,artifactRefs});
   if(computed.result==='PASS' && artifactRefs.length===0) return {...computed,result:'UNRESOLVED' as const,reason:'Observed facts satisfy the gate, but no immutable evidence/artifact reference has been attached. PASS is withheld.'};
   return computed;
 }),[observations,refs]);
 ...
 const passCount=findings.filter(f=>f.result==='PASS').length; const failCount=findings.filter(f=>f.result==='FAIL').length; const unresolvedCount=findings.filter(f=>f.result==='UNRESOLVED').length;
 function exportRecord(){const record={schema:'TA14-Machine-Adversarial-Examination-v1',generatedAt:new Date().toISOString(),standing,counts:{pass:passCount,fail:failCount,unresolved:unresolvedCount},evidenceRule:'PASS requires both qualifying observed facts and at least one immutable evidence/artifact reference.',findings};const blob=new Blob([JSON.stringify(record,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`ta14-machine-adversarial-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url)}
 return <main className="page"><section className="shell"><nav><Link href="/workspace/ai-governance/adversarial-examination">← Examination Lab</Link><span>TA-14 MACHINE EVIDENCE RUNNER · v1</span></nav><header><p>CONSEQUENCE-BEARING EVIDENCE EVALUATION</p><h1>PASS is computed.<br/><em>It is not selected.</em></h1><div className="standing"><div><small>WORKING STANDING</small><strong>{standing}</strong></div><div className="counts"><span>PASS {passCount}</span><span>FAIL {failCount}</span><span>UNRESOLVED {unresolvedCount}</span></div><button onClick={exportRecord}>Export machine findings</button></div></header><section className="notice"><strong>No admissible evidence. No admissible execution.</strong><p>Each machine-covered gate evaluates observed facts. Missing observations remain UNRESOLVED. Contradictory consequence-bearing observations produce FAIL. Even when the facts satisfy a gate, TA-14 withholds PASS until at least one immutable evidence or artifact reference is attached. This runner does not itself create Registry standing or an institutional finding.</p></section><section className="grid">{machineTests.map(([id,title],i)=>{const finding=findings[i] as TA14AdversarialFinding;return <article key={id}><div className="top"><b>{id}</b><span className={finding.result.toLowerCase()}>{finding.result}</span></div><h2>{title}</h2><p className="reason">{finding.reason}</p><div className="facts">{observationKeys[id].map(key=><label key={key}><span>{key}</span><select value={observations[id]?.[key]??'unset'} onChange={e=>setObservations(current=>({...current,[id]:{...(current[id]??{}),[key]:e.target.value as Tri}}))}><option value="unset">UNOBSERVED</option><option value="true">TRUE</option><option value="false">FALSE</option></select></label>)}</div><label className="refs"><span>Immutable evidence / artifact references — required for PASS</span><textarea value={refs[id]??''} onChange={e=>setRefs(c=>({...c,[id]:e.target.value}))} placeholder="One artifact ID, hash, execution log, repository commit/path, or preserved evidence reference per line"/></label></article>})}</section></section><style jsx>{`.page{min-height:100vh;background:#02070d;color:#edf7fb}.shell{width:min(1320px,calc(100% - 32px));margin:auto;padding:24px 0 70px}nav{display:flex;justify-content:space-between;border:1px solid #193243;background:#071824;padding:14px 16px;border-radius:14px;font-size:12px;font-weight:900;letter-spacing:.08em}nav a{color:#9bdcf2;text-decoration:none}header{padding:58px 0 24px}header>p{color:#e5b960;font-size:11px;font-weight:900;letter-spacing:.16em}h1{font-size:clamp(42px,7vw,78px);line-height:.95;margin:12px 0 28px}h1 em{font-style:normal;color:#83dff3}.standing{display:grid;grid-template-columns:1.4fr 2fr auto;align-items:center;gap:18px;border:1px solid #173347;background:#06141f;padding:18px;border-radius:16px}.standing>div:first-child{display:grid;gap:4px}.standing small{color:#7894a3;font-weight:900}.standing strong{font-size:20px}.counts{display:flex;gap:16px;color:#a8bdc7;font-size:12px;font-weight:900}.standing button{background:#e5b960;border:0;border-radius:10px;padding:12px 15px;font-weight:900}.notice{border:1px solid #514628;background:#171407;padding:20px;border-radius:16px;margin:14px 0 24px}.notice strong{color:#e5b960}.notice p{color:#b5b09e;line-height:1.6}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.grid article{border:1px solid #173347;background:#06141f;border-radius:16px;padding:18px}.top{display:flex;justify-content:space-between}.top b{color:#83dff3}.top span{font-size:11px;font-weight:900;letter-spacing:.08em}.pass{color:#83e6b0}.fail{color:#ff8e8e}.unresolved{color:#e5b960}.grid h2{font-size:19px}.reason{color:#9cb1bc;min-height:48px}.facts{display:grid;gap:8px}.facts label,.refs{display:grid;grid-template-columns:1fr 150px;gap:10px;align-items:center}.facts span,.refs span{font-size:12px;color:#bed0d8;overflow-wrap:anywhere}.facts select,.refs textarea{background:#03101a;color:#edf7fb;border:1px solid #234355;border-radius:9px;padding:9px}.refs{grid-template-columns:1fr;margin-top:12px}.refs textarea{min-height:70px}@media(max-width:850px){.grid{grid-template-columns:1fr}.standing{grid-template-columns:1fr;gap:10px}.counts{flex-wrap:wrap}nav{gap:12px;flex-direction:column}}`}</style></main>
}
