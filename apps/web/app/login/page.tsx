    ...
    <main className="account-page">
      <section className="account-shell">
        ...
        <div className="account-panel">
          ...
          <div className="autofill-notice" role="note">
            <strong>New registrant?</strong> Your browser or password manager may prefill saved credentials on this page. TA-14 does not display or assign an existing password to you. If you have never created an Exchange account, ignore any prefilled sign-in credentials and use the New to the Exchange form.
          </div>
          ...
          <div className="form-grid">
            <form action={login} autoComplete="on" className="account-form">
              ...
              <label>Email address<input autoComplete="username" name="email" placeholder="you@example.com" required type="email" /></label>
            ...
            <form action={signup} autoComplete="off" className={`account-form ${registeringGovernance ? "recommended" : ""}`}>
              <input type="hidden" name="next" value={next} />
              <div className="form-heading">
                <span className="form-number">02</span>
                <div><h3>New to the Exchange?</h3><p>Enter your own email and create a new password. TA-14 never supplies a password here.</p></div>
              </div>
              {registeringGovernance ? <div className="recommended-label">NEW REGISTRANTS START HERE</div> : null}
              <label>Email address<input autoComplete="off" name="email" placeholder="Enter your email address" required type="email" /></label>
              <label>Create password<input autoComplete="new-password" minLength={8} name="password" placeholder="Create at least eight characters" required type="password" /></label>
              <label>Confirm new password<input autoComplete="new-password" minLength={8} name="confirmPassword" placeholder="Repeat the password you just created" required type="password" /></label>
              <button type="submit">{registeringGovernance ? "Create my account & continue registration" : "Create my exchange account"}</button>
            </form>
          </div>
