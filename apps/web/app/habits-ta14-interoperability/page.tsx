...
type ContractCase = "both"|"auth-missing"|"habits-refuse"|"ta14-hold"|"habits-stale"|"ta14-stale"|"missing"|"unverifiable"|"conflict"|"changed";
...
function EnforcementContractLab(){
 ...
 const cases:Record<ContractCase,{habits:string;authorization:string;ta14:string;iface:string;release:string;why:string}>={
  "both":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"VERIFIED + CURRENT",release:"RELEASE ELIGIBLE · FINAL CHECK REQUIRED",why:"Both independently required determinations and the interface object/state have current standing. Release eligibility is not release. Current standing must be checked again at the final release boundary. Eligibility is limited to the exact jointly examined path and does not prove wider non-bypassability."},
  "auth-missing":{habits:"CURRENT · UAB: ALLOW",authorization:"ABSENT / NOT ATTRIBUTABLE",ta14:"CURRENT · ALLOW",iface:"VERIFIED + CURRENT",release:"NO RELEASE · AUTHORIZATION NOT ESTABLISHED",why:"Positive architectural determinations do not manufacture legitimate governance authorization. The authorization required for this proposed consequence must be separately attributable, in scope, current, and bound to the same action."},
  "habits-refuse":{habits:"CURRENT · REFUSE",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"VERIFIED + CURRENT",release:"NO RELEASE · UAB: REFUSE",why:"TA-14 ALLOW cannot route around UAB: REFUSE. A positive result from one architecture is not the other's permission."},
  "ta14-hold":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · HOLD",iface:"VERIFIED + CURRENT",release:"NO RELEASE · TA-14 HOLD",why:"UAB: ALLOW cannot route around TA-14 HOLD on the jointly examined path."},
  "habits-stale":{habits:"STALE",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"VERIFIED + CURRENT",release:"NO RELEASE · STALE DETERMINATION",why:"A stale HABITS determination lacks current standing for final release."},
  "ta14-stale":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"STALE",iface:"VERIFIED + CURRENT",release:"NO RELEASE · STALE DETERMINATION",why:"A stale TA-14 determination cannot be treated as present authority for consequence."},
  "missing":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"ABSENT",iface:"INCOMPLETE",release:"NO RELEASE · REQUIRED STATE ABSENT",why:"Absence is not permission. The release boundary remains closed until the required determination exists and is attributable to the same jointly examined state."},
  "unverifiable":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"UNVERIFIABLE",release:"NO RELEASE · INTERFACE NOT ESTABLISHED",why:"Two positive determinations cannot establish release if the object/state connecting them cannot itself be verified."},
  "conflict":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"CONFLICTING STATE",release:"NO RELEASE · CONFLICT UNRESOLVED",why:"Conflicting interface conditions prevent the final release boundary from establishing one attributable current state for the same proposed consequence. Conflict remains a prevention condition until resolved and revalidated."},
  "changed":{habits:"CURRENT · UAB: ALLOW",authorization:"ATTRIBUTABLE · CURRENT",ta14:"CURRENT · ALLOW",iface:"MATERIAL STATE CHANGED",release:"NO RELEASE · REVALIDATION REQUIRED",why:"A material change before irreversible release invalidates reliance on the prior release-eligible state. The changed state must be re-evaluated."}
 };
 const x=cases[test];
 return <section id="enforcement-contract"><p className="eyebrow">PROPOSED ENFORCEMENT CONTRACT</p><h2>Neither positive result releases execution by itself.</h2>
  ...
  <div className="contractFlow"><div><small>HABITS UAB + EXECUTION BOUNDARY</small><strong>{x.habits}</strong></div><i>+</i><div><small>GOVERNANCE AUTHORISATION</small><strong>{x.authorization}</strong></div><i>+</i><div><small>INTERFACE OBJECT / STATE</small><strong>{x.iface}</strong></div><i>+</i><div><small>TA-14</small><strong>{x.ta14}</strong></div><i>→</i><div className={x.release.startsWith("RELEASE ELIGIBLE")?"release yes":"release no"}><small>FINAL RELEASE HOLDER · JOINT DEFINITION OPEN</small><strong>{x.release}</strong></div></div>
  <div className="contractControls">
   <button className={test==="both"?"selected":""} onClick={()=>setTest("both")}>ALL REQUIRED STATES CURRENT</button><button className={test==="auth-missing"?"selected":""} onClick={()=>setTest("auth-missing")}>GOVERNANCE AUTHORISATION ABSENT</button><button className={test==="habits-refuse"?"selected":""} onClick={()=>setTest("habits-refuse")}>UAB: REFUSE + TA-14 ALLOW</button><button className={test==="ta14-hold"?"selected":""} onClick={()=>setTest("ta14-hold")}>UAB: ALLOW + TA-14 HOLD</button><button className={test==="habits-stale"?"selected":""} onClick={()=>setTest("habits-stale")}>HABITS STALE</button><button className={test==="ta14-stale"?"selected":""} onClick={()=>setTest("ta14-stale")}>TA-14 STALE</button><button className={test==="missing"?"selected":""} onClick={()=>setTest("missing")}>MISSING DETERMINATION</button><button className={test==="unverifiable"?"selected":""} onClick={()=>setTest("unverifiable")}>UNVERIFIABLE INTERFACE</button><button className={test==="conflict"?"selected":""} onClick={()=>setTest("conflict")}>CONFLICTING INTERFACE</button><button className={test==="changed"?"selected":""} onClick={()=>setTest("changed")}>MATERIAL CHANGE BEFORE RELEASE</button>
  </div>
  <div className="contractWhy"><b>WHY</b><p>{x.why}</p></div>
  <div className="rule">PROPOSED RELEASE RULE · For the jointly examined path, HABITS UAB: ALLOW + HABITS execution-boundary enforcement + separately attributable legitimate governance authorisation + a current verifiable interface state + TA-14: ALLOW may establish RELEASE ELIGIBLE only. Actual irreversible release remains unavailable until the final release boundary re-checks that every required determination, the interface object/state, freshness/continuity requirements, and material conditions still have current, attributable, verifiable standing for the same proposed consequence. Any material change before release returns the path to prevention and revalidation. This rule does not establish system-wide non-bypassability.</div>
 </section>
}
...
export default function Page(){
 ...
 return <main className="page"><div className="wrap">
  <nav><Link href="/">TA-14 EXCHANGE</Link><div className="navLinks"><Link href="/governance-showcase/TA-14-AIGR-000044">HABITS GOVERNANCE SHOWROOM →</Link><Link href="/governance-showcase">ALL GOVERNANCE SHOWROOMS →</Link></div></nav>
  <header>
   <p className="eyebrow">HABITS × TA-14 · INTEROPERABILITY EXAMINATION LAB</p><div className="identityLink"><span>REGISTERED GOVERNANCE · TA-14-AIGR-000044</span><Link href="/governance-showcase/TA-14-AIGR-000044">EXPLORE HABITS ITSELF →</Link></div>
  ...
  <section id="candidate-examination-object"><p className="eyebrow">CANDIDATE JOINT EXAMINATION OBJECT</p><h2>Define this together before anything is frozen.</h2>
  <p className="lede small">The conceptual seam is now sufficiently bounded to move toward a jointly authored object. The fields below are intentionally OPEN until HABITS and TA-14 agree on the exact ecological intervention and state meanings.</p>
  <div className="objectGrid">
   {[
    ["01","CONCRETE ECOLOGICAL INTERVENTION","OPEN · JOINT DEFINITION"],
    ["02","PROPOSED CONSEQUENCE","OPEN · JOINT DEFINITION"],
    ["03","HABITS GOVERNING PREDICATES","OPEN · HABITS DEFINITION"],
    ["04","EVIDENCE + EVIDENTIARY STANDING","OPEN · JOINT DEFINITION"],
    ["05","HABITS DETERMINATION STATE","UAB: ALLOW / REFUSE / UNKNOWN"],
    ["06","HABITS EXECUTION-BOUNDARY STATE","ENFORCED / NOT ESTABLISHED"],
    ["07","LEGITIMATE GOVERNANCE AUTHORISATION","SEPARATELY ATTRIBUTABLE · SCOPE + CURRENCY REQUIRED"],
    ["08","INTERFACE OBJECT / STATE","OPEN · JOINT DEFINITION"],
    ["09","TA-14 DETERMINATION STATE","ALLOW / HOLD / DENY / ESCALATE"],
    ["10","RELEASE ELIGIBILITY","NOT ACTUAL RELEASE"],
    ["11","COMPONENT HOLDING FINAL RELEASE","OPEN · JOINT DEFINITION"],
    ["12","FINAL CURRENT-STANDING CHECK","REQUIRED IMMEDIATELY BEFORE IRREVERSIBLE RELEASE"],
    ["13","STATE VOCABULARY","DEFINE ALLOW · REFUSE · HOLD · UNKNOWN + ANY REQUIRED STATES"],
    ["14","CONTINUED PREVENTION CONDITIONS","STALE · ABSENT · UNVERIFIABLE · CONFLICT · UNKNOWN · MATERIAL CHANGE"]
   ].map(([n,k,v])=><div className="objectRow" key={n}><b>{n}</b><span>{k}</span><strong>{v}</strong></div>)}
  </div>
  <div className="rule">FREEZE CONDITION · No joint freeze occurs until the intervention, consequence, predicates, evidence, interface object/state, release-holder and final-release conditions are agreed as one examinable object. An eventual interoperability finding remains contingent on the evidence.</div>
 </section>

  <section><p className="eyebrow">PROPOSED JOINT FREEZE</p><h2>What we would freeze before a formal examination.</h2>
   <div className="chain">{["EXECUTION SCENARIO","PROPOSED CONSEQUENCE","OBSERVATION PATH","MATERIALITY TEST","GOVERNING PREDICATES","HABITS UAB + ENFORCEMENT","LEGITIMATE GOVERNANCE AUTHORISATION","INTERFACE OBJECT / STATE","TA-14 RESPONSIBILITY","RELEASE ELIGIBILITY","FINAL RELEASE HOLDER","CURRENT-STANDING CHECK","STATE VOCABULARY","UNKNOWN / REVALIDATION BRANCHES","NON-BYPASSABILITY TEST"].map(x=><span key={x}>{x}</span>)}</div>
  </section>
