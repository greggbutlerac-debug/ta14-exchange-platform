...
export default function ChallengeExam(){
 ...
 const complete=Boolean(baseline&&changed&&bypass&&replay?.replay?.match);
 const observed=[
  Boolean(baseline),
  Boolean(baseline),
  ...
  Boolean(replay?.replay?.match),
 ];
 const download=last?'data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(runs[runs.length-1],null,2)):'#';
 const report=complete?{schema:'TA14_BOUNDED_EXAMINATION_REPORT_V1',finding:'SUPPORTED — BOUNDED EXAMINATION',mechanism:baseline.mechanism,acceptanceSource:'Frozen eight-condition public challenge',controlledDelta:{field:'localStanding',baseline:true,changed:false,unchanged:['passportIntegrity','freshness','scope','commitBinding','bypass']},executions:{baseline,changed,bypass},replay,claimBoundary:'Bounded to TA14-AFA-EABA-SX-001 and its durable database effect sink.'}:null;
