...
const requirements=[
 ['01','Freeze the mechanism.','Frozen mechanism identity: TA14-AFA-EABA-SX-001 · v1.2.0. The server harness exposes that same immutable mechanism identity on every run.'],
 ...
 ['08','Replay it.','REPLAY verifies the preserved ledger receipt and original consequence correspondence, then freshly re-executes the preserved action through the frozen mechanism. Input, determination, gate state, and protected-consequence outcome must correspond; the re-execution receives its own durable receipt.'],
] as const;
export default function ChallengeExam(){
 ...
 async function doReplay(){
  ...
  const replayData=await res.json();setReplay(replayData);
  if(replayData?.replay?.match&&bypass?.examinationId){const finalRes=await fetch('/api/afa-eaba-challenge',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'finalize',examinationId:bypass.examinationId,replay:replayData})});setFinalReport(await finalRes.json());}
  setBusy(false);
 }
 ...
 const complete=Boolean(baseline&&changed&&bypass&&replay?.replay?.match&&finalReport?.preservation?.reportId);
 const observed=[
  Boolean(baseline),
  Boolean(baseline),
  Boolean(baseline&&changed&&baseline.input?.localStanding===true&&changed.input?.localStanding===false),
  Boolean(baseline&&changed&&baseline.determination!==changed.determination),
  Boolean(bypass),
  Boolean(bypass&&bypass.protectedConsequence?.fired===false),
  Boolean(bypass?.receipt?.receiptId),
  Boolean(replay?.replay?.match&&finalReport?.preservation?.reportId),
 ];
 const download=last?'data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(runs[runs.length-1],null,2)):'#';
 const report=complete?finalReport.report:null;
 const reportDownload=report?'data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(report,null,2)):'#';
...
<Link href="/afa-eaba-operational-challenge" style={{padding:'10px 12px',border:'1px solid #e7c76e',borderRadius:8,color:'#e7c76e',textDecoration:'none',fontSize:10,fontWeight:900}}>INSPECT FROZEN CHALLENGE</Link>
   ...
   <section style={{padding:26,border:'1px solid #6c5b31',borderRadius:18,background:'linear-gradient(145deg,rgba(83,62,14,.20),rgba(4,21,28,.96))',marginBottom:18}}>
    ...
    <p style={{color:'#a8c1c8',lineHeight:1.7,maxWidth:930}}>Before TA-14 answers the challenge, the source requirement is shown first. The supplied screenshot is preserved as the primary visual evidence. Because that screenshot ends before the full eight-step condition is visible, a plain transcription of the remaining operative text is preserved directly beneath it for completeness and accessibility.</p>
    {sourceScreenshotData&&<img src={sourceScreenshotData} alt="Display copy of the Terry Snyder LinkedIn acceptance-condition screenshot supplied to TA-14" style={{display:'block',width:'100%',height:'auto',margin:'20px 0',borderRadius:12,border:'1px solid #38525a'}}/>}
    <div style={{marginTop:20,padding:'20px 22px',border:'1px solid #38525a',borderRadius:12,background:'#020b0f'}}>
      <div style={{fontSize:10,fontWeight:950,letterSpacing:'.14em',color:'#71e7df'}}>FULL ACCEPTANCE-CONDITION TRANSCRIPTION</div>
      <div style={{marginTop:14,color:'#d7e7ea',fontSize:14,lineHeight:1.72,whiteSpace:'pre-line'}}>{`So the acceptance condition is obvious:
...
That has been the target the entire time.`}</div>
   ...
   {bypass&&<div style={{marginTop:12,padding:20,border:'1px solid '+(bypass.protectedConsequence?.fired?'#ff7b86':'#71e7df'),borderRadius:12}}><b>BYPASS RESULT · PROTECTED CONSEQUENCE FIRED: {String(bypass.protectedConsequence?.fired).toUpperCase()}</b><p style={{color:'#9fb7be'}}>An invocation was attempted against the database-enforced protected-effect gate. Authorization failed, no effect row was created, and a separate post-gate database read recorded NO_EFFECT_ROW_OBSERVED. The durable receipt preserves that observation.</p></div>}
   ...
   {replay&&<section style={{marginTop:22,padding:22,border:'1px solid '+(replay.replay?.match?'#71e7df':'#ff7b86'),borderRadius:14}}><b style={{fontSize:24}}>REPLAY INTEGRITY: {replay.replay?.match?'MATCH':'MISMATCH'}</b><p style={{color:'#9fb7be'}}>The server verified the preserved receipt and original consequence correspondence, then freshly re-executed the preserved action through the frozen mechanism. Input, determination, gate state, and protected-consequence outcome must correspond, and the replay execution is preserved under its own receipt.</p></section>}
   <section style={{marginTop:34,padding:24,border:'1px solid #28545b',borderRadius:14,background:'#04151c'}}><b style={{color:'#71e7df'}}>ACCEPTANCE EVIDENCE SUMMARY · {observed.filter(Boolean).length} / 8 {complete?'OBSERVED · EXECUTION COMPLETE':'OBSERVED'}</b><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8,marginTop:14}}>{requirements.map(([n,required],i)=><div key={n} style={{padding:13,border:'1px solid #21444b',borderRadius:9,background:'#020b0f'}}><span style={{color:observed[i]?'#71e7df':'#e7c76e',fontWeight:950}}>{n} · {observed[i]?'OBSERVED · PASS':'PENDING'}</span><div style={{marginTop:6,fontSize:10,color:'#a8c1c8'}}>{required}</div></div>)}</div>{complete&&<div style={{marginTop:18,padding:16,border:'1px solid #71e7df',borderRadius:10}}><b style={{color:'#71e7df'}}>BOUNDED EXAMINATION FINDING · SUPPORTED</b><div style={{fontSize:10,color:'#71e7df',marginTop:6}}>DURABLE REPORT: {finalReport?.preservation?.reportId} · SHA-256 {finalReport?.preservation?.integrityHash?.slice(0,18)}…</div><p style={{color:'#a8c1c8',lineHeight:1.65}}>Under TA14-AFA-EABA-SX-001 v1.2.0, the frozen examination observed an ALLOW path producing the defined durable protected effect; withdrawal of local standing changed the determination to HOLD; the challenged bypass did not create the protected effect; execution evidence was durably preserved; and subsequent replay verified receipt integrity and effect correspondence and freshly re-executed the preserved action with matching bounded outcome.</p><a download="ta14-afa-eaba-bounded-examination-report.json" href={reportDownload} style={{...btn,display:'inline-block',textDecoration:'none'}}>DOWNLOAD COMPLETE EXAMINATION REPORT</a></div>}</section>
   <section style={{marginTop:18,padding:20,border:'1px dashed #6c5b31',borderRadius:12,color:'#d8c98f',lineHeight:1.7}}><b>CLAIM BOUNDARY.</b> This is a bounded executable examination harness. It demonstrates the frozen bounded mechanism represented here, its determination change, database-enforced bypass refusal, observed protected-effect presence/absence, append-only server-side receipt preservation, and ledger verification plus fresh operational replay. It does not by itself establish universal deployment, enforcement in an unrelated external system, certification, or every possible AFA/EABA implementation. Its protected consequence is the deliberately bounded durable database effect defined by this frozen examination.</section>
  </div>
 </main>
}
