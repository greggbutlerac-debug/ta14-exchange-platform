...
const requirements=[
 ['01','Freeze the mechanism.','Frozen mechanism identity: TA14-AFA-EABA-SX-001 · v1.0.1. The server harness exposes that same immutable mechanism identity on every run.'],
 ...
 ['06','Show the protected consequence did not fire.','The bypass run makes an explicit protected-consequence invocation attempt. The server-side EABA gate rejects authorization and records result=BLOCKED_AT_EABA_GATE and fired=false in the integrity-bound receipt.'],
 ['07','Preserve the receipt.','Each execution returns TA14_EXECUTION_RECEIPT_V1. Its SHA-256 hash covers the complete recursively canonicalized evidence object—including nested input, mechanism, determination, consequence result, and trace—and the JSON can be downloaded.'],
 ['08','Replay it.','REPLAY first verifies that the supplied receipt itself is intact, then independently recomputes the action under the frozen mechanism. Both receipt integrity and deterministic recomputation must match.'],
] as const;
...
That has been the target the entire time.`}</div>
   ...
   <div style={{padding:16,border:'1px solid #21444b',borderRadius:11,background:'#020b0f',marginTop:16,color:'#a8c1c8',lineHeight:1.65}}><b style={{color:'#71e7df'}}>WHAT COUNTS AS SUCCESS IN THIS BOUNDED EXAMINATION?</b><br/>Baseline reaches an ALLOW state and may fire the protected consequence. Removing local standing changes the determination and prevents release. A bypass attempt makes an explicit invocation attempt but the EABA gate returns BLOCKED_AT_EABA_GATE and the protected consequence remains unfired. Each run produces an integrity-bound receipt. Replay separately verifies receipt integrity and deterministic recomputation.</div><div style={{display:'flex',gap:9,flexWrap:'wrap',marginTop:20}}><button disabled={busy} onClick={()=>run('baseline')} style={btn}>1 · RUN BASELINE</button><button disabled={busy||!baseline} onClick={()=>run('changed-condition')} style={btn}>2 · CHANGE ONE CONDITION</button><button disabled={busy||!changed} onClick={()=>run('bypass')} style={btn}>3 · ATTEMPT BYPASS</button><button disabled={busy||!bypass} onClick={doReplay} style={btn}>4 · REPLAY LAST RECEIPT</button></div></section>
   ...
   {bypass&&<div style={{marginTop:12,padding:20,border:'1px solid '+(bypass.protectedConsequenceFired?'#ff7b86':'#71e7df'),borderRadius:12}}><b>BYPASS RESULT · PROTECTED CONSEQUENCE FIRED: {String(bypass.protectedConsequence?.fired).toUpperCase()}</b><p style={{color:'#9fb7be'}}>An invocation was attempted. The server-side EABA gate did not authorize it; the protected consequence returned BLOCKED_AT_EABA_GATE and fired=false. The trace and integrity-bound receipt preserve that result.</p></div>}
   ...
   {replay&&<section style={{marginTop:22,padding:22,border:'1px solid '+(replay.replay?.match?'#71e7df':'#ff7b86'),borderRadius:14}}><b style={{fontSize:24}}>REPLAY INTEGRITY: {replay.replay?.match?'MATCH':'MISMATCH'}</b><p style={{color:'#9fb7be'}}>The server first verified the supplied receipt evidence against its SHA-256 hash, then independently recomputed the same action under the frozen mechanism. Both checks must match.</p></section>}
