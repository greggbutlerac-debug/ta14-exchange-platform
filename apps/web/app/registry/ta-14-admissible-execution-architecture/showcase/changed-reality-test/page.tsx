...
const raw:Record<Run,RawState>={
 baseline:{tag:"T₀",name:"FROZEN BASELINE",tone:"blue",change:"No changed-context event has been introduced. The governed proposition is frozen so every examination case can be compared against the same starting state.",fact:"Baseline frozen",provenance:{sourceId:"T0-FROZEN-CONTROL",signed:true,sequenceContinuous:true},occupancy:21,fireSmokeOverride:false},
...
async function downloadReceipt(run:Run){
 ...
 run==="unresolved"?"ADVERSARIAL VARIABLE: provenance completeness for the stated changed fact.":run==="baseline"?"CONTROL STATE: no Delta N introduced.":"CHANGED-REALITY VARIABLE: Delta N · authenticated changed reality.",
...
<header className="crt-hero"><div className="crt-badges"><b>TA-14 AUTHORITY</b><b>PUBLIC TECHNICAL SHOWROOM</b><b>CONTROLLED RUNTIME TEST</b><b>R1 · FROZEN EXAMINATION RECORD</b></div><p className="crt-eye">THE CONSEQUENCE BOUNDARY · FORMAL CHANGED-REALITY EXAMINATION</p><h1>THE CHANGED-<br/><em>REALITY</em> TEST</h1><p className="crt-challenge">CAN ONE AUTHORITATIVE FACT STOP THE SAME CONSEQUENCE?</p><p className="crt-lede">R1 freezes the examination boundary: same T₀ authority, same actor, same proposed consequence, and same execution path. ΔN₁ and ΔN₂ vary authenticated changed reality; ΔN₃ separately challenges provenance sufficiency. Inspect whether TA-14 derives the consequence state from the supplied facts and provenance before commit.</p></header>
...
<section><p className="crt-eye">05 · VERIFIED PRODUCTION EVIDENCE SET</p><h2 className="crt-title">Four fresh receipts preserve the bounded runtime result.</h2><div className="crt-record"><div><small>T₀ · CONTROL</small><b>READY TO TEST · COMMIT NOT YET REQUESTED · SHA-256 4ca655769755c36b394ff208ea1b49a8a8b11b56cbef029970001ecaf5397194</b></div><div><small>ΔN₁ · AUTHENTICATED OCCUPANCY CHANGE</small><b>ESTABLISHED → ALLOW → COMMIT MAY PROCEED · SHA-256 a824df1cea86d952fd4dd1ec87f6efb4a3c5810bbbbad90e441f5d65a14a895f</b></div><div><small>ΔN₂ · AUTHENTICATED FIRE-SMOKE CHANGE</small><b>DEFEATED → HOLD → COMMIT BLOCKED · SHA-256 bbd8918bebbd47114c3d72f80ebcfc90615208af36c1423c179a3edb6bc76155</b></div><div><small>ΔN₃ · INCOMPLETE PROVENANCE</small><b>UNRESOLVED → ESCALATE → COMMIT BLOCKED · SHA-256 db47c21446b0351db81e02826f12441b9d928b84fd864cca1e48eebb5b4eb130</b></div></div><p className="crt-change">Production evidence set · 2026-09-22 UTC. Each receipt preserves the same frozen T₀ authority, actor, proposed consequence, and execution path. The receipts separately record raw examination input, evaluator-derived evidence / authority / standing, determination, execution disposition, rules applied, and commit-guard authorization. These hashes identify the four fresh receipts generated from the deployed R1 examination; they are integrity digests, not digital signatures or external notarization.</p></section>
<section><p className="crt-eye">06 · THE PUBLIC CHAIN</p><div className="crt-chain">{chain.map((x,i)=><div key={x}><small>0{i+1}</small><b>{x}</b></div>)}</div><div className="crt-bound"><span>PROPOSAL</span><i>→</i><strong>TA-14 CONSEQUENCE BOUNDARY</strong><i>→</i><span>REALITY</span></div></section>
<section className="crt-falsify"><p className="crt-eye">R1 · FROZEN EXAMINATION RECORD</p><h2 className="crt-title">The claim survives only if the preserved record survives its falsifier.</h2><div className="crt-record"><div><small>CONTROL VARIABLES</small><b>Same T₀ authority · same actor · same proposed consequence · same execution path</b></div><div><small>PRIMARY CHANGED-REALITY VARIABLE</small><b>ΔN₁ / ΔN₂ · authenticated changed reality</b></div><div><small>ADVERSARIAL PROVENANCE CASE</small><b>ΔN₃ · same stated occupancy change as ΔN₁, but provenance is incomplete</b></div><div><small>PRESERVED ACROSS CASES</small><b>Frozen control · raw inputs · derivation trace · exact decision point · determination · execution disposition</b></div><div><small>DERIVATION REQUIREMENT</small><b>TA-14 must derive standing from underlying authoritative facts before consequence binds.</b></div><div><small>FIXTURE DISCIPLINE</small><b>ΔN fixtures supply raw condition facts and structured provenance only; standing, determination, and commit disposition are evaluator outputs. A separate commit guard authorizes commit only for ALLOW.</b></div></div><p className="crt-eye">FALSIFICATION CONDITIONS</p><div className="crt-fgrid">{["The runs do not begin from the same frozen T₀ authority, actor, proposed consequence, and execution path.","An authenticated changed-reality case (ΔN₁ or ΔN₂) cannot be traced to its asserted authoritative source with preserved provenance.","TA-14 receives standing upstream instead of deriving it from underlying authoritative facts.","A different execution path is used between runs.","A hidden manual judgment determines the standing result.","The record cannot show where ΔN changes the derivation before consequence binds.","A HOLD, DENY, or ESCALATE can be ignored while the governed consequence still commits."].map((x,i)=><p key={x}><b>{String(i+1).padStart(2,"0")}</b>{x}</p>)}</div></section>
<section className="crt-falsify"><p className="crt-eye">R1 · BOUNDED EXAMINATION RESULT</p><h2 className="crt-title">R1 establishes this bounded derivation result.</h2><div className="crt-record"><div><small>CONTROL</small><b>Frozen across T₀, ΔN₁, ΔN₂, and ΔN₃</b></div><div><small>ΔN₁</small><b>Standing established · ALLOW · commit may proceed</b></div><div><small>ΔN₂</small><b>Standing defeated · HOLD · commit blocked</b></div><div><small>ΔN₃</small><b>Evidence unresolved · ESCALATE · commit blocked</b></div><div><small>RECORD INTEGRITY</small><b>Run-specific receipt · timestamp · ΔN identifier · SHA-256 integrity hash</b></div></div><p className="crt-change"><b>Bounded conclusion · examination run 2026-09-22:</b> With the frozen T₀ control held constant, ΔN₁ (occupancy 21 → 27) produced ESTABLISHED standing → ALLOW → COMMIT MAY PROCEED, while ΔN₂ (fire-smoke override INACTIVE → ACTIVE) produced DEFEATED standing → HOLD → COMMIT BLOCKED. The hardened fixtures supply raw changed facts, structured provenance, and raw condition fields only. Standing and determination are evaluator outputs, and an independent commit guard authorizes commit only for ALLOW. ΔN₃ additionally demonstrates that incomplete provenance cannot become execution permission. This establishes only the behavior demonstrated by these bounded examination cases. It does not establish universal runtime efficacy or independent physical enforcement outside this examination environment.</p></section>
