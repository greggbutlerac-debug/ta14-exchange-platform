...
export default function PublicRegistryRecordPage() {
  const params = useParams<{ rid: string }>();
  const registryIdentifier = useMemo(
    () => decodeURIComponent(params.rid ?? '').trim().toUpperCase(),
    [params.rid],
  );
