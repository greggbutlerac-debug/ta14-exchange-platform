...
export const metadata = {
  ...
  description: 'Permanent public TA-14 AI Governance Registry record for Validation Standing Assurance (VSA) v1.0 and its governed demonstration lineage.',
};
...
export default function VsaPermanentRegistryRecord() {
  ...
  return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 15% 0%,rgba(28,83,139,.24),transparent 30%),radial-gradient(circle at 90% 8%,rgba(213,167,75,.13),transparent 26%),linear-gradient(180deg,#020813,#06111e 48%,#020710)',color:'#f3f6f9',padding:'64px 20px'}}>
    <div style={{width:'min(1040px,100%)',margin:'0 auto'}}>
      <Link href="/artifacts/founding-demonstrations" style={{color:'#a9bfd2',textDecoration:'none'}}>← Founding Demonstrations</Link>
      <div style={{marginTop:38,border:'1px solid rgba(213,167,75,.28)',borderRadius:28,background:'linear-gradient(135deg,rgba(8,26,45,.92),rgba(4,15,27,.98))',padding:'clamp(28px,6vw,58px)',boxShadow:'0 28px 90px rgba(0,0,0,.3)'}}>
        ...
        <section style={{marginTop:32,borderTop:'1px solid rgba(255,255,255,.07)',paddingTop:28}}>
          ...
          <h2 style={{margin:'10px 0 8px',fontSize:30}}>Three completed governed examination records. Different bounded propositions.</h2>
          <p style={{margin:0,color:'#9fb5c4',lineHeight:1.7}}>Registration is not itself a finding. Each examination below preserves its own frozen proposition, evidence boundary, result, limitations, and governed finding record.</p>
        </section>
        ...
        {caseCard(
          'CASE 03 · FOURTH DEMONSTRATION · R1 · CLOSED',
          'Fourth Demonstration R1 — Independent Bounded Finding',
          'SUPPORTED — BOUNDED PASS',
          'TA14-VSA-4D-R1-GFR-001',
          'Eight declared falsifiers were examined against the frozen review surface. The finding remains bounded to the demonstrated synthetic governance scope. It does not establish global route exhaustiveness, global real-world mission possibility or impossibility, regulatory or clinical authority, treatment or release readiness, or claims outside the frozen surface. Earlier adverse states remain preserved; later success does not retrospectively repair them.',
          '/artifacts/founding-demonstrations#vsa-fourth-demonstration',
          '/artifacts/governed/ta14-vsa-4d-r1-gfr-001'
        )}
        ...
        <div style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:32}}>
          ...
          <Link href="/artifacts/registry" style={{display:'inline-flex',alignItems:'center',minHeight:48,padding:'0 18px',borderRadius:12,textDecoration:'none',fontWeight:800,border:'1px solid rgba(130,169,204,.28)',color:'#c9dbe9'}}>Execution Artifact Registry</Link>
        </div>
      </div>
    </div>
  </main>;
