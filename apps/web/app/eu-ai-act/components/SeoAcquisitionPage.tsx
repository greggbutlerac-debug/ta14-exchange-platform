...
import PaidPlanRail from './PaidPlanRail';
...
export default function SeoAcquisitionPage({eyebrow,title,accent,intro,sourceLabel,sourceHref,sourceNote,cards,steps,faqs,classifierHref,workspaceHref='/eu-ai-act/commercial',workspaceLabel='VIEW EU AI ACT WORKSPACES →'}:Props){
  return <main className="seoPage">
    <nav><Link href="/eu-ai-act">← EU AI ACT WORLD</Link><span>TA-14 · GOVERNED WORLD 05</span><Link href="/eu-ai-act/join?plan=passport">START AT $19/MO →</Link></nav>

    <header className="hero">
      <small>{eyebrow}</small>
      <h1>{title}<br/><em>{accent}</em></h1>
      <p>{intro}</p>
      <div className="actions"><Link className="primary" href={classifierHref}>CHECK MY AI SYSTEM — FREE →</Link><Link className="secondary" href="/eu-ai-act/join?plan=passport">START EVIDENCE PASSPORT · $19/MO →</Link><Link className="secondary" href={workspaceHref}>{workspaceLabel}</Link></div>
    ...
    <section className="cta">
      ...
      <h2>Not ready to subscribe? Classify first.</h2>
      <p>Use the free classifier to establish the system, intended purpose, possible actor role, EU exposure and unresolved facts. When continuing evidence infrastructure is needed, paid access begins at $19 per month. Independent human readiness review remains a separate service.</p>
