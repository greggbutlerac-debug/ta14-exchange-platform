...
export default function Page(){
 ...
 const taReached=scenario==='normal'||connection==='BOUND';
 const decision=useMemo<Decision>(()=>{
   ...
   return 'ALLOW';
 },[passportRevoked,current,evidence,authority,standing]);
 ...
 {scenario!=='normal'&&<label><input type="checkbox" checked={!passportRevoked} onChange={e=>setPassportRevoked(!e.target.checked)}/> Authority Passport not revoked</label>}</div>:<p className="muted">The attempted federated crossing was refused, so this proposed federated consequence does not reach the receiving-domain TA-14 examination. Record where the attempt stopped.</p>}</div></section>
