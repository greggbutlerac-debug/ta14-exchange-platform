...
export default function PublicGovernanceProfilePage() {
  const params = useParams();
  const routeGovernanceId = typeof params?.governanceId === "string" ? params.governanceId : GOVERNANCES[0].id;
  const routeProfile = GOVERNANCES.find((record) => record.id.toLowerCase() === routeGovernanceId.toLowerCase()) ?? GOVERNANCES[0];
  const [view, setView] = useState<DirectoryView>("profile");
  const [query, setQuery] = useState("");
  const [sector, setSector] = useState("ALL");
  const [jurisdiction, setJurisdiction] = useState("ALL");
  const [assurance, setAssurance] = useState("ALL");
  const [sort, setSort] = useState<SortMode>("evidence");
  const [selectedId, setSelectedId] = useState(routeProfile.id);
  ...
  const selected = GOVERNANCES.find((record) => record.id === selectedId) ?? routeProfile;
      ...
      <header className="hero">
        ...
        <div className="hero-layout">
          <section className="hero-copy">
            <span className="eyebrow">Public governance profile · {selected.id}</span>
            <h1>{selected.organization}</h1>
            <p className="hero-lead">{selected.architecture} v{selected.version}. Inspect its registered identity, declared scope, governed routes, execution artifacts, verification record, and append-only challenge history.</p>
            <div className="hero-actions">
              <a className="button primary" href="#profile-inspector">Inspect this governance</a>
              <Link className="button secondary" href="/governance/directory">Return to directory</Link>
            </div>
          ...
          <section className="hero-core" aria-label="Directory evidence core">
            ...
            <div className="core-center"><span>GOV</span><strong>PROFILE</strong><small>Identity → Evidence → Reliance</small></div>
      ...
      <section className="directory-workspace" id="directory">
        <div className="directory-main">
          <div className="section-intro">
            <div><span className="eyebrow">Registered governance profile</span><h2>Inspect the organization, then inspect its evidence.</h2></div>
            <p>This profile describes identity, architecture, scope, ownership, claims, and limitations. Execution artifacts—not registration alone—support evidence-based reliance.</p>
          </div>
