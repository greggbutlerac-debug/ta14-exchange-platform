...
export default function GovernanceRegistrationPage() {
  ...
  const readinessItems = useMemo(() => [
    { label: "Organization identity", passed: checks.identity, action: "Provide public name, legal name, and country." },
    { label: "Architecture identity", passed: checks.architecture, action: "Name, version, and describe the architecture." },
    { label: "Declared scope", passed: checks.scope, action: "Select sectors, jurisdictions, and determinations." },
    { label: "Accountable ownership", passed: checks.owners, action: "Name at least one accountable owner with email." },
    { label: "Claims and limits", passed: checks.claims, action: "Select claims and publish explicit limitations." },
    { label: "Institutional attestations", passed: checks.attestations, action: "Accept terms and attest accuracy and authority." },
  ], [checks]);
  const remainingItems = readinessItems.filter((item) => !item.passed);
  ...
  return (
    <main className="registration-page">
      ...
      <section className="lifecycle-shell" aria-label="TA-14 institutional lifecycle">
        <div className="lifecycle-heading"><span>Institutional lifecycle</span><strong>You are registering the governance identity that must exist before any artifact can enter the Registry.</strong></div>
        <div className="lifecycle-track">
          {EXCHANGE_LIFECYCLE.map((item, index) => (
            <div className={cx("lifecycle-node", item.state)} key={item.label}>
              <Link href={item.href}><span>{item.state === "complete" ? "✓" : String(index + 1).padStart(2, "0")}</span><b>{item.label}</b><small>{item.state === "current" ? "CURRENT STAGE" : item.state === "complete" ? "ESTABLISHED" : "UNLOCKS AFTER REGISTRATION"}</small></Link>
              {index < EXCHANGE_LIFECYCLE.length - 1 ? <i aria-hidden="true">→</i> : null}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="hero">
        ...
        <div className="hero-core">
          ...
          <div className="readiness-ledger">
            {readinessItems.map((item) => <div className={cx("readiness-row", item.passed && "pass")} key={item.label}><span>{item.passed ? "✓" : "!"}</span><b>{item.label}</b><small>{item.passed ? "Satisfied" : item.action}</small></div>)}
          </div>
          <div className="core-rule"><b>No registered governance.</b><strong>No registered artifact.</strong><p>Registration establishes identity and eligibility. It does not certify architecture performance.</p></div>
        </div>
      </section>
      ...
      <section className="workspace">
        ...
        <div className="stage">
          <section className="panel public-preview-panel">
            <div className="preview-head"><div><span>Live registry record preview</span><h2>This is what the public record is becoming.</h2><p>The preview updates from the registration draft. A candidate record is not public, certified, or eligible to register artifacts until institutional approval.</p></div><Badge tone={ready ? "pass" : "review"}>{ready ? "READY FOR REVIEW" : `${remainingItems.length} CONDITIONS REMAIN`}</Badge></div>
      ...
      <section className="after-registration">
        <div className="after-heading"><span>What happens after approval</span><h2>Registration is the beginning of the evidence lifecycle.</h2><p>Approval does not certify the governance. It establishes the attributable identity and active status required to build routes and register execution artifacts.</p></div>
        <div className="after-track">{AFTER_REGISTRATION.map((item, index) => <article key={item.title}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{item.title}</strong><p>{item.detail}</p></div></article>)}</div>
        <div className="after-actions"><Link className="secondary" href="/governance/directory">Browse registered governances</Link><Link className="primary" href="/artifacts">Inspect execution artifacts</Link></div>
      </section>
