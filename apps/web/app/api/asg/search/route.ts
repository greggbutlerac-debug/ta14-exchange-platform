...
function getTrustedUserIp(req:NextRequest):string{
  if(process.env.VERCEL){
    return (req.headers.get("x-vercel-forwarded-for")||req.headers.get("x-forwarded-for")||"").split(",")[0].trim();
  }
  return "";
}
...
export async function POST(req:NextRequest){
  ...
  const userIp=getTrustedUserIp(req);

  try{
    if(!userIp)throw new Error("GOOGLE_WEB_SEARCH_USER_IP_UNAVAILABLE");
    const result=await provider.search(request,{limit:10,userIp});
    const frozenProviderCandidates=result.candidates.slice(0,10).map((r,index)=>({
      providerRank:r.rank||index+1,title:r.title,url:r.url,snippet:r.snippet,displayUrl:r.displayUrl,provider:r.provider,providerResultId:r.providerResultId,
    }));
    ...
    return NextResponse.json({schema:"ta14.asg.runtime-record.v0.2",recordId,request,profile,engineVersion:ASG_ENGINE_VERSION,profileVersion:ASG_PROFILE_VERSION,provider:result.provider,providerLive:result.live,providerWindow:{startRank:1,endRank:frozenProviderCandidates.length,candidateCount:frozenProviderCandidates.length},frozenProviderCandidates,candidates:governed,deliveryCommit:{candidateRanks:admitted.map(c=>c.providerRank),deliveredCount:admitted.length,delivered:admitted},admittedRecord,nonAdmittedRecord,notice:"The provider's first ten candidates are frozen before TA-14 evaluation. Provider rank is preserved. Admitted and non-admitted outcomes are exported as separate evidence records."});
  }catch(error:any){
    const code=String(error?.message||"PROVIDER_ERROR");
    const notConfigured=code==="GOOGLE_WEB_SEARCH_PROVIDER_NOT_CONFIGURED";
    const noUserIp=code==="GOOGLE_WEB_SEARCH_USER_IP_UNAVAILABLE"||code==="GOOGLE_WEB_SEARCH_USER_IP_REQUIRED";
    const reasonCode=notConfigured?"PROVIDER_CANDIDATE_SET_UNAVAILABLE":noUserIp?"PROVIDER_USER_CONTEXT_UNAVAILABLE":"PROVIDER_ERROR";
    const explanation=notConfigured?"The request is preserved, but legitimate Google Web Search Service credentials are not configured. TA-14 will not manufacture provider candidates.":noUserIp?"The request is preserved, but a trusted end-user IP required by Google Web Search Service was not established. No provider request or delivery commit occurred.":"The provider did not establish a candidate set. No delivery commit occurred.";
    return NextResponse.json({schema:"ta14.asg.runtime-record.v0.2",recordId,request,profile,engineVersion:ASG_ENGINE_VERSION,profileVersion:ASG_PROFILE_VERSION,state:"HOLD",reasonCode,explanation,provider:"GOOGLE_WEB_SEARCH_SERVICE",providerLive:false,frozenProviderCandidates:[],candidates:[],deliveryCommit:{candidateRanks:[],deliveredCount:0,delivered:[]},admittedRecord:null,nonAdmittedRecord:null},{status:notConfigured||noUserIp?503:502});
  }
}
