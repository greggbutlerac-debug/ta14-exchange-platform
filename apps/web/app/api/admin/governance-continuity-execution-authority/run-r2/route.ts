...
export async function POST() {
  ...
  const now = new Date();
  const runId = `TA14-GCEA-R2-${now.toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}`;
  const demo = buildPrivateGceaDemonstration(now);
  const e1 = makeChronologyEvent({ runId, sequenceNo: 1, eventType: 'BASELINE', asset: demo.asset, evaluation: demo.baseline, authorityInput: demo.baselineInput, additionalPayload: { gate: 'R2_SEMANTIC_REPLAY_EXECUTION_ENFORCEMENT' } });
  ...
  const e3 = makeChronologyEvent({ runId, sequenceNo: 3, eventType: 'BOUNDARY_DETERMINATION', asset: demo.asset, evaluation: demo.challenged, authorityInput: demo.challengedInput, previousEventHash: e2.eventHash, additionalPayload: { boundary: demo.asset.consequence, executionAttempt: demo.deniedAttempt, expected: { determination: 'DENY', executionPermitted: false } } });
  ...
  const boundaryPass = demo.deniedAttempt.determination === 'DENY' && demo.deniedAttempt.executionPermitted === false;
  if (verification.status !== 'PASS' || verification.semanticReplayCount !== events.length || !boundaryPass) return NextResponse.json({ error: 'Pre-preservation R2 gate failed.', verification, boundaryPass }, { status: 409 });
  ...
  const post = verifyChronology(replay);
  const ok = post.status === 'PASS' && post.semanticReplayCount === replay.length && boundaryPass;
  return NextResponse.json({ ok, gate: 'R2_SEMANTIC_REPLAY_EXECUTION_ENFORCEMENT', run_id: runId, preserved_events: replay.length, execution_attempt: { determination: demo.deniedAttempt.determination, permitted: demo.deniedAttempt.executionPermitted, receipt_hash: demo.deniedAttempt.receipt.hash }, pre_preservation_verification: verification, post_preservation_verification: post }, { status: ok ? 200 : 409 });
}
