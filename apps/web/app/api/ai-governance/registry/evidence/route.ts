...
export async function POST(request: NextRequest) {
  try {
    ...
    const evidenceRelationship = stringValue(formData.get('evidenceRelationship'));
    const evidenceClassification = stringValue(formData.get('evidenceClassification'));
    ...
    const uploadedPath = `${user.id}/${submissionId}/${evidence.id}/${safeFilename}`;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (evidence.storage_bucket && evidence.storage_path) {
      const { error: storageError } = await supabase.storage.from(evidence.storage_bucket).remove([evidence.storage_path]);
