...
function createAdminClient() {
  ...
  if (!url || !serviceRoleKey) throw new Error('Registration recovery is missing Supabase server configuration.');
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to load registration recovery.' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, recovery: data, notice: 'Incomplete registration preserved to the signed-in account. This is recovery state only, not a Registry submission.' });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to preserve registration recovery.' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Unable to update registration recovery.' }, { status: 500 });
  }
}
