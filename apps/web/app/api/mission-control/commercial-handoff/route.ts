...
export async function GET() {
  ...
  const intersection=(downstream:Set<string>,upstream:Set<string>)=>new Set([...downstream].filter(id=>upstream.has(id)));
  ...
  const gatewayToWorkspaceVisitIds=intersection(workspaceVisitIds,gatewayVisitIds);
  const workspaceToOfferVisitIds=intersection(offerVisitIds,workspaceVisitIds);
  const offerToClickVisitIds=intersection(clickVisitIds,offerVisitIds);
  const registryTrials=degraded.trials?[]:trials.filter(trial=>{const visitId=String(trial.source_visit_id||'');const metadata=trial.metadata&&typeof trial.metadata==='object'?trial.metadata as Record<string,unknown>:{};return !adminUserIds.has(String(trial.user_id||''))&&Boolean(visitId)&&offerToClickVisitIds.has(visitId)&&(metadata.commercial_source==='registry'||Boolean(metadata.registry_identifier));});
  ...
  return NextResponse.json({generatedAt:new Date().toISOString(),windowDays:30,summary:{gatewayReachedEvents:gatewayEvents.length,gatewayReachedVisitors:gatewayVisitors,workspaceOpenedEvents:workspaceEvents.length,workspaceOpenedVisitors,offerViewedEvents:offerViewedEvents.length,offerViewedVisitors,ctaClickEvents:ctaClickEvents.length,ctaClickVisitors,trialActivated:degraded.trials?null:registryTrials.length,activeTrials:degraded.trials?null:activeTrials.length,convertedTrials:degraded.trials?null:convertedTrials.length,paidSubscriptions:degraded.paid?null:paidSubscriptions.length,gatewayToWorkspaceRate:pct(gatewayToWorkspaceVisitIds.size,gatewayVisitors),workspaceToOfferRate:pct(workspaceToOfferVisitIds.size,workspaceOpenedVisitors),clickThroughRate:pct(offerToClickVisitIds.size,offerViewedVisitors),clickToTrialRate:!degraded.trials?pct(registryTrials.length,offerToClickVisitIds.size):null},tracking:{gatewayReached:'AVAILABLE',workspaceOpened:'AVAILABLE',offerViewed:'AVAILABLE',ctaClicked:'AVAILABLE',trialStarted:degraded.trials?'TRACKING_UNAVAILABLE':'AVAILABLE',paid:degraded.paid?'TRACKING_UNAVAILABLE':'AVAILABLE'},boundary:'Registered participant continuation is measured as a separate voluntary commercial progression. Stage-transition rates use same-visit cohort intersections so downstream activity cannot be credited to an unrelated upstream cohort. Gateway reach and workspace-open events do not alter Registry status or establish consent. Offer views and CTA clicks use exact behavioral telemetry labels. Trial activation is counted only when source_visit_id belongs to the same Registry offer-to-CTA cohort and Registry source metadata is preserved. Paid is asserted only from authoritative subscription attribution; unavailable attribution is TRACKING_UNAVAILABLE, never zero.'},{headers:{'Cache-Control':'no-store, max-age=0'}});
}
