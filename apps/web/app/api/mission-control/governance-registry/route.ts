...
export async function GET() {
  try {
    ...
    const [{ data: notifications, error: readError }, { data: summaryRows, error: summaryError }] = await Promise.all([
    ...
    if (readError || summaryError) {
      console.error('TA-14 Mission Control Registry Watch read failed.', { readError, summaryError });
