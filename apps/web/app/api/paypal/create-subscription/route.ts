...
export async function POST(req:NextRequest){const c=cfg();if(!c)return NextResponse.json({error:'PAYPAL_CONFIGURATION_MISSING'},{status:503});let body:{planKey?:string;customerReference?:string};try{body=await req.json()}catch{return NextResponse.json({error:'INVALID_REQUEST_BODY'},{status:400})}const key=body.planKey?.trim()||'';if(!isPlan(key))return NextResponse.json({error:'INVALID_PLAN'},{status:400});const configuredPlanId=process.env[ENV_KEYS[key]]?.trim();const planId=configuredPlanId||(c.environment==='live'?LIVE_PLAN_IDS[key]:'');if(!planId)return NextResponse.json({error:'PAYPAL_PLAN_NOT_CONFIGURED',planKey:key},{status:503});const custom=(body.customerReference||'').replace(/[^a-zA-Z0-9._:@+-]/g,'-').slice(0,127)||undefined;try{const access=await token(c);const r=await fetch(`${c.base}/v1/billing/subscriptions`,{method:'POST',headers:{Authorization:`Bearer ${access}`,'Content-Type':'application/json',Accept:'application/json',Prefer:'return=representation'},body:JSON.stringify({plan_id:planId,custom_id:custom,application_context:{brand_name:'TA-14 Authority',locale:'en-US',shipping_preference:'NO_SHIPPING',user_action:'SUBSCRIBE_NOW',return_url:'https://TA14exchange.com/workspace/ai-governance/pricing?payment=approved',cancel_url:'https://TA14exchange.com/workspace/ai-governance/pricing?payment=cancelled'}}),cache:'no-store'});const data=await r.json();if(!r.ok)return NextResponse.json({error:'PAYPAL_SUBSCRIPTION_CREATION_FAILED',detail:data},{status:502});return NextResponse.json({subscriptionId:data.id,status:data.status,approvalUrl:data.links?.find((x:{rel?:string})=>x.rel==='approve')?.href??null,planKey:key,environment:c.environment,boundary:'Subscription payment grants only the purchased access tier. It does not create certification, approval, admissibility, endorsement, or a favorable governance determination.'},{status:201})}catch(e){console.error('TA14_PAYPAL_SUBSCRIPTION_ERROR',e);return NextResponse.json({error:'PAYPAL_SUBSCRIPTION_SERVICE_UNAVAILABLE'},{status:503})}}
