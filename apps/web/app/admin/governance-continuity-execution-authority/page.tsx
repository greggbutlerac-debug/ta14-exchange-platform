...
export default function GovernanceContinuityExecutionAuthorityPage() {
  ...
  return <main style={{ minHeight: '100vh', background: '#f4f6f8', color: '#17212b', fontFamily: 'Arial,sans-serif' }}>
    <div style={{ maxWidth: 1120, margin: '0 auto', padding: '52px 24px 90px' }}>
      ...
      <section style={{ ...card, marginTop: 22 }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: 1.3, color: '#66717c' }}>GOVERNING CHAIN</div><h2 style={{ margin: '8px 0 14px', fontSize: 25 }}>Execution-authority boundary</h2><div style={{ padding: '17px 18px', background: '#17212b', color: '#fff', borderRadius: 10, fontWeight: 700, lineHeight: 1.7 }}>Reality → Record → Continuity → Admissibility → Binding → Commit → Execution → Outcome</div><p style={{ lineHeight: 1.65, color: '#52606c' }}>A prior approval does not automatically survive a material change. Binding exists only while evidence continuity, admissibility, authority scope, and present standing all remain valid.</p></section>

      <section style={{ ...card, marginTop: 22, overflowX: 'auto' }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: 1.3, color: '#66717c' }}>R2 STATE TRANSITION</div><h2 style={{ margin: '8px 0 18px', fontSize: 25 }}>Standing challenge, refusal, and restoration</h2><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}><thead><tr><th style={head}>Stage</th><th style={head}>Determination</th><th style={head}>Standing</th><th style={head}>Binding</th><th style={head}>Reason</th></tr></thead><tbody>{stages.map((row) => <tr key={row[0]}>{row.map((value, i) => <td key={i} style={cell}>{i === 1 ? <b>{value}</b> : value}</td>)}</tr>)}</tbody></table></section>
      ...
      <section style={{ ...card, marginTop: 22 }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: 1.3, color: '#66717c' }}>DETERMINISTIC IDENTITIES</div><h2 style={{ margin: '8px 0 16px', fontSize: 25 }}>Current demonstration receipts</h2>{[['Baseline', run.baseline.receipt], ['Challenged', run.challenged.receipt], ['Restored', run.restored.receipt]].map(([label, receipt]) => <div key={String(label)} style={{ padding: '13px 0', borderTop: '1px solid #e7ebef' }}><b>{String(label)}</b><div style={{ marginTop: 5, fontFamily: 'monospace', fontSize: 12, overflowWrap: 'anywhere', color: '#52606c' }}>{typeof receipt === 'object' && receipt ? `${receipt.replayId}\n${receipt.hash}` : ''}</div></div>)}</section>

      <section style={{ ...card, marginTop: 22 }}><div style={{ fontSize: 12, fontWeight: 800, letterSpacing: 1.3, color: '#66717c' }}>BUILD BOUNDARY</div><h2 style={{ margin: '8px 0 12px', fontSize: 25 }}>What R1 + R2 prove — and what they do not</h2><p style={{ lineHeight: 1.65 }}><b>Established:</b> governed asset identity, scoped authority, evidence standing, material-change challenge, fail-closed standing determination, binding-scope collapse, challenged execution refusal, reauthorization, SHA-256 receipt identity, owner authentication, append-only production chronology, preserved authority inputs, and persisted semantic replay verification.</p><p style={{ lineHeight: 1.65, marginBottom: 0 }}><b>Not claimed:</b> OrchestrAI integration, OMG compatibility, customer deployment, Registry standing, external third-party execution control, regulatory certification, or automatic control of systems outside this bounded GCEA production demonstration.</p></section>
    </div>
  </main>;
}
