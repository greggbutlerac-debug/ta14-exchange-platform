...
export default function PlayerCommandCenter() {
  ...
  useEffect(() => {
    ...
    async function loadPlayerHistory() {
      ...
      const [recent, count, bestScore, bestStreak, fastest] = await Promise.all([
        recentQuery,
        countQuery,
        bestScoreQuery,
        bestStreakQuery,
      ...
      const firstError = recent.error || count.error || bestScore.error || bestStreak.error || fastest.error;
      if (firstError) {
  ...
  return (
    <section className="playerCommand">
      <style>{`
 .playerCommand{margin:0 14px 100px;border:1px solid #1b4a5d;border-radius:18px;background:linear-gradient(135deg,#04131e,#020912);padding:18px;color:#dff8ff}.pcHead{display:flex;justify-content:space-between;gap:20px;align-items:end;border-bottom:1px solid #163847;padding-bottom:12px}.pcHead small{color:#65eaff;font-size:9px;font-weight:1000;letter-spacing:.15em}.pcHead h2{margin:5px 0 0;font-size:24px}.pcStatus{font-size:9px;color:#ffd363;text-align:right}.pcGrid{display:grid;grid-template-columns:310px 1fr;gap:16px;margin-top:16px}.pcCard{border:1px solid #173a4b;border-radius:14px;background:#06131d;padding:14px}.pcCard h3{margin:0 0 10px;font-size:10px;letter-spacing:.12em;color:#69eaff}.pcInputs{display:grid;gap:8px}.pcInputs input{width:100%;border:1px solid #24495a;border-radius:9px;background:#020a10;color:white;padding:10px}.pcButtons{display:flex;gap:7px;margin-top:9px;flex-wrap:wrap}.pcButtons button{border:1px solid #3a7186;border-radius:9px;background:#092333;color:#e8fcff;padding:9px 11px;font-weight:900;font-size:9px;cursor:pointer}.pcButtons button.primary{border-color:#ffd363;color:#ffe7a0;background:#2b2208}.rankCall{margin-top:12px;padding:11px;border:1px solid #2e6555;border-radius:10px;color:#72efbb;background:#062019}.statGrid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-top:10px}.statTile{border:1px solid #173a4b;border-radius:10px;background:#04101a;padding:10px}.statTile small{display:block;font-size:8px;color:#718e9a;letter-spacing:.08em}.statTile strong{display:block;margin-top:4px;color:#ffd363;font-size:16px}.leaderRows,.historyRows{display:grid;gap:5px}.leader{display:grid;grid-template-columns:42px minmax(120px,1fr) 100px 85px 85px;gap:8px;align-items:center;padding:8px 10px;border:1px solid #123342;border-radius:9px;background:#04101a;font-size:10px}.leader.me{border-color:#ffd363;background:#241d08}.leader strong{color:#ffd363}.leader span:nth-child(n+3){text-align:right;color:#9fc2cf}.historyCard{grid-column:1/-1}.runRow{display:grid;grid-template-columns:120px minmax(100px,1fr) 90px 80px 80px 90px;gap:8px;align-items:center;padding:9px 10px;border:1px solid #123342;border-radius:9px;background:#04101a;font-size:10px}.runRow strong{color:#ffd363}.runRow span:nth-child(n+3){text-align:right;color:#9fc2cf}.runDate{color:#6e95a4!important;text-align:left!important}.empty{color:#688895;font-size:11px;padding:20px;text-align:center}@media(max-width:850px){.pcGrid{grid-template-columns:1fr}.leader{grid-template-columns:35px 1fr 80px}.leader span:nth-child(4),.leader span:nth-child(5){display:none}.pcHead{align-items:start;flex-direction:column}.pcStatus{text-align:left}.historyCard{grid-column:auto}.runRow{grid-template-columns:1fr 80px 70px}.runRow span:nth-child(4),.runRow span:nth-child(5),.runRow span:nth-child(6){display:none}.statGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
 `}</style>
            ...
            <>
              ...
              <div className="statGrid">
                <div className="statTile">
                  ...
                  <strong>{stats.bestScore.toLocaleString()} XP</strong>
                </div>
                <div className="statTile">
                ...
                <div className="statTile">
                  <small>FASTEST RUN</small>
                  <strong>{formatDuration(stats.fastestSeconds)}</strong>
                </div>
                <div className="statTile">
            ...
            <>
              <div className="pcInputs">
                <input
                  ...
                  placeholder="Player name (needed for sign up)"
                  maxLength={32}
                />
          ...
          <div className="leaderRows">
            {leaders.length ? (
              leaders.map((p, i) => {
                ...
                return (
                  <div className={`leader ${p.user_id === user?.id ? "me" : ""}`} key={p.user_id}>
                    <strong>#{i + 1}</strong>
                    <span>{p.display_name}</span>
                    <span>{Number(p.best_score).toLocaleString()} XP</span>
                    <span>{acc}% ACC</span>
                    <span>{p.games_played} RUNS</span>
                  </div>
                );
              })
            ) : (
        ...
        {user && (
          <div className="pcCard historyCard">
            ...
            <div className="historyRows">
              {runs.length ? (
                runs.map((run) => {
                  const accuracy = run.questions_answered
                    ? Math.round((run.correct_answers / run.questions_answered) * 100)
                    : 0;
                  return (
