 ...
 return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 50% 0,#0a2730 0,#031015 35%,#010609 78%)',color:'#efffff',fontFamily:'Inter,system-ui,sans-serif'}}>
  <div style={{width:'min(1220px,calc(100% - 34px))',margin:'0 auto',padding:'26px 0 100px'}}>
   ...
   <section style={{padding:'72px 0 22px'}}>
    ...
    <div style={{padding:25,border:'1px solid #214a51',borderRadius:15,background:'#04151c'}}><strong style={{fontSize:25,color:'#71e7df'}}>{d[0]}</strong><p style={{fontSize:15,color:'#a6bcc2',lineHeight:1.7}}>{d[1]}</p><b style={{fontSize:11,color:'#e7c76e'}}>EXECUTION AUTHORITY: NOT ESTABLISHED BY FEDERATION</b></div>
   </section>
   ...
   <section style={{padding:'70px 0',borderTop:'1px solid #143139'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:10,marginTop:24}}>{blocked.map(x=><div key={x} style={{padding:'24px 15px',border:'1px solid #6a402f',borderRadius:12,background:'rgba(74,25,13,.18)',textAlign:'center',fontWeight:950,color:'#ffb29a'}}>⊘ {x}<div style={{fontSize:9,color:'#a98275',marginTop:8}}>BLOCKED AT FEDERATION BOUNDARY</div></div>)}</div>
   </section>
   ...
   <section style={{padding:'70px 0',borderTop:'1px solid #143139'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'minmax(220px,.8fr) minmax(280px,1.4fr)',gap:14,marginTop:25}}>
     <div style={{display:'grid',gap:8}}>{failures.map((x,i)=><button key={x[0]} onClick={()=>setFailure(i)} style={{cursor:'pointer',textAlign:'left',padding:15,borderRadius:10,border:i===failure?'1px solid #71e7df':'1px solid #203f46',background:i===failure?'rgba(45,141,143,.16)':'#041318',color:i===failure?'#8ff5ee':'#a1b6bc',fontWeight:900}}>{x[0]}</button>)}</div>
     <div style={{padding:26,border:'1px solid #214a51',borderRadius:14,background:'#04151c'}}><div style={{fontSize:10,color:'#8ba2a8'}}>GOVERNED RESULT</div><strong style={{display:'block',fontSize:28,color:'#e7c76e',margin:'10px 0'}}>{f[1]}</strong><p style={{color:'#a5bbc1',lineHeight:1.7}}>{f[2]}</p><div style={{marginTop:18,fontSize:10,color:'#79d9d2',fontWeight:900}}>LOCAL EXECUTION OBSERVED: FALSE</div></div>
    </div>
   </section>
