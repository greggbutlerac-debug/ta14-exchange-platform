...
export default function TransientPM25AggregationDemo(){
  return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 50% 0%,#3b2910 0,#151007 38%,#060504 78%)',color:'#fff8e9',fontFamily:'Arial,Helvetica,sans-serif'}}>
    ...
    <ConsequenceLab eyebrow="LIVE TIME-RESOLUTION LAB" title="Keep the math correct. Change the evidentiary question." intro="A daily average can be mathematically valid while being the wrong evidence resolution for a transient-event proposition. Change what is preserved and watch the modeled determination respond." actionLabel="HIDE THE TRANSIENT EVENT →" allowReason="The native event record and declared aggregation remain distinguishable and fit the bounded proposition under examination." conditions={[
{id:'native',label:'NATIVE EVENT RECORD',question:'Is the short-duration PM2.5 event preserved at a resolution capable of showing it?',healthy:'VISIBLE',failed:'AGGREGATED AWAY',failure:'HOLD',failureReason:'The proposition asks whether a transient event occurred, but the relied-upon record no longer preserves it.'},
{id:'window',label:'AGGREGATION WINDOW',question:'Is the averaging window explicitly declared rather than silently substituted?',healthy:'DECLARED',failed:'UNDECLARED',failure:'HOLD',failureReason:'The aggregation window is not explicit enough for proposition-specific reliance.'},
{id:'source',label:'SOURCE / CONTEXT',question:'Are instrument identity, missingness and relevant context preserved?',healthy:'PRESERVED',failed:'INCOMPLETE',failure:'HOLD',failureReason:'Source or contextual evidence is incomplete.'},
{id:'authority',label:'INTERVENTION AUTHORITY',question:'If consequence is proposed, is authority separately established rather than inferred from the event?',healthy:'SEPARATE',failed:'INFERRED',failure:'DENY',failureReason:'Event evidence does not itself establish intervention authority.'}
]} />
