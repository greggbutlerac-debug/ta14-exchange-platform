...
export default function EnvironmentalEvidenceConformanceShowcase(){
  ...
  useEffect(()=>{
    if(typeof window!=='undefined' && window.location.hash==='#ta14-governed-state') setMode('governed');
  },[]);

  const showGoverned=()=>{
    setMode('governed');
    if(typeof window!=='undefined') window.history.replaceState(null,'','#ta14-governed-state');
    window.setTimeout(()=>comparisonRef.current?.scrollIntoView({behavior:'smooth',block:'start'}),0);
  };
  ...
  return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 50% 0%,#153d32 0,#071511 36%,#020605 82%)',color:'#edf9f4',fontFamily:'Arial,Helvetica,sans-serif'}}>
    ...
    <section style={{maxWidth:1180,margin:'0 auto',padding:'72px 24px 100px'}}>
      ...
      <div id='ta14-governed-state' ref={comparisonRef} style={{scrollMarginTop:24,marginTop:18,padding:24,border:'1px solid rgba(120,240,190,.2)',borderRadius:20,background:'rgba(5,20,15,.72)'}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:20,alignItems:'end',flexWrap:'wrap'}}><div><small style={{color:'#71e5ad',fontWeight:900,letterSpacing:'.12em'}}>SELECTED SYSTEM</small><h2 style={{fontFamily:'Georgia,serif',fontSize:34,margin:'7px 0'}}>{selected[1]}</h2><p style={{color:'#a9c0b5',margin:0}}>{selected[2]}</p></div><div style={{display:'flex',gap:8}}><button onClick={()=>setMode('monitoring')} style={tab(mode==='monitoring')}>MONITORING STATE</button><button onClick={showGoverned} style={tab(mode==='governed')}>TA-14 GOVERNED STATE</button></div></div>
