...
export default function ProofOverPromiseChicagoShowroom() {
  ...
  const [intake, setIntake] = useState<{intakeId:string;recordedAt:string;payloadSha256:string}|null>(null);
  ...
  const freezeExamination = async () => {
    if (!submittedProof.trim() || !submittedConsequence.trim() || freezing) return;
    setFreezing(true); setFreezeError(''); setReceiptOpen(false); setIntake(null);
    try {
      const response = await fetch('/api/chicago-examination-intake', {
        method: 'POST',
        headers: {'content-type':'application/json'},
        body: JSON.stringify({ proof: submittedProof, proposedConsequence: submittedConsequence, evidenceState: proof === 'supported' ? 'CURRENT + ATTRIBUTABLE' : 'INSUFFICIENT', authorityState: authority === 'supported' ? 'CURRENT' : 'EXPIRED', standingState: standing === 'supported' ? 'ESTABLISHED' : 'UNRESOLVED', executionScope: scope === 'supported' ? 'IN BOUNDS' : 'OUTSIDE', determination: result })
      });
      const body = await response.json();
      if (!response.ok || !body.recorded) throw new Error(body.error || 'Unable to record intake.');
      setIntake({intakeId:body.intakeId,recordedAt:body.recordedAt,payloadSha256:body.payloadSha256});
      setReceiptOpen(true);
    } catch (error) { setFreezeError(error instanceof Error ? error.message : 'Unable to record intake.'); }
    ...
    <main style={{
      ...
      <div style={{maxWidth:1280,margin:'0 auto'}}>
        ...
        <section style={{marginTop:24,padding:'clamp(34px,6vw,68px)',border:'1px solid rgba(113,242,182,.3)',borderRadius:28,background:'radial-gradient(circle at 76% 0%,rgba(113,242,182,.11),transparent 28%),linear-gradient(145deg,rgba(8,36,48,.96),rgba(3,11,19,.98))'}}>
          ...
          <div id="bring-proof" style={{marginTop:28,padding:'clamp(22px,4vw,38px)',borderRadius:20,border:'1px solid rgba(111,220,255,.2)',background:'rgba(2,10,17,.58)',scrollMarginTop:24}}><div style={{fontSize:11,fontWeight:950,letterSpacing:'.18em',color:'#70dcff'}}>BRING YOUR OWN VERIFIED RESULT</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:12,marginTop:16}}><label style={{fontSize:11,fontWeight:900,color:'#9bb1bc'}}>WHAT DID YOU PROVE?<textarea value={submittedProof} onChange={e=>{setSubmittedProof(e.target.value);setReceiptOpen(false);setIntake(null)}} style={{display:'block',width:'100%',minHeight:105,marginTop:8,padding:14,borderRadius:12,border:'1px solid rgba(111,220,255,.18)',background:'#020a11',color:'#eef8fb',font:'inherit',boxSizing:'border-box'}} /></label><label style={{fontSize:11,fontWeight:900,color:'#9bb1bc'}}>WHAT CONSEQUENCE DOES THAT PROOF PROPOSE?<textarea value={submittedConsequence} onChange={e=>{setSubmittedConsequence(e.target.value);setReceiptOpen(false);setIntake(null)}} style={{display:'block',width:'100%',minHeight:105,marginTop:8,padding:14,borderRadius:12,border:'1px solid rgba(111,220,255,.18)',background:'#020a11',color:'#eef8fb',font:'inherit',boxSizing:'border-box'}} /></label></div><button disabled={!submittedProof.trim()||!submittedConsequence.trim()} onClick={freezeExamination} style={{marginTop:14,padding:'13px 18px',borderRadius:11,border:'1px solid rgba(113,242,182,.4)',background:'rgba(113,242,182,.13)',color:'#e4fff1',fontWeight:1000}}>{freezing?'RECORDING FROZEN STATE…':'FREEZE THIS EXAMINATION →'}</button>{freezeError&&<div style={{marginTop:12,color:'#ff9aa5',fontSize:12,fontWeight:900}}>{freezeError}</div>}{receiptOpen&&intake&&<div style={{marginTop:18,padding:20,borderRadius:15,border:`1px solid ${color}55`}}><b>PERSISTENT EXAMINATION INTAKE · RECORDED</b><div style={{marginTop:10,lineHeight:1.7}}><b>INTAKE ID:</b> {intake.intakeId}<br/><b>RECORDED AT:</b> {new Date(intake.recordedAt).toISOString()}<br/><b>SHA-256:</b> <span style={{overflowWrap:'anywhere'}}>{intake.payloadSha256}</span><br/><b>EXAMINATION VERSION:</b> CHICAGO-POP-v0.2-CANDIDATE<br/><b>PROOF:</b> {submittedProof}<br/><b>PROPOSED CONSEQUENCE:</b> {submittedConsequence}<br/><b>ADMISSIBLE EVIDENCE STATE:</b> {proof==='supported'?'CURRENT + ATTRIBUTABLE':'INSUFFICIENT'}<br/><b>APPLICABLE AUTHORITY STATE:</b> {authority==='supported'?'CURRENT':'EXPIRED'}<br/><b>ESTABLISHED STANDING STATE:</b> {standing==='supported'?'ESTABLISHED':'UNRESOLVED'}<br/><b>EXECUTION SCOPE:</b> {scope==='supported'?'IN BOUNDS':'OUTSIDE'}<br/><b>DETERMINATION:</b> <span style={{color,fontWeight:1000}}>{result}</span></div><div style={{marginTop:8,fontSize:11,color:'#78909b'}}>PERSISTENT INTAKE ONLY. This frozen state is recorded, but it is not a TA-14 durable examination receipt, certification, endorsement, admissibility finding, authority determination, standing determination, or execution authorization.</div></div>}</div>
