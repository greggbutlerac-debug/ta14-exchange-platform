...
const STORAGE_KEY = "ta14-academy-evidence-conflict-resolution-lab-v2";
...
const missions: Mission[] = [
  {
    id: "sensor-vs-inspection",
    title: "Sensor Record Versus Physical Inspection",
    domain: "Environmental integrity governance",
    consequence: "A facility may be reopened while moisture remains concealed inside an occupied wall assembly.",
    routeSummary: "Continuous sensors show acceptable room conditions, while a qualified inspection records elevated material moisture behind the finished surface. Operations argues that the live sensor trend should control.",
    evidencePackage: [
      "Seven-day room temperature, relative humidity, and dew-point trend with attributable device identity.",
      "Timestamped moisture-meter readings from three concealed locations.",
      "Photographs showing staining and material deformation.",
      "A maintenance note stating that the room feels dry and has no visible standing water.",
    ],
    conflictingClaims: [
      "Ambient conditions are presently acceptable.",
      "Material conditions remain outside the release threshold.",
      "The sensor package does not measure concealed material moisture.",
    ],
    hiddenRisk: "The records are not mutually exclusive; they measure different realities. Treating one as a replacement for the other would create a false conflict resolution.",
    expectedDecision: "HOLD",
    expectedSeverity: "MATERIAL",
    expectedDefects: ["reality", "admissibility", "execution", "outcome"],
    explanation: "The ambient record cannot defeat direct evidence of concealed material moisture. The conflict must be classified by measurement domain, and reopening remains on HOLD until the material condition satisfies the governing threshold.",
  },
  {
    id: "model-evaluations",
    title: "Competing AI Model Evaluation Results",
    domain: "Enterprise AI governance",
    consequence: "A model release could impose automated consequences despite unresolved disagreement between validation environments.",
    routeSummary: "The development team reports that the model passed its benchmark suite. An independent reviewer reports materially higher false-positive rates when the model is tested on a current production sample.",
    evidencePackage: [
      "Developer benchmark report using a curated historical dataset.",
      "Independent evaluation using a recent stratified production sample.",
      "Model card identifying the intended population and stated limitations.",
      "Release request asserting that the independent result is an outlier.",
    ],
    conflictingClaims: [
      "The model satisfies the internal release benchmark.",
      "The model fails the production-representative error threshold.",
      "The two evaluations use different populations and sampling methods.",
    ],
    hiddenRisk: "The favored benchmark may be valid for its dataset while still being inadmissible for the actual deployment population.",
    expectedDecision: "ESCALATE",
    expectedSeverity: "CRITICAL",
    expectedDefects: ["record", "continuity", "admissibility", "binding", "commit"],
    explanation: "The disagreement concerns deployment correspondence, not merely statistical noise. A qualified independent authority must resolve population validity, threshold selection, and release conditions before commitment.",
  },
  {
    id: "service-records",
    title: "Technician Narrative Versus Instrument Record",
    domain: "HVAC diagnostic governance",
    consequence: "Equipment may be condemned and replaced based on a narrative that conflicts with preserved electrical and airflow evidence.",
    routeSummary: "A technician recommends compressor replacement after hearing abnormal sound. The preserved diagnostic package shows acceptable winding resistance, insulation condition, current draw, refrigerant conditions, and airflow after a corrected capacitor defect.",
    evidencePackage: [
      "Time-synchronized electrical measurements before and after capacitor replacement.",
      "Static pressure, airflow, superheat, and subcooling records.",
      "Body-worn video preserving the observed sound and service sequence.",
      "Technician narrative recommending replacement without a stated failure threshold.",
    ],
    conflictingClaims: [
      "The sound indicates imminent compressor failure.",
      "The measured system condition does not establish compressor failure.",
      "A corrected electrical defect explains the original operating symptom.",
    ],
    hiddenRisk: "Experience-based concern may justify continued monitoring, but it cannot silently become a diagnostic determination without evidence-bound criteria.",
    expectedDecision: "DENY",
    expectedSeverity: "MATERIAL",
    expectedDefects: ["admissibility", "binding", "execution", "outcome"],
    explanation: "The replacement recommendation is not supported by the preserved diagnostic record. The unsupported intervention should be denied while a bounded monitoring plan remains available.",
  },
  {
    id: "identity-record",
    title: "Identity Match With Contradictory Records",
    domain: "Administrative decision governance",
    consequence: "A person may lose access or benefits because two records with similar identifiers have been merged.",
    routeSummary: "An automated system links an adverse record to an applicant using name and birth month. A verified government identifier and address history indicate that the adverse record belongs to another person.",
    evidencePackage: [
      "Automated similarity match with confidence score and matching fields.",
      "Verified identity document and unique identifier.",
      "Address and employment history covering the disputed period.",
      "Adverse source record lacking the unique identifier used by the applicant.",
    ],
    conflictingClaims: [
      "The similarity model reports a probable match.",
      "The authoritative identity record reports a different person.",
      "The adverse record contains insufficient unique identifiers for binding.",
    ],
    hiddenRisk: "A probability score may trigger review, but it cannot replace authoritative identity evidence when consequence is individualized.",
    expectedDecision: "DENY",
    expectedSeverity: "CRITICAL",
    expectedDefects: ["reality", "record", "admissibility", "binding"],
    explanation: "The adverse record cannot be bound to the applicant. The similarity result is a review signal, not admissible identity proof, and the proposed adverse execution must be denied.",
  },
  {
    id: "clean-conflict",
    title: "Apparent Conflict Resolved by Version History",
    domain: "Governed records",
    consequence: "A valid execution could be delayed if superseded records are incorrectly treated as simultaneous contradictions.",
    routeSummary: "Two approved procedures specify different thresholds. The later procedure includes a complete change record, express supersession language, valid authority, and a future effective date that has now passed.",
    evidencePackage: [
      "Original approved procedure with version, authority, and effective period.",
      "Replacement procedure with preserved redline and approval record.",
      "Supersession notice linking both records.",
      "System clock and jurisdiction record establishing the current effective date.",
    ],
    conflictingClaims: [
      "The original threshold remains visible in the archive.",
      "The replacement threshold is currently effective.",
      "Version history explains why both records exist without making both operative.",
    ],
    hiddenRisk: "Conflict resolution must not erase the older record; it must preserve it as historical while identifying the presently binding version.",
    expectedDecision: "ALLOW",
    expectedSeverity: "NONE",
    expectedDefects: [],
    explanation: "The apparent conflict is resolved through attributable version history, authority, supersession, and effective date. The current procedure may govern while the prior record remains preserved.",
  },
];
...
export default function EvidenceConflictResolutionLabPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (parsed.version === "2.0" && parsed.reviews) {
  ...
  function exportMission() {
    const payload = {
      recordType: "TA-14 Academy Evidence Conflict Resolution Record",
      recordVersion: "2.0",
      exportedAt: new Date().toISOString(),
      principle: "No resolved evidence conflict. No admissible determination.",
      mission: activeMission,
      review: activeReview,
      metrics: missionMetrics,
      structuralWarning: decisionConflict,
    };
    downloadJson(`ta14-evidence-conflict-resolution-${activeMission.id}-${Date.now()}.json`, payload);
  }

  function exportLaboratory() {
    const payload = {
      recordType: "TA-14 Academy Evidence Conflict Resolution Lab Transcript",
      recordVersion: "2.0",
      exportedAt: new Date().toISOString(),
      principle: "No resolved evidence conflict. No admissible determination.",
    ...
    downloadJson(`ta14-evidence-conflict-resolution-laboratory-${Date.now()}.json`, payload);
  }
    ...
    <main className="laboratoryPage">
      ...
      <header className="topbar">
        <Link className="brand" href="/academy">
          ...
          <span className="brandText">
            ...
            <small>Evidence Conflict Resolution Lab</small>
          </span>
        </Link>

        <nav aria-label="Academy navigation">
          <Link href="/academy/admissibility">Admissibility</Link>
          <Link href="/academy/determination">Determination</Link>
      ...
      <section className="hero">
        <div className="heroCopy">
          <p className="eyebrow">Resolve contradiction before determination</p>
          <h1>
            Available evidence is not automatically
            <em> resolved evidence.</em>
          </h1>
          <p className="heroSummary">
            Compare competing records, preserve the contradiction, test continuity and authority, and determine whether the evidence package can support a governed conclusion. This laboratory teaches conflict resolution without erasing uncertainty or favoring the most convenient account.
          </p>

          <div className="heroRule">
            <span>Governing principle</span>
            <strong>No resolved evidence conflict. No admissible determination.</strong>
          </div>
        </div>
        ...
        <aside className="heroStatus">
          ...
          <dl>
            ...
            <div><dt>Version</dt><dd>Laboratory 2.0</dd></div>
          </dl>
          <button type="button" className="secondaryButton full" onClick={exportLaboratory}>Export laboratory transcript</button>
        </aside>
      </section>

      <section className="missionStrip" aria-label="Conflict-resolution missions">
      ...
      <section className="missionOverview">
        <div className="missionTitleBlock">
          <p className="eyebrow">Active conflict-resolution mission</p>
        ...
        <div className="overviewGrid">
          ...
          <article>
            <span className="cardKicker">Conflicting claims</span>
            {activeMission.conflictingClaims.length > 0 ? <ul>{activeMission.conflictingClaims.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="emptyState">No additional conflicting claim is disclosed in the mission record.</p>}
          </article>
          <article className="riskCard">
            <span className="cardKicker">Conflict pressure</span>
      ...
      <footer className="pageFooter">
        ...
        <nav><Link href="/academy/determination">← Route Determination</Link><Link href="/academy/review">Continue to Review Workspace →</Link></nav>
      </footer>
