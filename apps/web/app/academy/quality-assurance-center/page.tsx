...
/*
 * TA-14 Academy Quality Assurance Center
 * Governing principle: No admissible evidence. No admissible execution.
 * Production workspace for institutional quality review, internal audit, CAPA,
 * continuous improvement, evidence preservation, risk, and executive reporting.
 */
...
type WorkspaceState = {
  schema: "TA14_ACADEMY_QUALITY_ASSURANCE_V1";
...
const STORAGE_KEY = "ta14-academy-quality-assurance-center-v1";
...
const seedProgram: ProgramProfile[] = [
  ...
  },
  {
    ...
    goals: ["Institutionalize advisoring review", "Publish annual quality assurance report"],
...
const seedActivities: LearningActivity[] = [
  {
    ...
    description: "Structured quality assurance activity with preserved participation evidence, declared outcomes, and bounded authority.",
  },
  {
    id: "ACT-002",
...
function validateWorkspace(value: unknown): value is WorkspaceState {
  ...
  return (
    candidate.schema === "TA14_ACADEMY_QUALITY_ASSURANCE_V1" &&
...
export default function ProgramDevelopmentCenterPage() {
  ...
  function exportJson() {
    ...
    addAudit("WORKSPACE_EXPORTED", "Quality Assurance Center", "ALLOW", "Exported complete JSON workspace.");
  ...
  function resetWorkspace() {
    if (!window.confirm("Reset the Quality Assurance Center to its institutional seed state?")) return;
    ...
    <main style={styles.page}>
      ...
      <header style={styles.header}>
        <div>
          ...
          <h1 style={styles.title}>Quality Assurance Center</h1>
...
function ActivityModal({ competencies, onClose, onSave }: { competencies: Competency[]; onClose: () => void; onSave: (activity: LearningActivity) => void }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); onSave({ id: uid("ACT"), title, provider: "TA-14 Academy", category, format: "WORKSHOP", status: "PLANNED", start: today(), end: today(), hours, seats: 12, enrolled: [], competencyIds: [competencyId], facilitator: "To be assigned", description: "New quality assurance activity pending facilitator and evidence-plan review." }); }
...
function ImprovementModal({ program, onClose, onSave }: { program: ProgramProfile[]; onClose: () => void; onSave: (plan: ImprovementPlan) => void }) {
  ...
  function submit(event: FormEvent) { event.preventDefault(); onSave({ id: uid("IMP"), programId, title, trigger, status: "OPEN", priority: "HIGH", opened: today(), due, owner: "Quality Assurance Committee", actions: ["Complete bounded review with program member"], checkpoints: ["Initial evidence review", "Midpoint check", "Final determination"], progress: 0, determination: "HOLD" }); }
